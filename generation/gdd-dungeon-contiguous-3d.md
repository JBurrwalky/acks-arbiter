...
Every balcony zone must be reachable by at least one of: (a) a door/corridor connection to the upper band's own circulation network, or (b) an internal grand stair rising from the atrium's main floor. The composer prefers (a) and adds (b) with probability per theme (a temple gallery reached only from the upper cloister is good design; so is a wizard's observation balcony with its own sweeping stair). If neither can be routed, the balcony ring is removed and the room degrades gracefully to a plain double-height hall (main zone only) — degradation is logged. **(DG-C3D.G, 2026-07-13):** the composer now honors this as a real GUARANTEE — a valid ≥5×5 atrium always keeps its ring because (b) an internal grand stair is force-carved whenever (a) no upper-band door reaches the ring, so degradation fires only on geometry too small for a stair (never under the §7.3 size gate). The earlier build (F.4) left `has_upper_door` detection broken (it tested inside-the-footprint, but the router places the ring door on the perimeter), so keep-vs-degrade collapsed to the 35% `internal_stair` flavor roll → ~88% degraded; the fix restored path (a) detection AND added the (b) guarantee, inverting the sweep to 32/32 kept. See conventions §123.
...
5. **Integration:** party walks entrance → band −1 via straight run with no teleport call; archer on hand-authored balcony has LOS + cover vs. atrium floor; character stepping off balcony falls 10', takes 1d6 (`rules/acore_combat_and_wounds.xml:797`); fog: balcony cells follow light radius + occlusion only — no reveal from room membership (per Jedidiah's 2026-07-06 ruling; already the `FogRevealEngine` behavior). **(DG-C3D.G, 2026-07-13):** shipped as `scenario_balcony_features.gd` against the re-authored `test_dungeon.json` ("Sunken Hall"). NOTE the archer-cover assertion reads `VoxelLOS.get_cover_value` directly — DG-C3D.G did NOT wire parapet cover into the combat to-hit pipeline (a cross-system change, deferred); the parapet is inert in combat until that wiring lands. Fog reveal is same-band per-cell (a balcony observer does not reveal the atrium floor a band below even with clear LOS across the void — v1 scope).
