...
**Demihuman & beastman families:** **Elvish** — one people (the Vaneth) in **three branches** of a shared Proto-Elvish tongue ("Vanethir"), authored in `data/conlang/family_elvish.json` + three culture kits: **Aelvaneth** (§5.2 #48, forest-and-river, smooth high-elf, deep ancestor of Germanic/Celtic/Slavic), **Xilvaneth** (#49, jungle-and-river, harsh dark-elf stand-in, deep ancestor of Mesoamerican), and **Thalvaneth** (#50, coastal-forest sea-elf, smooth Mediterranean, deep ancestor of Latinate/Hellenic/Punic). The branches share cognate consonant roots under per-branch sound-laws (Ael smooth / Xil harsh-glottal / Thal Mediterranean) and keep deity/mortal/common-noun names distinct via three ending-classes (divine = sacred + gendered; see `family_elvish.json` theonym_set._endings). **Dwarven** — one people (the Durn) in **three dialects** (small drift) of a self-contained isolate tongue, DURNKAD, in `data/conlang/family_dwarven.json` + three culture kits: **Khordurn** (§5.2 #51, any mountain but glacial/volcanic), **Gormdurn** (#52, volcanic; the fire-cult), and **Khraaldurn** (#53, glacial/tundra; the most withdrawn). Hard consonants, soft vowels, short hammer-rhythm syllables; the three share the divine ending -drumm/-drumma and an ancestor-first faith; not ancestral to any human family; the **beastman** tier now has a full kit too: **Kazhur** ("the maker-tongue", `data/conlang/family_beastman.json`) — the harsh Old-Semitic speech of the Chaotic sorcerers who bred the beastmen, REGRESSED per race into bestial sounds (glottal stops, clicks, hisses, growls, barks, yelps) across ten races (kobold, goblin, orc, hobgoblin, gnoll, troll, bugbear, troglodyte, lizardman, ogre); always Chaotic, totemic-shamanist, deity-morphs prefixed Gul- with no gendered ending. Beastmen stay mechanically stripped (catalog §5.3) and are NOT in setting-lore §5.2; the kit supplies naming/flavor only.
...
- **Demihuman & beastman kits** — ✅ Elvish DONE (Vanethir base + Aelvaneth/Xilvaneth/Thalvaneth, §5.2 #48-50), ✅ Dwarven DONE (Durnkad base + Khordurn/Gormdurn/Khraaldurn, §5.2 #51-53), and ✅ Beastman DONE (Kazhur base + 10 races: kobold/goblin/orc/hobgoblin/gnoll/troll/bugbear/troglodyte/lizardman/ogre; not in §5.2, naming/flavor only per catalog §5.3). The full naming-kit corpus is now authored.

## 18. Revision History

- **2026-06-13 (rev 9):** **Name-bank build implemented** (handoff Stage 5). `tools/build_name_banks.py` resolves kit inheritance and assembles static per-culture banks → `data/name_banks/<culture_id>.json` (65) + `_manifest.json`, loaded by `NameBankLoader` and gated by `tests/test_setting_name_banks.gd` (the tool's `--check` freshness mode). The §13 "static name bank … deterministic from the seed" is realized as a deterministic-by-construction offline build (no RNG — byte-identical rebuilds; per-campaign name *selection* is the part that will use `WorldGenRng` at Stage 6). The build leans on the authored kits (title ladders, deity renames, seed stocks pass through) and assembles the categories the kits don't enumerate — settlement/feature compounds + re-gendered/patronymic/epithet top-ups — harvesting the concrete examples embedded in `banks_patterns` for the flavor banks. The §14 validation runs in the tool (core categories ≥10, dedup, family inherited, title ladder, endonym-only). Curation note (still open, §17): the generic `compound()` (the per-culture `compounding_rule` is prose, not executable) and the long agglutinative compounds (Elvish/Nahuatl) are the obvious targets for the LLM curation content-pass; gendered *suffix* endings were confirmed non-universal (semantic/prefix/filiation gendering) and are not a validation gate. See coding_conventions.md §81.
