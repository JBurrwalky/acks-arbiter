...
| Bag of Devouring transfer-hook timer | `update_inventory_item_equip_state` checks the target container against `BagOfDevouringService.is_bag_of_devouring`; if true, calls `_maybe_start_bag_of_devouring_timer` which invokes `start_timer_on_first_item` (which itself short-circuits if the bag is non-empty). Single integration point — UI drag-drop, programmatic transfers, and any future inventory mutation path all flow through the same gate. Production RNG uses `RandomNumberGenerator.new().randomize()`; tests inject seeded RNG via the service's direct API. Future improvement noted: integrate `DiceSystem.roll_digital` for project-wide deterministic-seed support. | ✅ landed 2026-05-31; 2 new tests cover transfer-activates-timer + non-devouring-bag-does-not-activate |
