...
| Spell-effect pass: `panic` + `gaseous_form` + portcullis bypass | `panic` (Arcane L5; `pc_spell_catalog_f-u.xml:621-637`) effect block added to `data/spells/spell_catalog.json`: `area_at_point` sphere with `radius_feet: 240` centered on caster, `save_spec` vs Spells negates, `resolution` applies the existing `frightened` condition (condition_catalog.json — prevents_attacking + prevents_casting + flees_from_source) for 30 rounds. V1 deferred: the 10' safe-zone exclusion around the caster (targeting layer needs annulus support) + the explicit "running speed" movement modifier (V1 relies on the frightened condition's AI flee gating). `gaseous_form` (Arcane L3; `pc_spell_catalog_f-u.xml:90-126`) effect block added: `single_creature` self-target, `save_spec` vs Spells negates, `resolution` applies the existing `is_gaseous` EntityFlag with consumer metadata `{ac_override: 11, movement_rate_override_feet_per_round: 30, drops_carried_items_on_apply: true, immune_to_non_magical_weapons: true, passes_closed_doors_and_portcullis: true}`. V1 wires ONE consumer: the portcullis-passability bypass in `MovementResolver._can_enter_3d`. New `VoxelCell.is_passable_by_gaseous()` returns true for any air cell regardless of `door_state` (closed/locked/stuck doors + closed portcullises don't block gas). New "gaseous" branch in `_can_enter_3d` uses `is_passable_by_gaseous()` + skips support check (gas doesn't fall). Auto-detection: when `movement_type == "ground"` AND the mover (via roster lookup by `mover_id`) carries the `is_gaseous` flag, movement_type promotes to "gaseous" automatically — so existing callers (charging, pathfinding, AI movement) all get the bypass without explicit detection. Combatants expose flags via `get_flags()` (not a public `flags` property); auto-detect uses the accessor. V1 deferred consumers (forward-looking; flag metadata documents the contract): AC override to 11, movement rate override to 30'/round, immune to non-magical weapons in damage resolver, drops carried items on apply. **Unblocks**: Drums of Panic binds to `panic` (arcane caster_level 9, target_mode self via use_misc_magic_active); Potion of Gaseous Form binds to `gaseous_form` (arcane caster_level 5, target_mode self via drink_potion). | ✅ landed 2026-06-01; 9 new tests in `tests/test_panic_gaseous_form.gd` cover catalog bindings, panic mechanics, gaseous_form mechanics, portcullis bypass. EXPECTED_DEFER_KEYS in `test_magic_item_catalog.gd` updated (-2 items). Existing Drums of Panic + Potion of Gaseous Form tests inverted from deferred-shape to bound-shape; the test_magic_item_activator no-binding-deferred exemplar switched from Drums of Panic to Crystal Ball (still deferred Cluster D). |
