...
| Boots of Speed — flat +30' movement | `WornMagicEffectResolver._add_boots_of_speed` — flat additive modifier on `movement_rate` (flows through `CharacterData.get_effective_movement()`). V1 explicitly avoids the "permanent Haste" interpretation. Magnitude project default `BOOTS_OF_SPEED_MOVEMENT_BONUS = 30` (constant); flagged for Jedidiah ruling on whether RAW intends "double base move" or a specific numeric value. | ✅ landed 2026-05-29 (Tier 3 V1); 2 new tests cover the movement bonus + unequip clear |
