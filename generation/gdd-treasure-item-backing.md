...
**Status (2026-05-29):** foundation + DG-V1 integration + first-visit materialization landed.
...
- Remaining: per-cell interaction wiring (Commit 4 — lock/search/loot gates on top of materialization), retirement of `claim_room_hoards` (Commit 5).
...
- **Per-cell interaction**: `_resolve_loot` already accepts a cell parameter. Lock/search gates fire here (Pick Lock proficiency throw for locked; Search proficiency throw for hidden). Trap firing is a stub until the traps system lands. The materialization call now slots in here: when the player interacts with a cell, call `materialize_hoard_cell` first to lazy-promote any hoard at that cell, then open the loot modal on the returned cache_id (gated by lock state on the backing container item).
