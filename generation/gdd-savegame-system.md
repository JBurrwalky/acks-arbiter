...
**Depends on project GDDs:** [gdd-realtime-scheduler.md](gdd-realtime-scheduler.md) (scheduled-event queue serialization, per-party clocks, combat-pause and dungeon time-lock semantics, context-as-entity-property); [gdd-dungeon-map-ui.md](gdd-dungeon-map-ui.md) (dungeon voxel grid + entity placement); [gdd-settlement-exploration-ui.md](gdd-settlement-exploration-ui.md) (settlement POI navigation, current-POI concept).
...
- **Per-party clocks.** Each party has its own in-game clock (scheduler §1.2). A save snapshots all parties' clocks; restore must reinstate each, not collapse them to one.
- **Dungeon time-lock.** A dungeon visit advances the party's clock independently; on exit the party may be time-locked until the overworld catches up (`check_party_time_lock`). A save made *inside* a dungeon must restore the party's advanced clock **and** the pending time-lock relationship, not silently re-sync.
...
| Per-party clocks / event queue / effects | ✅ | ✅ | unchanged |
...
- **Time-lock on dungeon save/restore.** Restoring a party saved mid-dungeon must reinstate its advanced per-party clock and the pending time-lock vs. the overworld (§3). Verify `check_party_time_lock` behaves correctly when entered via restore rather than via a live dungeon exit.
