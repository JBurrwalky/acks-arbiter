...
```json
{
  ...
  "mechanical": {
    ...
    "lifecycle": {
      ...
      "end_state": "string — enum: 'enduring' | 'enclave' | 'fading' — the arc the sim should steer toward by game-start (advisory, not guaranteed). MECHANICS for the deferred history sim: 'enduring' = no special decline pressure (the default healthy arc, steered only by peak_strength/collapse_proneness). 'enclave' = the demihuman golden-age then HARD-collapse arc (high peak, steep fall to scattered defensive enclaves). 'fading' = a slow DEGRADATION rather than a hard collapse: the sim applies a COMPOUNDING per-generation negative modifier to aggression, defense, and growth (size), so the culture gradually weakens and grows progressively more open to conquest by its neighbors without necessarily collapsing on its own. Humans default 'enduring'; demihumans 'enclave'; 'fading' suits stable-but-degrading powers (e.g. Axsatran the tolerant empire). Consumed by the deferred gdd-history-simulation; recorded here as the per-culture input.",
