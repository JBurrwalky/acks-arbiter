...
- **Seed biomes.** Elves seed in `forest` and `jungle`; dwarves in `hills` and `mountains`. Their homelands originate in wilderness — as do **all** cultures' (the universal wilderness-seeding model, §6.3); this is not demihuman-specific.
- **Alignment.** Both races are open to all three and both use an explicit `weights` map (the even-split default is for humans, §4.2). **Dwarves preference Neutral:** `weights: {Neutral: 0.50, Lawful: 0.30, Chaotic: 0.20}` (Lawful over Chaotic by dwarven tradition). **Elves split equally between Law and Chaos with Neutral a minority:** `weights: {Lawful: 0.40, Chaotic: 0.40, Neutral: 0.20}` — a people pulled to the poles, rarely content in the middle.
