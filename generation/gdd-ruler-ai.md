...
**Consumed by (forward dependency):** the build agent; [gdd-faction-framework.md](gdd-faction-framework.md) §5 (realm diplomacy/treaties/rebellion — fulfills the formerly-planned `gdd-ruler-diplomacy.md`; reference updated 2026-07-06), future `gdd-dynasties.md` (heirs/bloodline/succession-by-inheritance).
...
3. **Diplomacy — deferred.** v1 has **no** diplomacy actions; `RealmGraph.is_allied()` stays `false`; treaties/alliances/non-aggression are now designed in [gdd-faction-framework.md](gdd-faction-framework.md) §5 (the successor to the formerly-planned `gdd-ruler-diplomacy.md`) and unlock at that GDD's phase FF-3. This keeps v1 grounded in ACKS RAW, which is silent on inter-ruler diplomacy (§2.6).
...
| Action | Handler | Weight | base | ACKS | Preconditions |
|---|---|---|---|---|---|
| `raise_garrison` (fund to the 2gp/family RAW minimum; wilderness targets 4gp/family per the `:233` base-morale threshold — PROJECT CALL) | `conscript_troops.gd` → `levy_militia.gd` → `solicit_mercenaries.gd` (hiring needs the resulting offers per `ax_campaign_play.xml:566`); clanholds: `levy_tribal_warriors.gd` | fortification | 0.40 | garrison min `acore_…:216-234` | under funding target; clanhold blocks militia/conscript (`ax_domains_of_chaos.xml:36`) |
| `issue_decree(liturgy/tithe)` | `issue_decree.gd` | religious | 0.20 | liturgy morale `acore_…:486-502` | ruler |
| `repress_population` | `repress_population.gd` | oppression | 0.15 | `acore_…:486-502` (caps morale at 0) | ruler; has a non-militia force to repress with (militia cannot repress, `acore_…:510-516`); **last-resort** |
...
- Trigger: a player action crosses a significance threshold. **Starter values (Jedidiah, 2026-07-06; playtest-tunable):** fire on {a direct attack on the ruler or his stronghold, seizure of one of his vassals, or his domain morale dropping to Turbulent (−2) or worse}, cooldown **one game-month per ruler**. (Thresholds remain PROJECT CALL, §13.)
...
- **§5.2 build corrections — CONFIRMED (Jedidiah, 2026-07-06):** both RAW-driven corrections flagged 2026-07-01 are ratified. (1) `repress_population` precondition — the "has a non-militia repressing force" gate is correct; Jedidiah's precise reading: **mustered militia gp value cannot be counted toward the gp/family thresholds for repression — militia are invisible to the repression calculation.** Verified the implementation already honors this: the repression bonus derives solely from the declared `repression_cp_per_family_this_month` (additional troops the ruler funds specifically to repress), NEVER from existing garrison gp value (`domain_handlers._event_modifiers_sum` adds no repression term), and `RulerActionCatalog._has_non_militia_force` offers repression only when a `source_type != 'militia'` unit exists — so a militia-only domain cannot repress and militia gp value never enters the tier calc. No code change needed. (2) `raise_garrison` composition + wilderness-4gp funding target — confirmed correct.
- **Reassessment significance thresholds (§9.2) — RESOLVED (Jedidiah, 2026-07-06, "good enough for now"; still playtest-tunable):** Seam B fires on {a direct attack on the ruler or his stronghold, seizure of one of his vassals, or his domain morale dropping to Turbulent (−2) or worse}, with a cooldown of **one game-month per ruler**. Only binds once a real LLM provider is wired (no-op under the mock; the deterministic scorer already handles the situation).
- **`diplomatic` crisis_response degradation (§7.1):** in a no-diplomacy v1 this collapses to appeasement/cautious; [gdd-faction-framework.md](gdd-faction-framework.md) §5.6 (`sue_for_peace`, `propose_treaty`) replaces it with real parley at FF-3.
...
- **Forward dependencies:** [gdd-faction-framework.md](gdd-faction-framework.md) §5 (lights up `expansion_weight`/`diplomatic_weight`, the alliance/treaty writer, `is_allied()` — fulfills the formerly-planned `gdd-ruler-diplomacy.md`; updated 2026-07-06); `gdd-dynasties.md` (heirs/bloodline/succession-by-inheritance, replacing the `RulerDeathHandler` placeholder).
...
- **2026-07-01 — Phase-1 build corrections (§5.2, RAW-driven; flagged for Jedidiah review).** (1) `repress_population` precondition corrected: RAW constrains WHICH troops repress ("Militia cannot be used to repress the peasantry", `acore_axioms_strongholds_and_domains.xml:510-516`) — the old "no militia in force" wording wrongly blocked repression whenever militia existed; the gate is now "has a non-militia repressing force". (2) `raise_garrison` composition made RAW-faithful: conscripts before militia (conscripts levy "without affecting domain morale or revenue", `daw_armies_recruitment.xml:315`; militia cost revenue + morale while called up, `:429-431`); the mercenary leg runs through `solicit_mercenaries` (direct `hire_mercenaries` requires a prior successful solicitation, `ax_campaign_play.xml:566`); clanholds route to `levy_tribal_warriors` (`ax_domains_of_chaos.xml:36` blocks militia/conscript). (3) The "2/3/4 minimum" phrasing tightened: RAW's enforced floor is 2gp/family (+2 clanhold); borderlands 3gp is customary; wilderness 4gp is a base-morale threshold the planner treats as its funding target (PROJECT CALL). (4) `withstand_siege` registered in the activity vocabulary (`ruler_ai_category.json`) — it existed in no activity JSON, so the §9.2 validator could never validate it.
- **2026-07-06 — §5.2 corrections CONFIRMED + Seam-B thresholds set (Jedidiah).** Both 2026-07-01 §5.2 corrections ratified (see §13). For `repress_population`, Jedidiah gave the precise reading — **mustered militia gp value is invisible to the repression gp/family calculation** — and a code check confirmed the implementation already honors it (repression bonus derives only from the declared additional-troops expense, never from garrison gp value; `_has_non_militia_force` bars militia-only domains), so no code change was needed. Seam-B strategy-reassessment triggers set to a starter set (attack on ruler/stronghold, vassal seizure, or morale ≤ −2) with a one-game-month-per-ruler cooldown (§9.2/§13; playtest-tunable; inert until a real LLM provider lands).
