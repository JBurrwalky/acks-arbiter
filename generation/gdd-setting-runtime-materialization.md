...
- **M2c / M3 (NEXT, not yet built):** M2c = roads/rivers projection (`hex_overlays` geometry + 6-mile road/river generation) + richer terrain variation (cross-parent ecotones, river oases, polity/culture dithering). M3 = party placement + Strategic⇄Regional toggle = Day-1 MVP (needs the campaign-creation PC-creation flow + UI via the godot-ai MCP).
