...
- **M3-b (deferred — UI polish on working logic):** the Decision-K start-city picker on the review screen (auto-pick of the largest city works without it) + a Strategic⇄Regional world-map toggle BUTTON (the toggle *logic* already exists in `GameState.set_map_view_mode` + `wilderness_explore_state`; only a HUD trigger is missing). Both need live godot-ai-MCP verification; sequenced for the post-playtest correction pass.
