...
- **M2b-1 — Civ domain trees from `setting_domains`** (abstracted; no location yet) ✅ **LANDED 2026-06-18** (commit `f0132f3` + review-fix follow-up): replace one-domain-per-polity for civ polities; build the leaf+interior ladder; fuse war-vassalage; honor pre-rolled rulers; build the eager sovereign ruler set; tribute (incl. war-vassal crown up-tribute from the realm family total — Model E keeps crown families=0). The two history migrations land with their **first writer** (settlement `history_context` in M2b-3; domain `subjugated_since_tick` in M2b-5), not here — landing unused columns early buys nothing and risks a wrong shape. Adversarially reviewed (5 lenses); fixes applied: dangling-liege polity → independent realm+ruler (no NULL realm_id), war-vassal up-tribute, partial-materialization rollback on failure.
- **M2b-2 — Locate in-window domains.** ✅ **LANDED** (`55d1839`): `seat_q/r` → the (1,1) carrier child on the play map (in-memory `ctx.domain_seats`); `realm_kind`→'tracked' for in-window realms; off-window stays abstracted. (`domain_hexes` moved to M2b-3c.)
- **M2b-3 — Content placement onto carrier children.** ✅ **LANDED** (3a `612394e`, 3b `499c9d4`, 3c `2fc6738`): `setting_settlements`→`settlement_entrances` (+ derived `history_context`, mig 171); `setting_ruin_seeds`→`dungeon_entrances` (+ provenance in `dungeon_data`); `setting_poi_seeds`→`pois` (+ context/rumor_seeds); `setting_fortifications`→`strongholds`; `domain_hexes` for in-window territory (land_value from setting_hexes). Distinct carrier offsets per type (settlements 1,1 / forts 2,1 / dungeons 2,2 / POIs 3,3). Population conserved at the DOMAIN level (M2b-1); per-6-mile-hex population storage deferred (no consumer).
- **M2b-4 — No-domain hexes.** ✅ **LANDED** (`a1d3018`): titular wilderness (owned, pop-0) attached to its polity's crown in M2b-3c; orphaned populated land (unowned, pop>0, in-window) → persisted **pocket realms** (cluster contiguous same-dominant-culture via axial 6-neighbours; tier by families clamped Barony..County; rolled fighter ruler; fallen-realm toponym naming). Conservation extended to include in-window orphan families.
- **M2b-5 — History access.** ✅ **LANDED** (`b81bace`): `domains.subjugated_since_tick` (mig 172, set on war-vassal crowns from the latest subjugation event); `SettingHistoryReader` (zero-copy accessor over the frozen `setting_events` chronicle). Per-entity digests (settlement history_context, dungeon provenance, subjugation tick) + the full log are the narrator/NPC access surface. **M2b adversarially verified** (Workflow passes on M2b-1 and M2b-3/4/5). Suite 462/18 net-zero throughout; `SettingMaterializationTests` 67→115 checks.
- **M2c — Roads + rivers + ecotones.** ✅ **LANDED** (`ea7bfed` roads/rivers, `d14502a` ecotones): 24-mile `setting_river_edges`→6-mile boundary `hex_river_edges` (via real-adjacency `HexRiverEdgeData.canonicalize_edge`); 24-mile `setting_roads`→6-mile continuous path (carrier children + cube-lerp hex line) as a runtime `roads` entity + `hex_overlays(road)` geometry; cross-parent ecotones (RegionZoomIn feathers edge children toward different-biome neighbour parents). **Deferred enrichments (documented):** beastman per-race intermingling (§5.3), minor 6-mile settlement scatter (§5.2), river oases (§4.2), polity/culture-weight dithering (§4.4 — substrate is read-back per §5.10 for MVP).
- **M3-a — Playable integration.** ✅ **LANDED** (`6a633ba`): `CampaignCreationState` calls `materialize()` on world approval; `SessionLoadState` lands a new generated-campaign party on the 6-mile regional PLAY map at the start city (not the view-only 24-mile world map); new `SettingMaterializer.start_position()`. Settlement/dungeon entry + map loading already read materialized rows generically — no change needed. A generated campaign is now playable end-to-end (create → materialize → party → 6-mile play map). Headless-verified (`start_position` test, 125 checks); end-to-end flow to be MCP-smoke-tested + user-playtested.
- **M3-a fix — generated world → party creation.** ✅ **LANDED** (`c1d3bc6`): MCP playtest revealed the original M3-a routed "Begin Campaign" back to the menu (no PCs → empty party). `CampaignCreationState.handle_action("world_ready")` now routes to `"party_creation"` (the `{campaign_id}` payload flows to `PartyCreationState.enter`, same as the fixture "New Campaign" path). Verified end-to-end in the running game: Generate World → Begin Campaign → party creation → pick party → 6-mile play map at the start city with a generated settlement to enter.
- **M3 — playtest correction pass 1.** ✅ **LANDED 2026-06-19** (`bb86903`, `2c9149d`, `8ac976a`, `0237e36`) — three issues from the first full M3 playtest, all MCP-verified in the running game:
  - **Play-window ratio** (`bb86903` + `0237e36`): the 6-mile window was portrait. Swapped `RegionZoomIn.WINDOW_W/H_PARENTS` to 10×8 (was 8×10 → now wider-than-tall), AND fixed the near-edge clip: `build_start_region` TRUNCATED the window at the world border (kept only in-bounds parents), so a start city near a horizontal edge still yielded a portrait map (observed 28×32). Now the window SHIFTS inside the world's offset bounding box (`clampi` on `min_col/min_row`) so the full 10×8 footprint is used whenever the world is ≥ that (a Small world is 15×12 parents, so always). Empirically confirmed: a fresh Small world now materializes a **40×32 (5:4 landscape)** region map regardless of city position.
  - **Towns not enterable** (`2c9149d` + `8ac976a`): materialized settlements ship empty `settlement_data` and NO `settlement_pois`, so the enter path bailed (the play-map renderer's `_on_enter_settlement_pressed` JSON-parsed `""`→null→returned, never emitting `settlement_entry_requested`; `SettlementExploreState.enter` push_errored→bounced to wilderness). Wired the existing **lazy interior stocker** (Decision H) at BOTH gates: when a settlement has no POIs and empty data, `SettlementLayoutGenerator.seed_pois()` stocks a baseline interior (tavern + workshops + gates, scaled by market class) on first entry; runs once (the state gate then sees the rows exist). The renderer is a SCENE script — never loaded by the headless suite — so this surfaced ONLY under live MCP play, not the 462/18 run. Verified end-to-end: entered a generated Class III town through its North Gate into the district/gates interior.
  - **Coastline jitter** (`bb86903`): the 24→6-mile zoom inherited water wholesale, leaving dead-straight parent-block coastlines. Added a coherent coastal-fringe pass in `_children_for_parent` driven by the existing smoothed noise (`COAST_STRENGTH`): a water child bordering a land neighbour parent can jut to a peninsula (top noise band), a land child bordering a water neighbour can flood to a cove (bottom band); new `_coastal_neighbor` helper. Capes/inlets cluster into shapes, not speckle. Suite-green (region "same>differ" inheritance-dominance still holds).
- **M3 — playtest correction pass 2: load-path map selection.** ✅ **LANDED 2026-06-19** (`d157a6f`) — playtest follow-up: after loading a saved generated campaign the party was off its start town with no enterable town. **Root cause (NOT a renderer/persistence bug — the town row + lazily-stocked `settlement_pois` round-trip faithfully via the VACUUM-INTO save snapshot, both being explicit save-scope entries):** `SessionLoadState` rendered `hex_maps[0]`, and `list_hex_maps_for_campaign` sorts `parent_map_id IS NULL` first, so `hex_maps[0]` is the 24-mile **view-only WORLD map** (zero settlement entrances), while the `regional_6mi` PLAY map sorts after. The M3-a re-point to the play map (`start_position`) is gated on `current_map_id` being empty — only true for a NEW party — so a RELOAD skipped it and loaded the world map. **Fix:** for a LOADED party in a `campaign_origin='generated'` campaign, set `primary_map_id = pd_init.current_map_id` (its play map), validated against the campaign's `hex_maps` (stale id → fall back to `hex_maps[0]`); the existing restore branch then seeds `map_data.party_hex` from `current_hex_q/r`. New-party + fixture paths untouched. Diagnosis adversarially corroborated by a 4-trace workflow (load/renderer/persistence/placement) + suite 462/18 net-zero; **MCP-verified**: a loaded generated save now lands on the 6-mile play map at the saved hex with its dungeon/settlement entrances live. **Separate edge-case follow-up (NOT this fix):** a save made INSIDE a dungeon whose `dungeon_id` no longer resolves takes `_restore_into_dungeon`→false→`wilderness` and leaves the campaign-select screen un-dismissed (occludes the play map); a normal `wilderness` save dismisses cleanly. Rare (needs a stale dungeon ref); flagged for a later pass.
- **M3-b (deferred — UI polish):** the Decision-K start-city picker on the review screen (auto-pick of the largest city works without it); the placeholder world name (`create_campaign("Generated World", "w")` in `campaign_creation_flow.gd:151` — the "Welcome to w" title + "Generated World — w" campaign label both show the stub; should derive a real name). Need live godot-ai-MCP verification; sequenced for the next correction pass.
- **M3-c — Notebook "World Map" tab (the 24-mile strategic view). NOT YET BUILT.** Supersedes the earlier "world-map toggle BUTTON" idea (`GameState.set_map_view_mode` stays for a future travel-scale switch). A new Notebook tab showing the strategic political map, built by adapting the campaign-creation Review screen.
  - **New file** `scenes/ui/notebook/tab_pages/world_map_tab_page.gd` — extends `notebook_tab_page.gd`, `TAB_ID := "world_map"`, builds its content in `_build_content()` (the notebook idiom). **Register** it: add `"world_map": preload(".../world_map_tab_page.gd")` to `notebook.gd`'s `TAB_PAGE_SCRIPTS` dict + a labelled/iconed entry in `notebook_tab_strip.gd` (match the existing tab-strip pattern).
  - **Reuse from `screen_review.gd`:** the `scenes/ui/campaign_creation/political_map_view.gd` renderer (`bind_map(ordered_hexes, palette, settlements, polities, rivers)`, `set_mode(layer)`, `set_sovereign_view(true)`) + the **Layer** OptionButton (Political/Biome/Elevation/Territory/Culture) + the informational right-panel tabs **Brief / Realms / Peoples / Issues** (from `CampaignReviewAssembler.assemble`).
  - **STRIP:** the history replay (that lives in Screen C / the "Watch the history again" button), the **Regenerate world** button, the **Begin Campaign** button (`approved`), the right-panel **Vassalage** + **History** tabs (and their `set_domains` / `set_event_log` feeders), and the **Sovereign** CheckButton.
  - **Lock to Sovereign mode ON:** call `_map.set_sovereign_view(true)` at build; no toggle.
  - **Data source:** the frozen `setting_*` tables at runtime (`SettingRepository.list_hexes / list_polities / list_settlements / list_river_edges` — bound on tab open via `GameState.campaign_id`). They persist in `CampaignRepository._SCOPE_DIRECT_CAMPAIGN`, so this is the world AS GENERATED (a live realm sim would refresh it at M5). **Only meaningful for `campaign_origin='generated'`** — for fixture campaigns there is no `setting_hexes`, so hide/disable the tab (or show an empty state).
  - **Verify:** godot-ai MCP — open the Notebook → World Map tab → screenshot the political map + Layer switching, Sovereign view locked on.

### 15.5 M4 — AX3-density 6-mile fill (the per-window content-density arc)

**Motivation (Jedidiah 2026-06-19):** the published ACKS principality *AX3 "Capital of the Borderlands"* + its domain-economics spreadsheet + 53-location POI map set the **per-6-mile-window detail bar**: a complete feudal vassal tree with **every hex accounted for down to Baron**, dense settlements/dungeons/POIs, and a stronghold+garrison+ruler in essentially every settled hex. Our handoff today is *structurally complete but one full 16:1 scale-step too coarse* — the `setting_domains` tree bottoms at **one leaf per 24-mile hex** (so a leaf covers 16 six-mile hexes with no per-hex domain/ruler/stronghold/settlement), settlements are Class III+-only (~11/map), and dungeon/POI budgets are 24-mile-scale. M4 closes that gap.

**The algorithm lives in `gdd-region-zoom-in.md §5.6` (the decomposition spine) + §2 (RAW urbanization table, title mapping, family-sizing) — read it for the "how."** This section owns the **phase sequencing, migrations, and acceptance bar.** All work is at the **runtime materialization layer** (`RegionZoomIn`/`SettingMaterializer`), driven by the 24-mile handoff as the structural budget; deterministic; eager in the play window, lazy at the frontier; off-camera stays coarse.

**Rulings folded in (2026-06-19):** (1) synthesize the detail at 6-mile materialization, NOT by expanding `setting_*`; (2) denormalized `garrison_composition` now (display-ready), real troop economy is a later Domain phase; (3) place full site density now as data + LLM stubs, deep POI mechanics later; (4) beastman/clanholds get a shallow chieftain→sub-clanhold tree + per-race intermingling. **Corrections folded in:** Tribune=Marquis/March, Castellan=Baron/Barony (**floor — no sub-Baron**); urban settlements **cluster at the domain stronghold**; County border forts (Téros) = Class V civ / **Class VI BL-WIL** (market class derives from actual urban families, so this is automatic — the Túros Tem case needs no special-case); Marquis/Baron **watchtowers** (no settlement) surface as **Stronghold POIs** (UI hooks) — the watchtower line is the leaf strongholds, not a separate scatter; **domain size by family budget, not hex count.**

**Sub-phases (commit + suite-green each; sequence approved). ✅ ALL LANDED 2026-06-19/20 — `SettingMaterializationTests` 126→155 checks, suite 462/18 net-zero throughout. Headless/structurally verified; VISUAL density + interaction is the next playtest pass (the densities are MCP-eyeballed dials).**
- **M4-1 — Decomposition spine + per-hex population.** ✅ **LANDED** (M4-1a `038f035` migration 173 `domain_hexes.families` + conserved per-hex distribution; M4-1b `6d5c589` `_decompose_in_window_domains`/`_split_6mi`). Each in-window CIV leaf → a County→March→Barony runtime `domains` ladder by **family budget** (mirror of the 24-mile `_split_realm`), every populated 6-mile hex its own Barony, `liege_domain_id` chained; the gov keeps its seat (Model E), leaves carry families. One **stronghold per sub-fief** + a **LAZY/names-only** ruler (owner NULL — perf split; tagged `establishment_method='materialized_subfief'`). Eager rulers stay at Count+. (region-zoom-in §5.6a)
- **M4-2 — Urbanization (settlements at seats).** ✅ **LANDED** (`31f92e7`). `_materialize_county_settlements`: each in-window CIV realm-head's largest settlement, sized by its REALM families (the 24-mile hex pop) via `_market_class_for_families` (the §2 table), placed at its seat IF **Class V+**; Class VI = stronghold-market (no row). Sub-fiefs get none. Urban families on the settlement (Q-MERC-15), recovering folded urban pop without double-count. (region-zoom-in §5.6b)
- **M4-3 — Garrison composition (denormalized).** ✅ **LANDED** (`dbc2e29`, migration 174 `domains.garrison_composition`). `_garrison_composition` per the JJ Step-10 model (gp = families × {2/3/4 by territory} → troops by unit cost, ~23% vet) for every populated CIV domain; also sets the legacy `garrison_troops`. (region-zoom-in §5.6d) Real troop economy flagged for a later Domain phase.
- **M4-4 — Stronghold POIs.** ✅ **LANDED** (`7c0ef66`). `_materialize_stronghold_pois`: one `stronghold` POI per distinct sub-fief watchtower hex (the UI hooks). **DEFERRED to the playtest calibration pass:** the broader 6-mile dungeon/POI DENSITY re-budget (the §9.3/§9.7 budgets are 24-mile), the `faction_stronghold` archetype, and the landmark-vs-POI icon dedup — all MCP-eyeballed density dials / visual decisions. (region-zoom-in §5.6c/e)
- **M4-5 — Shallow clanhold tree.** ✅ **LANDED** (`cd1d7c3`). `_decompose_clanholds`: each in-window clanhold → a chieftain→sub-clanhold tree (per `ax_domains_of_chaos.xml:43`, ~68–192 fam/clanhold), each sub-clanhold a `clanhold` domain + clanhold stronghold + `clanhold` POI + LAZY sub-chieftain; tribal warriors split with the territory. **DEFERRED follow-on:** the full per-race **intermingling scatter** (new clanholds across empty wilderness from `beastman_distribution.json` — §5.3 "Part B") + per-race clanhold sizing.

**Acceptance bar = the "interactivity floor"** (region-zoom-in §5.6): no large empty stretch in settled/frontier bands. **Golden test (validated against the ACKS II Judges Journal "Realm Apportionment", received 2026-06-19):** the M4-1/M4-2 decomposition+urbanization, run on the AX3 Borderlands realm parameters (185 fam/6-mi-hex; a Count realm ≈3,950 fam = 725 personal + 110 urban + 3,115 vassal → 21-hex County), reproduces its published structure: **1 Prince → 4 Dukes → 20 Counts → ~80 Marquis → ~320–480 Barons (≈425–625 domains)**. The JJ *abstracts* the Marquis/Barons ("too messy"); **we materialize them down to Baron for UI hooks.** Téros County forts land Class V civ / VI BL-WIL automatically; the watchtower line = the Baron/Marquis leaf strongholds. JJ guardrails: map its titles to our ACKS tiers, ignore its color-reuse, **collapse outlands/unsettled/wildlands → our `Wilderness`** (no new classes). **Perf split (region-zoom-in §5.6a):** ~750–1,100 domains/eager-window → domain rows + Stronghold-POI rows eager (clickable hooks); **Marquis/Baron ruler NPCs lazy/names-only**, promoted on visit. **Garrison (M4-3)** uses the JJ Step-10 model: 2/3/4 gp per family by class → troops by unit cost (~25% vet), stacking up the liege chain.

**Likely migrations (sequential from head 172):** `domain_hexes.families`; `domains.garrison_composition`; possibly a `synthesized`/`origin` flag on runtime domains to distinguish sim-pre-rolled leaves from handoff-invented 6-mile sub-fiefs (for frontier-growth + replay provenance). **No `setting_*` schema change** (the density is runtime-materialized) — so no `_SCOPE_DIRECT_CAMPAIGN`/`_DATA_TABLES` additions; the affected runtime tables (`domains`/`settlement_entrances`/`strongholds`/`pois`/`domain_hexes`) are already in the save scope.

**Deferred (explicitly out of M4):** the urban/agrarian/centralized/dispersed urbanization variation + per-culture adjustments (assume baseline table); the real garrison troop economy (`troop_units`, recruitment, upkeep — a Domain phase); POI contents-on-discovery mechanics (the geas/healing-shrine depth — a separate system); the culture-kit ruler archetype sub-roll (§7.4, post-MVP).

---

## 14. Revision History

- **2026-06-19 (v0.5):** **M4 — AX3-density 6-mile fill (§15.5).** Jedidiah supplied the published ACKS principality *AX3 "Capital of the Borderlands"* (+ domain spreadsheet + 53-POI map) as the per-6-mile-window detail bar, ruling the handoff must reach a full vassal tree down to Baron with every hex accounted for + dense settlements/dungeons/POIs/strongholds. Captured the phase plan (§15.5) cross-referencing the controlling algorithm in `gdd-region-zoom-in.md §5.6`/§2. Key rulings: synthesize at 6-mile materialization (not by expanding `setting_*`); denormalized garrison now; rich POIs as stubs now; beastman shallow clanhold tree. Key corrections: Tribune=Marquis, Castellan=Baron (floor, no sub-Baron); settlements cluster at strongholds; border-fort County seats Class V/VI; watchtowers = leaf strongholds surfaced as Stronghold POIs; domains sized by families. Also logged the M3 playtest correction passes 1 & 2 (window ratio, enterable towns, coastline, load-path map) in §15.4.
