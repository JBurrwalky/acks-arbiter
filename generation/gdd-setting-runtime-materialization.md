...
- **M3-b (deferred — UI polish):** the Decision-K start-city picker on the review screen (auto-pick of the largest city works without it); the placeholder world name (`create_campaign("Generated World", "w")` in `campaign_creation_flow.gd:151` — should derive a real name). Need live godot-ai-MCP verification; sequenced for the post-playtest correction pass.
