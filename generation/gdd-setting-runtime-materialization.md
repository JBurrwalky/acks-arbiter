...
**Sub-phases (commit + suite-green + MCP-verify each; sequence approved):**
- **M4-1 — Decomposition spine + per-hex population.** Migration: `domain_hexes.families` (per-6-mile-hex, conserved: 16 children sum to parent `population_band`). Extend the partition one scale-step deeper at materialization (reuse the 24-mile `_split_realm` logic): each in-window 24-mile leaf → County→March→Barony runtime `domains` by **family budget**, every populated 6-mile hex in exactly one Barony, `liege_domain_id` chained to the sovereign. One **stronghold per domain** at its seat (`DomainTierTable.stronghold_value_gp × 100`) + **one ruler character** (honor pre-rolled; deterministic bare roll for invented leaves). *Named-ruler density (~50–150/window) falls out automatically.* (region-zoom-in §5.6a)
- **M4-2 — Urbanization (settlements at strongholds).** Per the §2 Villages/Towns/Cities table, recurse the realm: each domain's largest settlement at/near its stronghold on water/road; surface **Class V+** as `settlement_entrances`, **Class VI** as Stronghold POIs. Replaces the flat "~15 settlement-points" scatter with table-derived urbanization. Conservation-consistent (recovers the folded Class IV–VI urban families). (region-zoom-in §5.6b)
- **M4-3 — Garrison composition (denormalized).** Migration: `domains.garrison_composition` (TEXT JSON), deterministic from tier per the AX3 garrison tables; populate for all materialized domains. Strongholds already carry `cp_value` + `garrison_capacity`. (region-zoom-in §5.6d) Flag the real troop economy for a later Domain phase.
- **M4-4 — Adventure-site density re-budget + Stronghold POIs.** Re-budget dungeon/POI density from 24-mile to 6-mile; add the `faction_stronghold` POI archetype; place as data + LLM stubs; surface the no-settlement Marquis/Baron strongholds as Stronghold POIs (the UI-hook watchtowers). (region-zoom-in §5.6c/e)
- **M4-5 — Beastman shallow clanhold tree + per-race intermingling.** Give clanhold realms a chieftain→sub-clanhold structure + the per-race 6-mile intermingling scatter (region-zoom-in §5.3), so chaotic frontier hexes reach comparable interactivity.
