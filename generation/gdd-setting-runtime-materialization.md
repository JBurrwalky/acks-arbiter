...
**Acceptance bar = the "interactivity floor"** (region-zoom-in §5.6): no large empty stretch in settled/frontier bands. **Golden test:** the M4-1/M4-2 decomposition+urbanization, run on the AX3 Borderlands realm parameters, reproduces its published structure (≈185 domains; 1 Prince→4 Duke→20 Count→80 Marquis→80 Baron; Téros forts Class V/VI; watchtower line) — validated against the **ACKS II Judges Journal** partition of AX3 (Jedidiah to supply; treat as illustrative of the *spatial* partition only — map its titles to our ACKS tiers, ignore its color-reuse, and keep our 3-class Civilized/Borderlands/Wilderness model — **no Outlands/Unsettled**).
