...
>
> **REVISED 2026-06-20 (Jedidiah) — rulers keep a personal demesne; its stronghold is back-derived.** Higher tiers now hold a multi-hex personal demesne instead of a single seat (gov) or nothing (interior, the old Model E). The demesne family budget is the RAW `revenue_by_realm_type` personal-domain column (Barony 160 / March 320 / County 780 / Duchy 1,500 / Principality 7,500) — used as the **upper-bound peel-stop** ("filled out") ± 15% deterministic jitter, NOT a hard cap. The materializer peels the seat + densest neighbour hexes until the budget is met, assigns them to the ruler, and the stronghold value is then the **reverse of the securing formula** (demesne hexes × territory rate) — exactly the "fill the domain, then derive its minimum stronghold" knock-on. RAW grounding: `acore_aging_poisons_high-level-start_optional_rules.xml:862-873` ("the personal domain begins with the maximum number of families allowed by the stronghold's size"). **Emergent caveat — demesne HEX count is density-driven, not tier-driven.** Because demesnes are sized by *families* (per "domains sized by families, hexes are estimation") and the RAW budgets (160–780) are small relative to a dense 6-mile hex's population, in the **dense civilized core a County/March/Baron demesne all collapse to ~1–2 hexes**, while in **sparse borderlands/wilderness a single ruler's demesne can span many hexes**. So a sparse Baron can occupy more *hexes* than a dense Count even though the Count holds far more *families/realm*. This is RAW-faithful but means "higher tier ⇒ visibly larger demesne on the map" holds only in **families**, not hexes. If a guaranteed visual per-tier hex progression is wanted instead, switch the demesne basis from a family budget to a **per-tier hex band** (e.g. Barony 1 / March 2 / County 4 / Duchy 8 / Principality 16 ± jitter) — the peel/assign/stronghold scaffolding is unchanged; only `_demesne_budget`/`_peel_demesne` swap their unit. **OPEN for Jedidiah.**
