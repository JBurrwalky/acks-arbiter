...
Distribute each parent's `population_band` across its **LAND** children only weighted by land value / settlement presence, **with no hard cap** (the clanhold 125 soft limit is a runtime economy penalty, §2). Write per-child `domain_hexes` land values; aggregate to the domain's `peasant_families`. **The per-6-mile-hex family count produced here is the input to the §5.6 decomposition** — store it (new `domain_hexes.families`, conserved: a parent's land children sum to its `population_band`).

> **Land-only (Jedidiah 2026-06-21).** Ocean/lake children get **no population and no `domain_hexes` row at all** — you don't own the open sea, and a populated water child would otherwise materialize a sub-fief + watchtower on the ocean (the reported bug). Both the owned-hex distribution (`_materialize_domain_hexes`) and the orphan→pocket-realm claim (`_create_pocket_realm`) consult a shared region water set (`_region_water_set`, from `hex_cells.water`); future passes that distribute population or claim land across a parent's children must do the same. Conservation is preserved (land children sum to `population_band`); an all-water owned hex carries nothing. Regression-guarded: no `domain_hex` and no Stronghold POI ever sits on a water hex.
...
**(a) Decompose every in-window civ/BL realm to Barony, by family budget.** For each in-window 24-mile leaf domain (today `setting_domains` bottoms at one leaf per *24-mile* hex — 16× too coarse), split its 16 children's families into a continuous **County → March → Barony** runtime `domains` ladder so **every populated 6-mile hex sits in exactly one domain** — a leaf Barony, or a higher ruler's **personal demesne** (below). Partition by **families, not hexes** (§2): peel children whose own families stand as a `(tier−1)` realm into leaves at that rank; cluster the remainder into synthesized intermediate nodes (mirrors the existing 24-mile `_split_realm`, one scale-step deeper). Wire `liege_domain_id` up to the 24-mile leaf and on to the sovereign. Fan-out targets **~`_FANOUT` (3) vassal nodes per parent** (≈3 Barons per March, ≈3 Marches per County) — the clustering divisor is the tier's full realm hexes (`_ratio_group_hexes`: March 4 / County 16), not the family target (which collapsed to ~1 Baron per March in the dense core; see callout). Each ruler (the gov + every interior node) **keeps a personal demesne** = the seat + densest neighbour hexes taken until **both** its tier's RAW personal-domain family budget (`revenue_by_realm_type`: Barony 160 / March 320 / County 780 / Duchy 1,500 / Principality 7,500) **and** a per-tier **hex floor** (Barony 1 / March 1 / County 4 / Duchy 8 / Principality 16) are met, both ± 15% deterministic jitter, **hard-capped at half the realm's hexes** so the vassal tree always gets the rest; the remainder becomes its vassals. (A March's personal demesne is normally a single hex — the 1-hex March floor frees a hex per March for a Baron vassal, which is what lands the ~3:1 ratio; County+ keep multi-hex floors for visible realm-head demesnes.) Each domain gets a **stronghold at its seat hex**, valued by the **reverse of the securing formula**: `stronghold gp = (demesne hexes) × the per-6-mile-hex securing rate by territory` (civ 16,000 / BL 24,000 / wild 32,000 — exactly 16× the RAW 1.5-mile manor price), floored at one hex. Helpers live on `DomainTierTable` (`min_stronghold_gp` / `stronghold_gp_for_hexes` / `max_hexes_for_stronghold`), shared with the player securing system.

> **RESOLVED 2026-06-20 (Jedidiah) — minimum domain size = one 6-mile hex.** The RAW smallest domain is a 1.5-mile hex (a 6-mile hex = 16 of them); we do **NOT** build that local-map layer (it's the 6-mile zoom-in again, and little play happens there). Instead **one Barony = one full 6-mile hex**, sized by that hex's families — so a dense civilized hex yields a single **"fat" Barony** (up to ~780 families) rather than ~6 small Castellans, and sparse wilderness/borderlands hexes (~125–250 families) land at true Castellan size. This conserves area + families and matches the AX3 *average* Castellan density (~1 per habitable hex); the AX3 "~6 watchtowers per road hex" dense-corridor cluster collapses into the one fat Barony (those tiny holdings lack a market and aren't worth accessing). Knock-on: the **minimum stronghold cost rises** from the 1.5-mile RAW figure (1,000/1,500/2,000 civ/BL/wild) to the 6-mile-hex figure (16,000/24,000/32,000 = 16× the manor price) — the per-hex securing rate below. Also simplifies player land grants / conquest / securing (1 hex = 1 grant). **Per-Barony tribute up the chain is DEFERRED** (open question: build explicit tribute flows for NPC realms, or keep the abstract "realm families × 18^0.6 gp" NPC model? — revisit; player realms may differ).
>
> **REVISED 2026-06-20 (Jedidiah) — stronghold value is a FORMULA, not per-tier.** Replaced the per-tier `DomainTierTable` stronghold figures (for sizing actual domain strongholds) with: **max 6-mile-hex territory of a domain = `floor(stronghold gp ÷ (16,000 × territory modifier))`**, modifier civ 1.0 / borderlands 1.5 / wilderness 2.0 (so the per-hex securing cost is 16,000 / 24,000 / 32,000 gp). The base is **exactly 16× the RAW 1.5-mile manor price** (1,000 × 16 = 16,000; a 6-mile hex = 16 of the 1.5-mile hexes), which is why the three rates scale cleanly — the RAW flat per-tier Barony figure of 22,500 reflects a 15,000-base typo, so the formula uses the corrected 16,000 base. **Remainders round down** (a partial securing value claims no hex — no toe-holds), and the 1-hex cost is kept as a **hard floor** (spend less and you secure nothing — no orphaned strongholds, no frustration). Every domain owner must hold a stronghold. Implemented on `DomainTierTable` (`min_stronghold_gp`, `stronghold_gp_for_hexes`, `max_hexes_for_stronghold`) so the materializer and the future player-securing/conquest system share one rule; `stronghold_value_for_tier` is retained only as the abstract 24-mile revenue/tribute reference.
>
> **REVISED 2026-06-20 (Jedidiah) — rulers keep a personal demesne; its stronghold is back-derived.** Higher tiers now hold a multi-hex personal demesne instead of a single seat (gov) or nothing (interior, the old Model E). The demesne family budget is the RAW `revenue_by_realm_type` personal-domain column (Barony 160 / March 320 / County 780 / Duchy 1,500 / Principality 7,500) — used as the **upper-bound peel-stop** ("filled out") ± 15% deterministic jitter, NOT a hard cap. The materializer peels the seat + densest neighbour hexes until the budget is met, assigns them to the ruler, and the stronghold value is then the **reverse of the securing formula** (demesne hexes × territory rate) — exactly the "fill the domain, then derive its minimum stronghold" knock-on. RAW grounding: `acore_aging_poisons_high-level-start_optional_rules.xml:862-873` ("the personal domain begins with the maximum number of families allowed by the stronghold's size"). **RESOLVED 2026-06-20 (Jedidiah) — HYBRID: family budget + per-tier hex floor, half-capped.** The pure family-budget approach (above) made the demesne HEX count density-driven, not tier-driven: because the RAW budgets (160–780) are small relative to a dense 6-mile hex's population, in the dense civilized core every tier's demesne collapsed to ~1 hex (measured: Marquis demesne = 1 hex), so higher tiers were *not* visibly larger on the map. Fix: peel until **both** the family budget **and** a per-tier **hex floor** (Barony 1 / March 2 / County 4 / Duchy 8 / Principality 16, ± jitter) are met. **Critical guard — the demesne is hard-capped at half the realm's hexes**, because applying the raw floor to interior nodes ate the entire Barony layer (with fat Baronies, a dense March realm is only ~2 hexes, so a 2-hex March floor consumed both — Barons crashed 37→1 in testing). The half-cap means the floor binds where there's room (a County gov's 16-child realm → a 3–5-hex demesne, up from 1.3) but yields in tight quarters so the vassal tree survives (Barons restored to ~30, ≈ Marquis). Net: **realm heads (County+) get visibly larger demesnes; interior Marches in the dense core stay ~1 hex to protect the tree; sparse-frontier demesnes still sprawl by family budget.** The stronghold value follows the demesne hex count (reverse formula) as before — so a County's keep is now naturally bigger than a Baron's. The fat-Barony minimum-domain rule fundamentally limits how much both demesne and tree can expand in the dense core (concentrated land = few hexes); the half-cap is the principled tie-break (a ruler's personal domain is a minority of his realm — RAW County 780 / realm 4,600+ ≈ 17%).
>
> **REVISED 2026-06-20 (Jedidiah) — fan-out fix: ~3 Barons per March (was ~1).** With demesnes in, the clustering's *family target* (one March-realm ≈ 960 families) made the Barony:March ratio collapse to ~1:1 in the dense core: a 960-family March realm is only ~2 hexes there, so each March got 1 demesne + 1 Baron (measured: 6 Marches × 1 Baron per County, ratio 1.0). Fix (Option A): drive the clustering by a **fan-out target** instead — each interior node's realm targets `_FANOUT` (3) vassal nodes, so the clustering divisor is the tier's full realm *hexes* (`_ratio_group_hexes`: a Barony = 1, March = demesne 1 + 3×1 = 4, County = demesne 4 + 3×4 = 16). `n_marches = round(small_hexes / 4)`, floored at `_MIN_INTERIOR_VASSALS` (2) where the realm has the hexes. Paired with dropping the **March hex floor to 1** (so a March's 1-hex demesne leaves the other 3 hexes as Baron vassals). Result: each in-window County now decomposes to **~3 Marches × ~3 Barons (ratio 2.7–3.0)** — Marquis count roughly halved, Barons up, overall ratio 1.0 → ~2.9. The stronghold reverse-formula and the half-cap are unchanged; this only changes *how many* Marches a County is split into. Tunable via `_FANOUT`. Note the hard constraint: at the dense fat-domain 6-mile scale you cannot have both RAW Marches-per-County (4–6) and RAW Barons-per-March (3–4) — not enough hexes — so this favours ~3 Marches/County × ~3 Barons each (the user's call) over the degenerate 6×1.
