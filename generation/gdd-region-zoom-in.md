...
**(d) Garrison composition (denormalized, display-ready).** Materialize a per-domain **`domains.garrison_composition`** (new column, TEXT JSON: troop counts by unit type, derived deterministically from tier per the AX3 garrison tables — e.g. a County fort ≈ 90 troops across ~6 unit types) so the map/LLM/UI can render AX3-level fort detail **now**, without the live troop economy. **The real economic troop entity** (recruitment, monthly upkeep, `troop_units`) is a later Domain-phase system, explicitly out of handoff scope — flag, don't build here. `strongholds` already carries `cp_value` + `garrison_capacity`.
