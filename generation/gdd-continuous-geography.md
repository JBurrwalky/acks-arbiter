...
**Temperature-dependent aridity (LANDED 2026-06-25).** The Köppen `B` (desert/steppe) cutoff now rises with mean temperature (real Köppen: the dry threshold ∝ `2·T`), replacing the flat `ARID_THRESHOLD`: `arid_t(temp) = clamp(ARID_BASE + ARID_PER_DEG·(temp − ARID_REF_TEMP_C), ARID_MIN, ARID_MAX)` in `ClimateGenerator._arid_threshold`, with a `STEPPE_BAND` above it. So hot regions need far more rain to escape desert — the great deserts sit in the hot subtropics/tropics and cold zones stay non-arid. Measured per-band desert: tropical 18.9% / subtropical 14.9% / temperate 9.0% / continental 5.0% (was a flat ~13–15% everywhere); polar "desert" is the EF ice-cap (separate, kept). The flat-threshold "driest steppe → desert" fringe in `_assign_biome` is retired (the temp-dependent BW cutoff now draws the desert/steppe line). Suite 473/16 net-zero (the biome shift moved seed-424242's politics enough to retune one over-coupled materialization assertion → a Model-E up-tribute invariant; a dedicated up-tribute unit test is a follow-up).
...
- **Elevation band:** each sample is classified by a **three-signal terrain rule** (2026-06-26), not height alone — `mountains` = absolute height ≥ a gate (raw 0.72 ≈ 1,300 m) **AND** (steep `slope` **OR** high `prominence`); `hills` = `slope` ≥ a rolling threshold; else `flat`. `slope` is immediate relief (max-neighbour Δheight); `prominence` is rise above the local valley floor over ~18 mi (mountains-only signal — it floods hills otherwise). The footprint then aggregates the per-sample tags with a **"mountains if ≥25% of the footprint is mountain"** override (a hex with a peak reads as mountains even if most of it is valley). Also store the continuous **area-mean height** as `elevation_raw` — an honest footprint mean — which region painting reads to tell a high flat *plateau* from low *plains* (both `flat`; the plateau/basin distinction is non-mechanical). See `HeightmapGenerator.elevation_tag_for`.
...
> **CUTOVER LANDED 2026-06-25.** Continuous-geography is now the DEFAULT world-gen: `GeoFieldToGrid.is_enabled()` defaults true (+ `project.godot`), so 24-mile generation, 6-mile materialization (field-sampled terrain + corner-graph rivers), and the play surface all run field-first. The flag remains only as a legacy escape hatch (set false → the retiring hex-native path). Determinism holds (the §9.1 hash is self-consistency, not a golden value — no re-baseline needed; RNG golden values are terrain-independent). The 24-mile river projection (`SettingMaterializer._materialize_rivers`) is skipped in continuous mode (build_start_region's field rivers are authoritative — no double-write). Full suite 473/16 under continuous = the flag-off baseline, net-zero new failures. Blast radius was 4 assertions (2 test-maintenance + the river double-write + the retired "inheritance dominates" sub-hex assertion).

> **LEGACY PATH RETIRED 2026-06-25** (two phases, ~750 lines of dead code removed). Phase A made continuous unconditional + removed the flag (`is_enabled`/`SETTING`/`project.godot` line gone; `setting_generator._run_geography/_run_climate`, `region_zoom_in.build_start_region`, `setting_materializer._materialize_rivers` rewired to always-continuous; the 24-mile river projection deleted; stage1/3/4a test fixtures + the flag tests rewired). Phase B deleted the now-dead hex-native code: `HeightmapGenerator` gutted to the shared `_elevation_tag` + thresholds util (386→22 lines); `ClimateGenerator` gutted to the shared Köppen classifier + biome/swamp/land-value + temperature/threshold consts (342→204); `region_zoom_in` legacy `_children_for_parent` + variation helpers removed (458→240); `setting_materializer._child_set` removed. Both gutted classes KEEP their names (now classifier utils, not generators — a rename is future cleanup). Suite 473/16 net-zero throughout. STILL NOT DONE: `setting_field_raster` persistence (the field is regenerated deterministically at materialization, not stored) + the `HeightmapGenerator`/`ClimateGenerator`→util rename.
