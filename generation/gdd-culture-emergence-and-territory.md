...
A hybrid is not in the seed pool but must exist as a culture instance once it emerges. Proposed: a **hybrid definition table** (built from the rework CSV — parent pair → name + linguistic profile) plus **trait blending** from the two parent base records for the mechanical fields (aggression, defense, sphere weights, terrain affinity, alignment, lifecycle), with `civ_or_clan` set by §3.4. The instance carries `culture_synthesis_parents = [base_a_id, base_b_id]` (generalizing the existing `synthesis_sources`).
...
- **Concern — Hybrid as a new data shape.** Culture instances are currently per-realm and sourced from static catalog JSON. Sim-time synthesized hybrids are a genuine departure; the `culture_synthesis_parents` field and a synthesis path touch the culture-instance contract. This is the highest-architecture-risk item and should get an Opus review when specced.
