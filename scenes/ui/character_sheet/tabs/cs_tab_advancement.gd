...
func display(bundle: CharacterBundle, registries: Dictionary) -> void:
	...
	var class_display_name := character.character_class.replace("_", " ").capitalize()
	if class_registry != null and class_registry.has_class(character.character_class):
		class_display_name = class_registry.get_class_display_name(character.character_class, character.sex)
	_add_row("Class:", class_display_name)
