...
## FoundSyndicateDialog — the syndicate-class analogue of EstablishDomainDialog
## (Thief→Syndicate refactor). Lets a level-9+ thief / assassin / elven nightblade
## found a SYNDICATE operated from a HIDEOUT in a chosen urban settlement. Calls
## FoundSyndicateFlow.found_syndicate on confirm and emits
## `syndicate_founded_requested(syndicate_id)` so the Domain tab can refresh.
##
## v1 simplification: the host-settlement dropdown lists ALL campaign settlements;
## the hideout is placed at the chosen settlement's own hex, which is always
## within RAW's 6-mile range — there is no party-proximity filter yet.


signal syndicate_founded_requested(syndicate_id: String)
...
## market_class INTEGER → roman numeral, for display.
const _ROMAN := {6: "VI", 5: "V", 4: "IV", 3: "III", 2: "II", 1: "I"}


func _ready() -> void:
	title = "Found a Syndicate"
...
func _build_ui() -> void:
	...
	intro.text = ("Found a syndicate from a hideout planted in an urban settlement. The "
		+ "hideout secures no domain and attracts no families — it is the secret base from "
		+ "which your guild runs hijinks. You gain 2d6 1st-level followers of your class.")
...
## Configure before popup. [param character] should be a CharacterData-shape
## dict with `id`, `character_class`, and `level`.
...
func _refresh_preview() -> void:
	...
	var max_size := HideoutCostTable.max_syndicate_for_market_class(mc)
	_preview_label.text = "Hideout minimum cost: %d gp   •   Maximum syndicate: %d members" % [
		min_gp, max_size]
...
func _on_confirmed() -> void:
	...
	if not result["errors"].is_empty():
		_error_label.text = "Could not found syndicate: %s" % str(result["errors"])
		return
	syndicate_founded_requested.emit(String(result["syndicate_id"]))
	hide()
