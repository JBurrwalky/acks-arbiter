...
func test_paralyzed_full_profile() -> void:
	var catalog := ConditionCatalog.new()
	check(catalog.prevents_action("paralyzed", "attacking"), "paralyzed: prevents attacking")
	check(catalog.prevents_action("paralyzed", "casting"), "paralyzed: prevents casting")
	check(catalog.prevents_action("paralyzed", "movement"), "paralyzed: prevents movement")
	check(catalog.prevents_action("paralyzed", "speech"), "paralyzed: prevents speech")
	check(catalog.is_helpless("paralyzed"), "paralyzed: is helpless")
	check(catalog.is_vulnerable("paralyzed"), "paralyzed: is vulnerable")
	check(catalog.grants_auto_hit_melee("paralyzed"), "paralyzed: auto-hit melee")
	check(catalog.get_attacker_bonus_vs("paralyzed") == 4, "paralyzed: +4 ranged bonus")
	check(catalog.get_attacker_bonus_vs("paralyzed", "melee") == 2, "paralyzed: +2 melee bonus")


func test_charging_profile() -> void:
	var catalog := ConditionCatalog.new()
	check(not catalog.prevents_action("charging", "attacking"), "charging: does not prevent attacking")
	check(catalog.get_attack_modifier("charging") == 2, "charging: +2 attack")
	check(catalog.get_ac_modifier("charging") == -2, "charging: -2 AC")
	check(not catalog.is_helpless("charging"), "charging: not helpless")
	check(not catalog.is_vulnerable("charging"), "charging: not vulnerable")


func test_restrained_can_still_attack() -> void:
	var catalog := ConditionCatalog.new()
	check(not catalog.prevents_action("restrained", "attacking"),
		"ConditionCatalog: restrained should NOT prevent attacking")
	check(not catalog.prevents_action("restrained", "casting"),
		"ConditionCatalog: restrained should NOT prevent casting")
	check(catalog.prevents_action("restrained", "movement"),
...
func test_vulnerable_no_helpless() -> void:
	var catalog := ConditionCatalog.new()
	check(catalog.is_vulnerable("vulnerable"), "vulnerable: is vulnerable")
	check(not catalog.is_helpless("vulnerable"), "vulnerable: is NOT helpless")
	check(not catalog.grants_auto_hit_melee("vulnerable"), "vulnerable: no auto-hit melee")
	check(catalog.get_attacker_bonus_vs("vulnerable", "melee") == 2, "vulnerable: +2 melee bonus")
	check(catalog.get_attacker_bonus_vs("vulnerable") == 4, "vulnerable: +4 ranged bonus")
