...
func test_get_flag_sources() -> void:
	...
	check("fly_spell" in sources,
		"EntityFlags: fly_spell should be in sources")
	check("magic_carpet" in sources,
...
func test_get_flag_metadata() -> void:
	...
	check(meta.get("break_on_attack", false) == true,
...
func test_set_flag_updates_existing_source_metadata() -> void:
	...
	var meta := f.get_flag_metadata("is_invisible")
	check(meta.get("break_on_attack", true) == false,
		"EntityFlags: re-setting same source should update metadata")
	check(f.get_flag_sources("is_invisible").size() == 1,
...
func test_get_all_flags() -> void:
	...
	check("can_fly" in all_flags, "EntityFlags: can_fly should be in all_flags")
	check("is_invisible" in all_flags, "EntityFlags: is_invisible should be in all_flags")
	check("is_hasted" in all_flags, "EntityFlags: is_hasted should be in all_flags")
...
func test_clear_all() -> void:
	...
	f.set_flag("is_invisible", "s2")
	f.clear()
	check(f.get_all_flags().is_empty(),
...
func test_clear_nonexistent_flag_is_safe() -> void:
	...
	check(true, "EntityFlags: clearing nonexistent flags should not crash")
