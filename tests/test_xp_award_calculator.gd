...
func test_monster_xp_basic() -> void:
	...
	check(xp == 10, "HD1 no abilities: expected 10, got %d" % xp)
...
func test_monster_xp_with_abilities() -> void:
	...
	check(xp == 500, "HD5 2 abilities: expected 500, got %d" % xp)
...
func test_monster_xp_high_hd() -> void:
	...
	check(xp_0 == 3250, "HD22 0 abilities: expected 3250, got %d" % xp_0)
	## With 1 ability: 3250 + 2250 = 5500.
	var xp_1 := XPAwardCalculator.calculate_monster_xp("22", 1)
	check(xp_1 == 5500, "HD22 1 ability: expected 5500, got %d" % xp_1)
...
func test_party_shares_even() -> void:
	...
	check(shares["a"] == 300, "PC a: expected 300, got %d" % shares["a"])
	check(shares["b"] == 300, "PC b: expected 300, got %d" % shares["b"])
	check(shares["c"] == 300, "PC c: expected 300, got %d" % shares["c"])
...
func test_party_shares_with_henchmen() -> void:
	...
	check(shares["pc1"] == 200, "PC1: expected 200, got %d" % shares["pc1"])
	check(shares["pc2"] == 200, "PC2: expected 200, got %d" % shares["pc2"])
	check(shares["hm1"] == 100, "Henchman: expected 100, got %d" % shares["hm1"])
...
func test_prime_req_adjustment_positive() -> void:
	...
	check(result == 220, "+10%% on 200: expected 220, got %d" % result)
...
func test_prime_req_adjustment_negative() -> void:
	...
	check(result == 190, "-5%% on 200: expected 190, got %d" % result)
...
func test_clamp_to_one_level() -> void:
	...
	check(clamped == 3999, "Fighter L1 clamped from 5000 to 3999 (L3-1), got %d" % clamped)

	# Award that wouldn't exceed two levels should not be clamped.
	var not_clamped := calc.clamp_to_one_level(fighter, 1000)
	check(not_clamped == 1000, "Fighter L1 award of 1000 should not be clamped, got %d" % not_clamped)
...
func test_domain_xp_above_threshold() -> void:
	...
	check(xp == 3000, "Level 9 domain XP: expected 3000, got %d" % xp)
...
func test_domain_xp_below_threshold() -> void:
	...
	check(xp == 0, "Level 9 domain XP below threshold: expected 0, got %d" % xp)
...
func test_domain_xp_henchman() -> void:
	...
	check(xp == 1500, "Henchman domain XP: expected 1500, got %d" % xp)
...
func test_bankers_round_half_even() -> void:
	...
	check(r == 2, "bankers_round(2.5) should be 2, got %d" % r)
...
func test_bankers_round_half_odd() -> void:
	...
	check(r == 4, "bankers_round(3.5) should be 4, got %d" % r)
