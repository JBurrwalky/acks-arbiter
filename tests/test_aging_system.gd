...
func test_age_category_human_boundaries() -> void:
	...
	var aging := AgingSystem.new()
	# Youth: 13-17
	check(aging.get_age_category("human", 13) == "youth",  "age 13: expected youth")
	check(aging.get_age_category("human", 17) == "youth",  "age 17: expected youth")
	# Adult: 18-35
	check(aging.get_age_category("human", 18) == "adult",  "age 18: expected adult")
	check(aging.get_age_category("human", 35) == "adult",  "age 35: expected adult")
	# Middle aged: 36-55
	check(aging.get_age_category("human", 36) == "middle_aged", "age 36: expected middle_aged")
	check(aging.get_age_category("human", 55) == "middle_aged", "age 55: expected middle_aged")
	# Old: 56-75
	check(aging.get_age_category("human", 56) == "old",    "age 56: expected old")
	check(aging.get_age_category("human", 75) == "old",    "age 75: expected old")
	# Ancient: 76-95
	check(aging.get_age_category("human", 76) == "ancient","age 76: expected ancient")
	check(aging.get_age_category("human", 95) == "ancient","age 95: expected ancient")
...
func test_age_category_elf_capped() -> void:
	...
	var aging := AgingSystem.new()
	check(aging.get_age_category("elf", 51)  == "adult", "elf age 51: expected adult")
	check(aging.get_age_category("elf", 100) == "adult", "elf age 100: expected adult")
	check(aging.get_age_category("elf", 200) == "adult", "elf age 200: expected adult")
	...
	check(cat_500 == "adult", "elf age 500: expected adult, got '%s'" % cat_500)
...
func test_category_transition_adjustments_adult_to_middle_aged() -> void:
	...
	check(result.get("category_changed", false),
		"Category should have changed from adult to middle_aged")
	check(c.age_category == "middle_aged",
		"Expected middle_aged, got '%s'" % c.age_category)
	check(c.strength == str_before - 2,
		"STR: expected %d, got %d" % [str_before - 2, c.strength])
	check(c.dexterity == dex_before - 2,
		"DEX: expected %d, got %d" % [dex_before - 2, c.dexterity])
	check(c.constitution == con_before - 2,
		"CON: expected %d, got %d" % [con_before - 2, c.constitution])
	# INT and WIS should NOT change on adult→middle_aged transition.
	check(c.intelligence == int_before, "INT should not change on adult->middle_aged")
	check(c.wisdom == wis_before, "WIS should not change on adult->middle_aged")
...
func test_death_from_age_trigger_max() -> void:
	...
	check(check.get("required", false),
		"Death save should be required at max age 95")
	check(check.get("trigger", "") == "max_age_and_beyond",
...
func test_death_from_age_no_trigger() -> void:
	...
	var aging := AgingSystem.new()
	var check := aging.check_death_from_age(c)
	check(not check.get("required", false),
...
func test_next_category_age_human() -> void:
	...
	check(next == 36, "Human adult next category age: expected 36, got %d" % next)
	## Ancient has no next category.
	var no_next := aging.get_next_category_age("human", "ancient")
	check(no_next == -1, "Human ancient next category: expected -1, got %d" % no_next)
...
func test_no_adjustment_for_same_category() -> void:
	...
	check(c.age_category == "adult", "Should still be adult at age 25")
	check(c.strength == str_before, "STR should not change within same category")
