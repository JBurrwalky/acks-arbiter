...
func test_has_effect() -> void:
	...
	check(t.has_effect("e1"), "ActiveEffectTracker: has_effect should be true after add")
	check(not t.has_effect("e99"), "ActiveEffectTracker: has_effect should be false for unknown id")


func test_add_returns_effect_id() -> void:
	var t := ActiveEffectTracker.new()
	var returned_id := t.add_effect(_make_effect("e_abc", "c1", ["t1"]))
	check(returned_id == "e_abc",
...
func test_tick_rounds_decrements() -> void:
	...
	check(expired.is_empty(), "ActiveEffectTracker: no effect should expire after decrement")
	var e := t.get_effect("e1")
	check(e["duration_remaining"] == 3,
...
func test_break_concentration_removes_concentration_effects() -> void:
	...
	check(not t.has_effect("e1"), "ActiveEffectTracker: e1 should be removed")
	check(not t.has_effect("e2"), "ActiveEffectTracker: e2 should be removed")
...
func test_dispel_auto_succeeds_when_level_equal() -> void:
	...
	check(results[0]["dispelled"] == true,
...
func test_dispel_only_affects_target() -> void:
	...
	check(not t.has_effect("e_victim"),
		"ActiveEffectTracker: victim's effect should be dispelled")
	check(t.has_effect("e_other"),
...
func test_clear_removes_all() -> void:
	...
	t.clear()
	check(t.get_all_effects().is_empty(),
...
func test_tick_multiple_n() -> void:
	...
	check(expired.is_empty(), "ActiveEffectTracker: 10 - 4 = 6 rounds left, should not expire")
	var e := t.get_effect("e1")
	check(e["duration_remaining"] == 6,
