...
func run_all_tests() -> void:
	...
	# §9.3: stair anchors
	test_invalid_anchor_oob_rejected()
	test_invalid_anchor_conflicting_directions_rejected()
	test_anchor_creates_antechamber_room()
	test_anchor_position_becomes_stair()
	test_anchor_count_offsets_free_placement_stairs()
	if not has_failures():
...
func test_invalid_anchor_oob_rejected() -> void:
	# An anchor whose 3×3 antechamber would not fit in the grid must cause
	# compose() to return -1 (validation failure).
	var composer := DungeonRoomComposer.new()
	var theme := DungeonThemeCatalog.get_theme("wizards_dungeon")
	var rng := RandomNumberGenerator.new(); rng.seed = 300
	var bad_anchors: Array = [
		{"position": Vector2i(0, 0), "direction": DungeonStairData.DIRECTION_DOWN},
	]
	var result: int = composer.compose(21, 21, 5, Vector2i(2, 3), theme, 1, 1, rng, 1, bad_anchors)
	check(result == -1,
		"OOB anchor should cause compose to return -1, got %d" % result)


func test_invalid_anchor_conflicting_directions_rejected() -> void:
	# Two anchors at the same position with different directions → error.
	var composer := DungeonRoomComposer.new()
	var theme := DungeonThemeCatalog.get_theme("wizards_dungeon")
	var rng := RandomNumberGenerator.new(); rng.seed = 301
	var bad_anchors: Array = [
		{"position": Vector2i(10, 10), "direction": DungeonStairData.DIRECTION_UP},
		{"position": Vector2i(10, 10), "direction": DungeonStairData.DIRECTION_DOWN},
	]
	var result: int = composer.compose(31, 31, 5, Vector2i(2, 3), theme, 1, 1, rng, 1, bad_anchors)
	check(result == -1,
		"conflicting anchors should cause compose to return -1, got %d" % result)


func test_anchor_creates_antechamber_room() -> void:
	# A valid anchor produces a 3×3 RoomPlan with is_anchor_room = true,
	# centered on the anchor position.
	var composer := DungeonRoomComposer.new()
	var theme := DungeonThemeCatalog.get_theme("wizards_dungeon")
	var rng := RandomNumberGenerator.new(); rng.seed = 302
	var anchors: Array = [
		{"position": Vector2i(15, 15), "direction": DungeonStairData.DIRECTION_DOWN},
	]
	composer.compose(31, 31, 5, Vector2i(2, 3), theme, 1, 1, rng, 1, anchors)
	# Find the anchor room.
	var anchor_room: DungeonRoomComposer.RoomPlan = null
	for r in composer.rooms:
		var room: DungeonRoomComposer.RoomPlan = r
		if room.is_anchor_room:
			anchor_room = room
			break
	check(anchor_room != null, "expected at least one is_anchor_room=true RoomPlan")
	if anchor_room != null:
		check(anchor_room.bounds.size == Vector2i(3, 3),
			"anchor antechamber should be 3x3, got %s" % anchor_room.bounds.size)
		check(anchor_room.bounds.has_point(Vector2i(15, 15)),
			"anchor antechamber should contain the anchor cell (15,15); bounds=%s" % anchor_room.bounds)


func test_anchor_position_becomes_stair() -> void:
	var composer := DungeonRoomComposer.new()
	var theme := DungeonThemeCatalog.get_theme("wizards_dungeon")
	var rng := RandomNumberGenerator.new(); rng.seed = 303
	var anchors: Array = [
		{"position": Vector2i(15, 15), "direction": DungeonStairData.DIRECTION_UP},
	]
	composer.compose(31, 31, 5, Vector2i(2, 3), theme, 1, 1, rng, 1, anchors)
	var found_anchor_stair: bool = false
	for s in composer.stairs:
		var stair: DungeonRoomComposer.StairPlan = s
		if stair.position == Vector2i(15, 15) and stair.direction == DungeonStairData.DIRECTION_UP:
			found_anchor_stair = true
			break
	check(found_anchor_stair,
		"expected a stair_up at the anchor position (15,15); got %s" % str(composer.stairs))


func test_anchor_count_offsets_free_placement_stairs() -> void:
	# Request: stairs_up=2, stairs_down=1. One anchor provides 1 up-stair.
	# Free placement should add 1 more up-stair + 1 down-stair (total 3).
	var composer := DungeonRoomComposer.new()
	var theme := DungeonThemeCatalog.get_theme("wizards_dungeon")
	var rng := RandomNumberGenerator.new(); rng.seed = 304
	var anchors: Array = [
		{"position": Vector2i(15, 15), "direction": DungeonStairData.DIRECTION_UP},
	]
	composer.compose(31, 31, 8, Vector2i(2, 4), theme, 2, 1, rng, 1, anchors)
	var ups: int = 0
	var downs: int = 0
	for s in composer.stairs:
		var stair: DungeonRoomComposer.StairPlan = s
		if stair.direction == DungeonStairData.DIRECTION_UP:
			...
			downs += 1
	check(ups == 2, "expected 2 up-stairs (1 anchor + 1 free), got %d" % ups)
	check(downs == 1, "expected 1 down-stair (free placement), got %d" % downs)
