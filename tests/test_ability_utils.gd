...
func test_xp_adjustment_single_prime() -> void:
	...
	check(AbilityUtils.get_xp_adjustment([16]) == 10,
		"single prime 16 should give +10%")
	check(AbilityUtils.get_xp_adjustment([13]) == 5,
		"single prime 13 should give +5%")
	check(AbilityUtils.get_xp_adjustment([10]) == 0,
		"single prime 10 should give 0%")
	check(AbilityUtils.get_xp_adjustment([5]) == -10,
		"single prime 5 should give -10%")
	check(AbilityUtils.get_xp_adjustment([8]) == -5,
...
func test_xp_adjustment_multiple_primes() -> void:
	...
	check(AbilityUtils.get_xp_adjustment([16, 10]) == 0,
		"[16, 10] should use lowest (10) -> 0%")
	check(AbilityUtils.get_xp_adjustment([13, 15]) == 5,
		"[13, 15] should use lowest (13) -> +5%")
	check(AbilityUtils.get_xp_adjustment([18, 18]) == 10,
		"[18, 18] should give +10%")
	check(AbilityUtils.get_xp_adjustment([16, 5]) == -10,
...
func test_max_henchmen() -> void:
	...
	# CHA 3 -> mod -3 -> 4+(-3)=1
	check(AbilityUtils.get_max_henchmen(3) == 1,
		"CHA 3 -> 1 henchman")
	# CHA 10 -> mod 0 -> 4+0=4
	check(AbilityUtils.get_max_henchmen(10) == 4,
		"CHA 10 -> 4 henchmen")
	# CHA 18 -> mod +3 -> 4+3=7
	check(AbilityUtils.get_max_henchmen(18) == 7,
		"CHA 18 -> 7 henchmen")
	# CHA 13 -> mod +1 -> 4+1=5
	check(AbilityUtils.get_max_henchmen(13) == 5,
		"CHA 13 -> 5 henchmen")
	# CHA 6 -> mod -1 -> 4+(-1)=3
	check(AbilityUtils.get_max_henchmen(6) == 3,
		"CHA 6 -> 3 henchmen")
...
func test_languages_bonus() -> void:
	...
	check(AbilityUtils.get_languages_bonus(3) == 0,
		"INT 3 -> 0 bonus languages (min 0)")
	# INT 10 -> mod 0 -> 0
	check(AbilityUtils.get_languages_bonus(10) == 0,
		"INT 10 -> 0 bonus languages")
	# INT 13 -> mod +1 -> 1
	check(AbilityUtils.get_languages_bonus(13) == 1,
		"INT 13 -> 1 bonus language")
	# INT 18 -> mod +3 -> 3
	check(AbilityUtils.get_languages_bonus(18) == 3,
		"INT 18 -> 3 bonus languages")
	# INT 8 -> mod -1 -> clamped to 0
	check(AbilityUtils.get_languages_bonus(8) == 0,
