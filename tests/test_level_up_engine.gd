...
func test_can_level_up_eligible() -> void:
	...
	check(engine.can_level_up(fighter), "Fighter with 2000 XP at L1 should be eligible")
...
func test_can_level_up_xp_too_low() -> void:
	...
	var engine := _make_engine()
	check(not engine.can_level_up(fighter), "Fighter with 1999 XP at L1 should not be eligible")
...
func test_can_level_up_max_level() -> void:
	...
	var engine := _make_engine()
	check(not engine.can_level_up(fighter), "Max-level fighter should never be eligible")
...
func test_hp_roll_within_hd_count() -> void:
	...
	check(gained == 5, "Fighter L1->L2 HP (rolled 5, CON +0): expected 5, got %d" % gained)
...
func test_hp_roll_past_hd_count() -> void:
	...
	var engine := _make_engine()
	var gained := engine.roll_level_up_hp(fighter)
	check(gained == 2, "Fighter L9->L10 HP (fixed +2): expected 2, got %d" % gained)
...
func test_spell_slot_expansion_mage() -> void:
	...
	check(not slots_l3.is_empty(), "Mage L3 should have spell slots")
	check(int(slots_l2[0]) == 2, "Mage L2 slot[0]: expected 2, got %d" % slots_l2[0])
	check(int(slots_l3[0]) == 2, "Mage L3 slot[0]: expected 2, got %d" % slots_l3[0])
	check(int(slots_l3[1]) >= 1, "Mage L3 slot[1]: expected >= 1, got %d" % slots_l3[1])
	check(int(slots_l2[1]) == 0, "Mage L2 slot[1]: expected 0, got %d" % slots_l2[1])
...
func test_auto_level_up_updates_level_and_title() -> void:
	...
	check(result.get("new_level", 0) == 2, "Expected new_level=2, got %d" % result.get("new_level", 0))
	check(not (result.get("new_title", "") as String).is_empty(), "new_title should not be empty")
	check(int(result.get("hp_gained", 0)) >= 1, "hp_gained should be >= 1")
