...
func test_known_power_exists() -> void:
	var reg := PowerRegistry.new()
	check(reg.has_power("backstab") == true,
		"backstab should exist in power catalog")
	check(reg.has_power("open_locks") == true,
		"open_locks should exist in power catalog")
	check(reg.has_power("nonexistent") == false,
		"nonexistent should not exist in power catalog")
	check(reg.has_power("") == false,
...
func test_power_type_filter() -> void:
	...
	for power in skill_throws:
		if power.get("power_id", "") == "open_locks":
			...
			break
	check(found_open_locks, "open_locks should be in skill_throw powers")
...
func test_power_metadata() -> void:
	...
	check(not backstab.is_empty(), "backstab power should not be empty")
	check(backstab.has("power_name"), "backstab should have power_name")
	check(backstab.get("power_name", "") == "Backstab",
		"backstab power_name should be 'Backstab'")
	check(backstab.has("power_type"), "backstab should have power_type")
	check(backstab.get("power_type", "") == "scaling_multiplier",
		"backstab power_type should be 'scaling_multiplier'")
	check(backstab.has("description"), "backstab should have description")
	check(not backstab.get("description", "").is_empty(),
