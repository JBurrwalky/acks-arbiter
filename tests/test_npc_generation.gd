...
func test_npc_generation_basic() -> void:
	...
	for score in [npc.strength, npc.intelligence, npc.wisdom,
		...
		assert(score >= 3 and score <= 18,
			"ability score %d out of range [3, 18]" % score)
	# HP should be at least 1
	assert(npc.hp_max >= 1, "HP should be >= 1")
	# ID should be set
	assert(not npc.id.is_empty(), "NPC ID should not be empty")
...
func test_npc_attack_throw() -> void:
	...
	assert(npc.attack_throw == expected_at,
	...
	assert(fighter_npc.attack_throw == expected_at_f,
...
func test_npc_proficiencies() -> void:
	...
	for p in proficiencies:
		if p.get("proficiency_key", "") == "adventuring":
			...
			break
	assert(has_adventuring,
		"adventuring should be auto-selected for NPC proficiencies")

	# All proficiencies should have required fields
	for p in proficiencies:
		assert(p.has("proficiency_key"), "proficiency should have proficiency_key")
		assert(p.has("rank"), "proficiency should have rank")
		assert(p.has("slot_type"), "proficiency should have slot_type")
		assert(p.get("slot_type", "") in ["general", "class"],
