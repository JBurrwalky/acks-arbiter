...
func test_disabled_classes_are_hidden_from_roster() -> void:
	...
	for disabled_id in ["elven_ranger", "dwarven_delver"]:
		check(not panel._class_buttons.has(disabled_id),
			"%s is disabled and must not appear as a selectable class button" % disabled_id)
	# Warlock, mystic, anti-paladin, and elven_courtier are all enabled and
	# must appear for qualifying scores.
	for enabled_id in ["warlock", "mystic", "anti_paladin", "elven_courtier"]:
