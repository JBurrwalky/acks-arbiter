...
func test_character_round_trip() -> void:
	...
	check(saved, "save_character should succeed")

	# Load
	var loaded_dict := CampaignRepository.get_character(c.id)
	check(not loaded_dict.is_empty(), "get_character should return data")

	var loaded := CharacterData.from_dict(loaded_dict)

	# Verify all fields match
	check(loaded.id == c.id, "id should match")
	check(loaded.campaign_id == c.campaign_id, "campaign_id should match")
	check(loaded.name == c.name, "name should match")
	check(loaded.character_type == c.character_type, "character_type should match")
	check(loaded.race == c.race, "race should match")
	check(loaded.character_class == c.character_class, "character_class should match")
	check(loaded.level == c.level, "level should match")
	check(loaded.xp == c.xp, "xp should match")
	check(loaded.combat_progression == c.combat_progression, "combat_progression should match")
	check(loaded.strength == c.strength, "STR should match")
	check(loaded.intelligence == c.intelligence, "INT should match")
	check(loaded.wisdom == c.wisdom, "WIS should match")
	check(loaded.dexterity == c.dexterity, "DEX should match")
	check(loaded.constitution == c.constitution, "CON should match")
	check(loaded.charisma == c.charisma, "CHA should match")
	check(loaded.hp_max == c.hp_max, "hp_max should match")
	check(loaded.hp_current == c.hp_current, "hp_current should match")
	check(loaded.armor_class == c.armor_class, "armor_class should match")
	check(loaded.attack_throw == c.attack_throw, "attack_throw should match")
	check(loaded.save_petrification == c.save_petrification, "save_petrification should match")
	check(loaded.save_poison_death == c.save_poison_death, "save_poison_death should match")
	check(loaded.save_blast_breath == c.save_blast_breath, "save_blast_breath should match")
	check(loaded.save_staffs_wands == c.save_staffs_wands, "save_staffs_wands should match")
	check(loaded.save_spells == c.save_spells, "save_spells should match")
	check(loaded.hit_die_type == c.hit_die_type, "hit_die_type should match")
	check(loaded.max_level == c.max_level, "max_level should match")
	check(loaded.xp_for_next_level == c.xp_for_next_level, "xp_for_next_level should match")
	check(loaded.xp_adjustment_percent == c.xp_adjustment_percent, "xp_adjustment_percent should match")
	check(loaded.title == c.title, "title should match")
	check(loaded.alignment == c.alignment, "alignment should match")
	check(loaded.is_dead == c.is_dead, "is_dead should match")
	check(loaded.is_active == c.is_active, "is_active should match")
...
func test_powers_round_trip() -> void:
	...
	check(saved, "save_character_powers should succeed")
	...
	check("backstab" in power_ids, "backstab should be in loaded powers")
	check("open_locks" in power_ids, "open_locks should be in loaded powers")

	# Verify progression_data survives the round-trip
	for p in loaded:
		if p.get("power_id", "") == "backstab":
			var prog_str: String = p.get("progression_data", "")
			check(not prog_str.is_empty(),
...
func test_proficiencies_round_trip() -> void:
	...
	check(saved, "save_character_proficiencies should succeed")
	...
	check("adventuring" in prof_keys, "adventuring should be in loaded proficiencies")
	check("combat_reflexes" in prof_keys, "combat_reflexes should be in loaded proficiencies")
	check("riding" in prof_keys, "riding should be in loaded proficiencies")
...
func test_inventory_round_trip() -> void:
	...
	check(saved, "save_character_inventory should succeed")
	...
	for item in loaded:
		if item.get("item_key", "") == "longsword_plus_1":
			sword_found = true
			check(item.get("name", "") == "Longsword +1",
				"sword name should match")
			check(int(item.get("encumbrance_units", 0)) == 1000,
				"sword encumbrance should be 1000 units (1 stone)")
			check(item.get("item_category", "") == "weapon",
				"sword category should be 'weapon'")
			check(int(item.get("is_magical", 0)) == 1,
				"sword should be magical")
			check(int(item.get("magical_bonus", 0)) == 1,
				"sword magical_bonus should be 1")
			check(item.get("weapon_damage", "") == "1d8",
				"sword weapon_damage should be '1d8'")
			break
	check(sword_found, "longsword_plus_1 should be in loaded inventory")
