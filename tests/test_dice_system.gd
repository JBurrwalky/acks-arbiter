...
func test_roll_digital_positive_modifier() -> void:
	for i in 20:
		...
		check(r.modified_total == r.raw_total + 2,
			"modifier +2 not applied: raw=%d modified=%d" % [r.raw_total, r.modified_total])
		check(r.modified_total >= 3 and r.modified_total <= 8,
...
func test_roll_digital_negative_modifier() -> void:
	for i in 20:
		...
		check(r.modified_total == r.raw_total - 1,
...
func test_roll_digital_zero_modifier() -> void:
	...
	check(r.modified_total == r.raw_total,
...
func test_roll_result_fields_populated() -> void:
	...
	check(r.roll_type == "attack_throw", "roll_type not set")
	check(r.sides == 20, "sides not set")
	check(r.count == 1, "count not set")
	check(r.modifier == 3, "modifier not set")
	check(r.individual_results.size() == 1, "individual_results should have 1 entry")
	check(r.raw_total >= 1 and r.raw_total <= 20, "raw_total out of range")
	check(r.modified_total == r.raw_total + 3, "modified_total wrong")
	check(not r.was_overridden, "was_overridden should be false")
	check(not r.was_player_entered, "was_player_entered should be false")
...
func test_natural_one_flag() -> void:
	...
	check(r.natural_one, "natural_one should be true when result is 1")
	check(not r.natural_max, "natural_max should be false when result is 1")
...
func test_natural_max_flag() -> void:
	...
	check(r.natural_max, "natural_max should be true when d20 result is 20")
	check(not r.natural_one, "natural_one should be false when result is 20")

func test_natural_flags_false_on_multi_die() -> void:
	# Multi-die rolls never set natural_one / natural_max regardless of total
	for i in 20:
		var r := DiceSystem.roll_digital(6, 3)
		check(not r.natural_one, "natural_one should always be false for 3d6")
		check(not r.natural_max, "natural_max should always be false for 3d6")
...
func test_override_consumed_on_matching_type() -> void:
	...
	check(r.was_overridden, "was_overridden should be true")
	check(r.modified_total == 7, "override forced value not used: got %d" % r.modified_total)
	check(not GameState.dice_overrides.has("damage_roll"),
...
func test_override_not_consumed_on_different_type() -> void:
	...
	check(GameState.dice_overrides.has("morale_check"),
...
func test_override_single_use() -> void:
	...
	check(r1.was_overridden, "first roll should use override")
	var r2 := DiceSystem.roll_digital(6, 1, 0, "initiative")
	check(not r2.was_overridden, "second roll should NOT use override — already consumed")
...
func test_override_was_overridden_flag() -> void:
	...
	check(r.was_overridden, "was_overridden must be true for forced roll")
	check(not r.was_player_entered, "was_player_entered must be false for override")
...
func test_expression_parse_simple() -> void:
	...
	check(r.count == 2, "2d6 count should be 2, got %d" % r.count)
	check(r.sides == 6, "2d6 sides should be 6, got %d" % r.sides)
	check(r.modifier == 0, "2d6 modifier should be 0")
	check(r.modified_total >= 2 and r.modified_total <= 12,
...
func test_expression_parse_with_positive_modifier() -> void:
	...
	check(r.modifier == 3, "1d8+3 modifier should be 3, got %d" % r.modifier)
	check(r.modified_total >= 4 and r.modified_total <= 11,
...
func test_expression_parse_with_negative_modifier() -> void:
	...
	check(r.modifier == -2, "3d6-2 modifier should be -2, got %d" % r.modifier)
	check(r.modified_total >= 1 and r.modified_total <= 16,
...
func test_expression_parse_invalid_returns_zeroed_result() -> void:
	...
	check(r.modified_total == 0,
		"invalid expression should return zeroed result, got %d" % r.modified_total)
	check(r.sides == 6, "invalid expression zeroed result should have default sides=6")
