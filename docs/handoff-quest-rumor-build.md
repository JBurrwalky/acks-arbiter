...
| **Q-3** | Rumor delivery: reaction-share, carousing hook (60/40 cash/rumor + 5%/level accuracy bonus), venturer-rumormonger hook, notice-board read, **Gather Information (distinct 1-hour activity)**, monthly decay + invalidation. No reliability cue (verification-only). | Q-2; hijink/carousing system (built); Dialogue P1 *or* standalone Gather-Info verb | Sonnet |
| **Q-4** | `QuestCompletionWatcher` over existing signals; turn-in + reward disbursement (all types; XP = reward GP value; domain single-owner, no level gate); failure/expiry/abandon; Unified-Log `quest` entries | Q-1; combat/lair/exploration signals (built) | Sonnet (Opus for domain-grant/vassalage path) |
...
**Global definition of done (Q-1…Q-6):** on a freshly generated world — the seeder mints 3–8 quests per standard region, each with a solvent questgiver, a resolvable completion, and a quest-sourced rumor; identical seed → byte-identical mechanical quest/rumor columns (prose columns excluded from the hash, GDD §10.3); a player can hear a rumor (carouse/board/ask/Gather-Info), travel, satisfy a completion, turn in, and receive the reward + XP equal to the reward's GP value (domain grants XP-exempt); rumors decay and invalidate on source destruction; save/load mid-quest restores exact state; a dead questgiver's quests fail cleanly; the whole loop runs on the mock provider; suite net-zero each session; `build_log.md` entry per session.

**Reward-XP rule (RESOLVED, GDD §8.2 / O-Q1):** quest rewards grant XP equal to their GP value on disbursement (gold at face; item/favor at GP-equivalent); **domain grants are XP-exempt** (a domain generates its own passive XP). `xp_eligible` defaults **true** and remains only as a rare per-quest override. No open gating decision remains — all of GDD §16 O-Q1–O-Q14 are resolved.
...
**GDD refs.** §12 (tables — column lists normative; note `domain_grants.single_owner_pc_id`, no `reliability` on rumors), §3.1 (services/placement), §4.1 (rumor runtime fields), §6.6 (quest fields), §8 (RewardValuator formulas — §8.1 gold incl. **2× monster-XP bounty + pre-estimated treasure**, §8.2 reward-XP ruling, §8.3 tone, §8.5 recovery, §8.6 affordability, §8.8 domain-grant equivalent), §11.6 (the twelve signals), §10.3 (determinism + hasher treatment).
...
- `engine/subsystems/quests/reward_valuator.gd` (`class_name RewardValuator`, pure/static) — §8.1 formula: treasure-bearing = **pre-estimated treasure × multiplier**, **creature bounty = 2× monster XP**, plus the expected-treasure estimation from ACKS treasure types; ±10% variance, 25/100-gp banker's rounding, §8.3 Motivation tone nudge, §8.5 recovery, §8.6 affordability clamp, §8.8 domain gp-equivalent; reward XP = `total_gp_value` on disbursement (domain XP-exempt, §8.2). `xp_eligible` default true.
...
**Goal.** Make rumors reachable: reaction-share on conversation, the carousing pool, venturer rumormongering, the notice board, the distinct Gather-Information activity, and the monthly decay + source-invalidation pass. After this, players hear about the world. **No reliability cue** — accuracy is discoverable only by verification (§4.4).

**GDD refs.** §4.3 (channels — a: carousing **60/40 cash/rumor + 5%/level accuracy bonus**, b: spying, c: reaction-share + the distinct 1-hour **Gather-Information** activity, d: venturer), §4.4 (accuracy hidden — verification-only, no reliability cue), §4.5 (decay), §4.6 (invalidation), §5 (notice board read; `market_class ≥ IV`), §2 constraints (carousing `acore-campaign-hijinks.xml:130-172` — also read `ax_campaign_play.xml` for carousing **duration**; spying `:174-180`; venturer `ax_venturer_class.xml:172-177` — cite, don't reinvent), §10.1 (the monthly tick order).
...
- **Gather Information** (GDD §4.3c): a distinct **1-hour** settlement activity (not carousing) — a reaction roll against a district's public NPCs; on success surface one rumor to an "I heard…" modal + session log + Quests-tab tracker; **no gp reward, no criminal-charge risk**. Built on `share_for_npc`; hooked for the future nested District/POI settlement menu (wire the mechanic now, the surface later).
- **No reliability cue** (GDD §4.4, O-Q3): do NOT expose a reliability/trust signal; accuracy stays hidden until the party verifies the rumor at its source. (There is no `reliability` column — §12.)
...
**Acceptance bar.** A caroused Hear-Noise success rolls 60/40 cash/rumor; on the rumor branch it returns a weighted draw (unheard×3/quest×2), marked known, `rumor_heard` emitted, and applies the +5%/carouser-level accuracy bonus (20th level always `true`); Gather Information (1-hour) resolves a public-NPC reaction roll → one rumor to the modal/log/tracker, no gp and no criminal charge; the board reveals quests + public rumors with no throw, gated at Market Class IV; venturer path yields 1d4 rumors once/month; **no reliability cue is surfaced** (accuracy hidden until verification); the monthly decay ages `current`→`stale` after 1d6 months (seeded), persistent rumors never decay, and source destruction invalidates immediately; determinism on the decay pass; suite net-zero.

**Paste-ready prompt:**
> Implement session Q-3 per `generation/gdd-quest-rumor-system.md` §4.3–§4.6/§5/§10.1 and `docs/handoff-quest-rumor-build.md` §5. Run the Build Session Protocol; cite carousing/spying/venturer RAW via `acks-raw-lookup`. Build `RumorRegistry.share_for_npc` (per-band reaction-share), the carousing-pool hook (60/40 cash/rumor on Hear-Noise success + weighted draw + 5%/carouser-level accuracy bonus; check `ax_campaign_play.xml` for duration — verify the carousing surface in `activity_time_cost_executor.gd`/`settlement_handlers.gd`/`thief_skill_resolver.gd`), the venturer rumormonger hook, the notice-board read (no-throw reveal, `market_class ≥ IV`), the distinct 1-hour **Gather Information** activity (public-NPC reaction roll → modal/log/tracker, no gp/no criminal charge), and `RumorRegistry.decay_pass` slotted into `DomainHandlers._handle_monthly_tick` in the §10.1 order (batch style, no auto_pause/LLM). **Do NOT build a reliability cue** — accuracy is verification-only (§4.4). Tests: 60/40 carouse split + accuracy bonus, Gather-Information resolve, board reveal + market gate, venturer 1d4, decay-to-stale + persistence + invalidation, determinism. Net-zero suite; `build_log.md` entry.

---

## 6. Session Q-4 — Completion, turn-in, and lifecycle (the spine)

**Goal.** Detect completion against real signals, disburse rewards (all types; **XP = reward GP value**; **domain forces single-owner selection, no level gate**), and handle failure/expiry/abandon — with Unified-Log entries at each beat. Opus for the domain-grant/vassalage disbursement path.

**GDD refs.** §9 (§9.4 completion watcher + the signal-mapping table, §9.5 turn-in, §9.6 recipient selection, §9.7 failure/expiry/abandon), §8.2 (reward-XP = GP value; domain XP-exempt), §8.8 (domain-grant conditions: secure territory, legitimate giver, vassalage, **one chosen owner — no level gate**), §16 O-Q1/O-Q4/O-Q14.
...
- Turn-in + disbursement in `QuestRegistry.disburse_reward(quest_id, recipient_pc_id)` (§9.5/§9.6): gold → PC, +1 XP/gp; item → inventory, XP = item GP-equivalent (no sold-unused double-count); political favor → recorded on the sheet, XP = GP-equivalent; **domain → force single-owner selection, vassalage flow, NO level gate (§8.8), XP-exempt** [Opus]. Emit `quest_turned_in`. Support the unaccepted-completion path (§9.5): pay if Friendly/Indifferent, not if Unfriendly — **except high-`honesty`/low-`self_interest` givers pay unless Hostile** (O-Q4).
...
**Acceptance bar.** Each `completion_type` flips `is_complete` exactly once on its mapped signal and is idempotent on re-fire; backdrop quests ignored; turn-in disburses per type with **XP = reward GP value** (domain XP-exempt); the domain path **forces single-owner selection with no level gate** and writes vassalage; the unaccepted path honors the O-Q4 personality gate (high-honesty/low-self-interest pays unless Hostile); expiry re-mints if the threat persists; failure/abandon transition + emit correctly; a dead questgiver fails its quests; **save/load mid-quest** (accepted, complete-not-turned-in, expired) restores exact state; determinism; suite net-zero.

**Paste-ready prompt:**
> Implement session Q-4 per `generation/gdd-quest-rumor-system.md` §9/§8.2/§8.8/§16 and `docs/handoff-quest-rumor-build.md` §6. Run the Build Session Protocol. Build `QuestCompletionWatcher` over the verified EventBus signals (`lair_cleared`:271, `combat_ended`:37, `combatant_downed`:48, `hex_entered`:82, `poi_discovered`:276 — mapping per §9.4; idempotent; backdrop-ignored; emits `quest_completion_ready`), the turn-in/`disburse_reward` flow (all reward types, **XP = reward GP value** with domain XP-exempt, **domain forces single-owner selection + vassalage, no level gate** [use Opus for this path], unaccepted-completion personality gate — high-honesty/low-self-interest pays unless Hostile; emits `quest_turned_in`), failure/expiry/abandon transitions (expiry re-mints if the threat persists), and `category:"quest"` Unified-Log entries via `log_entry_added`. Tests: per-signal completion + idempotency, disbursement per type + XP, single-owner domain (no level gate), unaccepted personality path, failure/expiry-remint/abandon, dead-questgiver fail, save/load mid-quest, determinism. Net-zero suite; `build_log.md` entry.
...
**Files.**
- `QuestRegistry.create_faction_quest(faction_id, front_npc_id, goal, terms)` — mints a `faction_goal`-or-typed quest with `questgiver_faction_id` set, reward drawn against the org treasury (faction §6.6), and a `faction_goal_id` when the success is a faction predicate. **Reward forms (O-Q12/O-Q13): gold only for now** (magic treasure once faction treasuries/inventories exist); **never faction membership/rank** (per-character, level/class-gated); guard against the impossible — no party-wide membership, and a domain reward still forces a single owner (§9.6). Improved faction standing is the separate ledger side-effect below, additive to the reward.
...
- Update `docs/coding_conventions.md`: the `quests/` service shape (repositories + pure valuator + signal-driven watcher + monthly batch pass), the seed→materialize pattern for quest/rumor rows, the reward-XP-on-disbursement convention (XP = reward GP value; domain XP-exempt), and the reward-valuation banker's-rounding buckets.
