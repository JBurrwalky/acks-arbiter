...
**Clock-driven scheduling (single shared timeline):** The game clock advances via the EventScheduler and SchedulerLoop. Each entity's orders are scheduled events in a priority queue keyed to absolute `fire_time` in elapsed rounds; the SchedulerLoop advances Timekeeping toward the next event and resolves it. There is ONE shared world timeline (Jedidiah rulings 2026-06-11/12): `Timekeeping.get_total_rounds()` is the canonical "now" for all scheduling, and per-party clocks do not exist. Asynchronous per-party timelines are permanently out of scope. Orders are not locked: **a new order supersedes the old one** — order surfaces cancel the party's pending travel/activity events before scheduling replacements. Time already spent is spent (the world clock moved); a cancelled activity yields nothing. Cross-party interactions resolve naturally when entities share a spatial location at the same timestamp. In the dungeon context, unit movement uses a renderer-tween-driven layer (`gdd-realtime-scheduler.md` §3): orders are converted to BFS paths, each cell is animated with tweens whose duration scales with clock speed, and cell-arrival signals update logical positions in `VoxelMapData`. **Combat pauses the SchedulerLoop** for combat's duration; on resume, the world clock advances by combat's elapsed rounds rounded up to the next turn boundary (ACKS RAW: combat shorter than a turn consumes a full turn), and any past-due scheduled events — including background parties' — resolve immediately in priority order: the world keeps moving. Non-combatant scheduled timers (search, lockpick, construction, research, missions) progress through combat naturally because they were scheduled with absolute `fire_time` (`gdd-realtime-scheduler.md` §6.8–§6.9). See `gdd-realtime-scheduler.md` §2–§3. *(Replaces the half-built per-party clock model, and before that the simultaneous-declaration day-cycle model.)*
...
Max 8 PCs. Henchmen per PC by CHA. Marching formation (point, front, middle, rear). Shared resource pool. Travel speed set by slowest member. Splitting/regrouping supported via the EventScheduler on the single shared timeline (ruling 2026-06-11) — split parties are independent entities whose orders are scheduler events owned by their `party_id`; daily ticks (sustenance, foraging, weather) are seeded per-party at split time, and a merge cancels the dissolved party's queued events (re-pointing the session at the survivor if the primary was merged away).
...
- Timekeeping (scheduler-driven clock advancement, turn/round zoom, single shared timeline)
