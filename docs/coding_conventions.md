...
### 9.7 Signal-observation and detached-instance anti-patterns (2026-07-14)

Two test-authoring bugs recur often enough to name. Both make a test fail while the production code is correct — the failure is in the test, not the system under test.

- **Never observe a signal by reassigning a captured primitive inside a lambda.** GDScript lambdas capture variables **by value**; reassigning a captured `bool`/`String`/`int` inside the handler mutates the lambda's private copy and never reaches the outer scope, so the post-signal `check()` reads the stale initial value and fails even though the signal fired correctly.

  ```gdscript
  # BAD — `emitted` is always false at the assertion, even though the signal fired.
  var emitted := false
  obj.some_signal.connect(func(): emitted = true)
  obj.do_thing()
  check(emitted, "signal emitted")   # FAILS spuriously

  # GOOD — mutate a reference type (Array/Dictionary) shared with the outer scope.
  var sink: Array = []
  obj.some_signal.connect(func(): sink.append(true))
  obj.do_thing()
  check(not sink.is_empty(), "signal emitted")
  ```
  For signals that carry payload, accumulate the payload: `sink.append([from, to])` then assert on `sink[-1]`, or `var seen := {}; ...seen["pos"] = p...` then assert on `seen["pos"]`. This is the pattern already used in `test_session_runner.gd::test_nested_transition_skips_stale_postamble`.

- **A bare `.new()` on a Node/scene script skips `_ready()`.** Fields that the script initializes in `_ready()` (registries, sub-services, scheduler loops, monster registries) stay `null`, so any method that assumes full init null-derefs and aborts mid-way — often *before* the line the test is actually checking, producing a misleading "got 0 / empty" result rather than an obvious crash. Fixes, in order of preference: call the script's own init helper if it is factored out (e.g. `screen._init_registries()`), set the specific fields the exercised path needs (`runner._monster_registry = MonsterRegistry.new()`), or use a helper that stubs them (`_make_runner_with_hex_controller()` wires `_scheduler`/`_handler_registry`/`_scheduler_loop`). If the code under test needs the scene tree (e.g. `PopupPanel.popup_centered_ratio` errors with `!is_inside_tree()`), `add_child(node)` before exercising it and free it afterward. Prefer the test-side fix over adding a production null-guard unless the guard is independently warranted.
### 9.8 Test-runner per-suite reclamation (2026-08-06)
...
| Loot policy — never silently discard | Encumbrance **bands are the soft cap** (a movement penalty), not a hard limit (`gdd-inventory-tab.md` O-I6: auto-distribute respects only the HARD cap). So `LootAutoDistributor.distribute()` places band-crossing loot on the best carrier that still fits under raw hard-cap (move `reason = "capacity_fallback"`, strongest carrier for heavy items); an item is left `unassigned` **only** when it exceeds EVERY carrier's raw capacity. Every unassigned entry carries an `unassigned_reason` (`"over_capacity"` \| `"coin_excluded"`); `describe_unassigned_reason()` is the static phrase helper. The loot modal auto-distributes on open, pre-selects each item's recipient picker (plus an explicit "— Leave on ground —" option), and on non-empty over-capacity surfaces a warning banner + **Make Room** flow (player drops chosen carried gear → capacity frees → distribution re-runs). Declined loot stays recoverable on the ground / in the source cache behind an explicit confirmation; fresh (no-cache) loot is grounded via `add_inventory_item` + `transfer_item_to_cache`, never deleted. | `loot_auto_distributor.gd`, `loot_distribution_modal.gd`, `gdd-party-inventory.md` §6.5/§7.4, 2026-07-14 |
...
| Type | Role |
|---|---|
| `VoxelCell` | Single 5' cube cell. Fields: `solidity`, `feature`, `floor_type`, `door_state/type/detected`, `fog_state` (String), `room_id`, `is_corridor`, `zone_index`, `cover_value`, `is_evil`. Derived methods: `is_passable_by_walker()`, `blocks_los()`, `blocks_flight()`, `blocks_burrow()`. (The legacy `stair_target_col/row/level` teleport-pairing fields were removed at DG-C3D.F.3 — composed dungeon stairs are ordinary walkable geometry.) |
...
Ground-walker level changes: level diff 0 = free, level diff 1 requires a stair/ramp feature with matching compass direction suffix (e.g., `stairs_up_N`, `ramp_SE`), level diff 2+ = blocked. EXCEPTION (2026-07-17): on maps flagged `VoxelMapData.natural_slopes = true` (generated wilderness battle maps only), any diagonal ±1-level step is legal without a feature — a natural 5' slope. See §124.

### 13.5 Movement modes

On `MovementResolver`'s 3D primitives:

- `"ground"` — passable + supported + stair for level changes (or natural slope on battle maps, §124); deep-water cells gate through the wading hook (`MovementRules.can_wade`, allowance 0 in v1)
...
- `"swimming"` — (hook, 2026-07-17; no swimmers wired) liquid water + shallow-water cells; lava never
...
## 56. Tier 3 UI display sweep — cp/gp boundary conventions (2026-05-19; extended 2026-07-31)
...
> **2026-07-31 — see §127.** Jedidiah has since ruled that **cp is the canonical unit for the entire codebase**, integers only. That supersedes this section's tolerance for gp-native internal math (the `TributeCalculator`-style calculators named below). The suffix and display-site discipline in this section is unchanged and is the enforcement mechanism for §127; only the "gp is an acceptable internal unit" allowance is withdrawn.
...
- **Per-troop-type stat tables stay separate from the race composition.** `_COMPOSITION_PER_120` and the stat tables are distinct dictionaries — one is "which troop types for this race", the other is "what does each troop type cost / weigh in battle". They join at the composition helper. **Pattern:** keep cross-cutting tables separate when they have different growth dimensions. Joining at the helper composes them.

  <!-- Corrected 2026-08-01 (Jedidiah ruling). -->
  **The stat side is keyed by (race, troop_type), not troop_type alone.** The original 2026-05-22 import used a single race-agnostic `_TROOP_TYPE_STATS` row per troop type, described as a v1 simplification. It is not a simplification — RAW's per-creature ratings are race-dependent by up to ~10× (light infantry: kobold 0.003 @2gp, goblin 0.004 @3gp, orc and human 0.008 @6gp, hobgoblin 0.017, gnoll 0.025, lizardman 0.036, bugbear 0.050, ogre 0.077 @40gp), so one value understated every ogre/bugbear/lizardman/gnoll/hobgoblin levy in both battle strength and payroll while overstating kobolds and goblins. The table is now `_HUMAN_TROOP_TYPE_STATS` (the three human tribal cultures) plus `_BEASTMAN_TROOP_TYPE_STATS` keyed by race, resolved through `TribalWarriorRegistry.stats_for(race, troop_type)` with the human table as fallback. Every `(race, troop_type)` pair reachable from `_COMPOSITION_PER_120` has an exact RAW row, so no value is interpolated. **Pattern:** before calling a per-X table a "representative value" simplification, check the spread across X — if it is an order of magnitude, the axis is load-bearing and belongs in the key.

  **Unit size is per (race, troop_type) too, and is not implied by `is_cavalry`.** RAW L273 sizes a unit at 120 for infantry but 60 for cavalry *or large creatures*, so ogre INFANTRY forms 60-strong units — confirmed by the per-unit wage arithmetic (2,400gp ÷ 60 = the 40gp per-creature wage). Supply follows the same split, and carnivorous mounts break it again: wolf- and boar-rider units pay 960gp/week per 60 riders, four times a normal squadron, per L274's "carnivorous units pay more". The stat rows therefore carry an explicit `unit_size` rather than deriving one from `is_cavalry`.
...
<!-- First bullet rewritten 2026-07-10: the dual-mechanism footgun was RESOLVED by
     deriving delete_campaign() from the scope map — the old "update both" rule is gone. -->
- **There is ONE per-campaign table registry: `_campaign_scope_entries()`** (`_SCOPE_DIRECT_CAMPAIGN` / `_SCOPE_VIA_*` / custom `e.append` entries in `CampaignRepository`). Both consumers derive from it — savegame restore (`_restore_campaign_from_slot`) and campaign deletion (`delete_campaign`), sharing the `_scope_where()` clause builder — and it is completeness-checked by `test_savegame_snapshot.gd` (`test_scope_map_covers_all_tables`: every DB table must be scoped or in `SNAPSHOT_EXCLUDED_TABLES`; `test_delete_campaign_purges_all_scoped_tables`: deletion leaves zero in-scope rows). When you add a new campaign-scoped table, classify it in the scope map — that is the ONLY place; restore and delete inherit it automatically. Historical note: before 2026-07-10, `delete_campaign()` was a second hand-rolled DELETE cascade that silently drifted (it never covered the `setting_*` family, dungeon-content tables, or `voxel_map_cells` — the latter scoped via `hex_maps` when `map_id` is actually a dungeon id). Two deliberate exceptions live outside the map's derivation: `game_snapshots` (excluded from snapshots, but `delete_campaign` still drops the campaign's slot metadata explicitly) and the `campaigns` root row (in the map for restore, but deleted explicitly last so the success contract keys to it). A table with MULTIPLE parent paths needs a union via-clause (`pending_divine_effects`: character OR domain; `henchman_pool_members`: character OR pool) — a single-parent clause strands the other path's rows.
...
## 113. Dialogue Phase 4 — the live performance layer, the Faction↔Dialogue seam, and the prose-consumer re-prompt (2026-07-09) [PROVISIONAL]

- **The performer is an awaitable sibling over the deterministic reply, NOT a rewrite (mirrors §107 `narrate_action_live`).** `DialoguePerformer.perform_reply(plan, templates, slots)` is the sync Tier-0 render; `perform_reply_live(...)` is the coroutine sibling. The UNCONFIGURED path executes **zero awaits** — `DialogueSession.perform_reply_live()` branches on `LLMManager.is_configured()` and returns `DialoguePerformer.fallback_result(...)` same-frame (byte-identical to the Tier-0 line `submit_move` already produced). Only the configured branch enters the coroutine + awaits ONE `generate()`. Tests drive the unconfigured path via `Callable(session, "perform_reply_live").call()` (the §102 analyzer trick). The conversation NEVER blocks: `submit_move` is fully deterministic and instant; the live upgrade swaps the transcript line in place if/when it resolves.
- **`submit_move` stays synchronous; `_capture_reply` is the single funnel.** Every reply-producing return in `submit_move` is wrapped `return _capture_reply(<handler>(...), move_id, free_text)` — it records the running `_transcript` (player turn + npc line) and stashes `_last_reply_plan`/`_last_reply_slots` so `perform_reply_live()` can upgrade the just-shown line without re-adjudicating. `_transcript_for_prompt()` slices out the CURRENT exchange (its move+free-text are passed to the prompt separately) to avoid duplication.
- **`PromptAssembler` gained a `===USER===` template split (GDD §10.1 point 2), backward-compatible.** A template with the delimiter renders the pre-part into the system prompt and the post-part into the user message; templates WITHOUT it behave exactly as before (`_build_user_message` synthesizes the user turn). `_enforce_budget` re-renders BOTH sections when trimming a truncatable key (a key like `transcript_tail`/`memory_lines` can live in either). Dialogue is the first user-section template; all pre-P4 consumers are untouched.
- **Output is PLAIN TEXT + optional trailing `#mood:`/`#social_flag:` tag lines — never JSON (only the summary call is JSON).** `DialogueReplyValidator.parse_tags` splits the spoken body from the tags (the performer calls it on the envelope text); `make_validator(must_not_reveal, speaker)` is the `opts.validator` consumer screen (must_not_reveal strings, first-person-as-player, demeanor-beat editorializing). The layer's `validate_prose` (length cap + meta-leakage) runs first; the consumer screen second.
- **The prose consumer re-prompt (§13.4) now exists in `generate()`.** Previously only JSON parse failures re-prompted; P4 added `LLMManager._prose_consumer_reprompt` (+ `LlmResponseValidator.build_prose_reprompt`): on a prose consumer-validator rejection with `retries_left > 0`, ONE re-prompt names the violation, re-executes, re-validates, then fails to the consumer's template. A content rejection consumes a retry slot but is NOT a transport failure (no circuit-breaker decrement) — mirrors the §11.2 JSON re-prompt. Prose consumers WITHOUT a validator (Seam A) never reach this branch.
- **The offense/enticement `#social_flag` seam is validate-before-apply (mirror Seam-B, do NOT relax).** `SocialFlagValidator.validate(flag, ctx)` gates kind∈{offense,enticement} + non-empty grounds consistent with axes/relationship/move + attitude headroom + dedupe (`already_fired`) + severity cap (1 step; 2 ONLY when a deterministic §6.6 trigger also fired). It returns a SIGNED `tone_steps`; the ENGINE (`DialogueSession._apply_social_flag` via `Attitude.shift_tier`) applies it — the LLM NEVER writes a relationship score. Session-scoped `_social_flags_fired[kind]` maps the `npc_issues.offense_fired` dedupe onto flags not bound to a request issue.
- **The demeanor beat is engine-rolled/seeded/logged (already, §112); P4 only instructs the model to WEAVE it unlabeled.** The prompt carries the cue as a stage direction; `_BEAT_EDITORIALIZING` in the validator + the layer meta-leakage screen catch a labeled tell ("…, lying"). Templates render the cue verbatim, so mock and live are identical in substance.
- **`InterjectionSelector` (§13.6) is deterministic, cadence-capped (~1/4 exchanges), off-switched in `LlmSettings.dialogue_interjections_enabled`.** It runs inside `_reply_ctx_for(outcome)` (it needs the outcome — a blunt henchman fires on a FAILED diplomacy), fills the plan's `interjection` slot, and the performer renders the aside. A henchman = a present member with `npc_role != 'player'`; expressiveness gates the base weight, the outcome selects the loud relevant axis (civility/compassion/stress on trouble; jocularity/curiosity/self_interest otherwise).
- **The Faction↔Dialogue seam is PUBLIC-only by construction (§7.4 discovery-only).** `DialogueFactionContext.build(npc, party, day)` reads ONLY non-secret memberships (drops `is_secret=1`), `get_stance()` PUBLIC projections (band WORDS, never `grievance_score` numbers), and public faction fields — so `true_stance`/plot rows CANNOT enter any prompt payload (a grep-proof test asserts the string never appears across the block, the reply context, and the assembled prompt). The LLM never DECIDES a disclosure: when the engine decides a leak, `inject_reveal_directive(fc, fact_text)` adds the exact fact (passed as a literal, never read from `true_stance`) and the performer instructs the model to disclose that one fact. Prompt-side "know-but-don't-tell" secrecy is prohibited.
- **Summarization at close is one JSON call that rewrites PROSE ONLY (§8.2/§104).** `close_live()` runs the deterministic `close()` (facts + template summary always written) and, when configured, passes an LLM-rewritten `summary_override` into `summarize_move_log` — the engine-derived `facts` are untouched. Zero awaits unconfigured (guarded by `is_configured()`), template summary on any failure.
## 114. Faction FF-4 — the Allegiance Engine, feign/betrayal, covert ops, and divided loyalties (2026-07-09) [PROVISIONAL]

- **The feign predicate is `pref_side != seat_side`, NOT the §7.5 prose numbers.** `AllegianceEvaluator.evaluate` sums the §7.3 support stack for each side, picks `pref_side = argmax(support)`, resolves `seat_side` from the faction's `home_domain_id → domain.realm_id` vs each side's realm-mirror `realm_id`, and FEIGNS when the higher-support side is NOT the seat-holder (public = seat-holder, true = higher-support side) — exactly the §7.3 pseudocode. The §7.5 worked-example numbers ("Δ +1 for Orso") are internally inconsistent with that pseudocode and are treated as illustrative; the golden test asserts the documented OUTCOMES (guild feigns, Tulras declares openly, Grey Rats side openly), not the prose term magnitudes. Every term constant is PROJECT CALL (§7.2/§7.3) and tunable.
- **Three feign-adjacent decisions, one gate.** `open` / `lean` / `neutral` when there is no seat tension (pref == seat, or no seat); `feign` when pref != seat AND the gate passes; `open_defiant` (declare the true preference openly + flip `status='underground'`) when pref != seat AND the gate REJECTS. The gate (`feign_eligibility`): Fanatic-faithful temples/orders NEVER feign (checked first); else self_interest ≥ 6 OR type ∈ {syndicate, merchant_guild} OR survive-goal active. "Fanatic-faithful" = a temple/holy_order with a `spread_doctrine`/`defend_patron` goal or an explicit `context.fanatic_tie_side`.
- **Term-override + self-interest test seam.** `evaluate` reads each support term from `context.terms[<mirror_id>][<term>]` when present, and `context.leader_self_interest` when present, before computing from the DB. This keeps the golden test deterministic (assert decisions, not fragile DB-derived magnitudes) while production still computes real terms. `_term_or` evaluates the computed fallback eagerly (GDScript args) — harmless (a no-row DB read), do not "optimize" it into a lazy branch that changes determinism.
- **`true_stance` + `betrayal_condition` are written ONLY via `FactionStanceService.set_conflict_stance`** (authority-split guarded, audit-traced, emits `faction_stance_changed` only on a PUBLIC band change). Ordinary `get_stance` / `to_public_dict` never surface them (discovery-only, §7.4). A successful (uncaught) covert op attributes NO grievance — the target notices damage, not the culprit; attribution (`op_discovered` + a stance drop) happens ONLY on a caught op (fail-by-14+/nat-1 via the reused `HijinkThrowTarget.classify_outcome`).
- **Betrayal firing is event-driven, scanned from the stance rows.** `BetrayalResolver.check_and_fire(campaign_id, event_kind, event_data, day, dice)` reads `ff_list_stances_with_betrayal` (the `betrayal_condition IS NOT NULL AND != ''` filter), matches the parameterized enum (`side_loses_field_battle` / `siege_of_seat_begins` / `patron_payment_missed(n)` / `rival_org_declares_for` / `power_ratio_crosses(x)` / `evidence_of_persecution_plan`), and on a match runs `execute_betrayal`: stance swap (professed → hostile, mask cleared; true side → friendly) + one prepared free §6.7 op at **+4** + a `betrayal_executed` ledger row that NEVER expires (§4.5). The condition rides the stance toward the PROFESSED (seat) side, keyed by the professed realm id so cross-conflict fixtures never cross-fire.
- **Covert ops reduce to hijinks (RAW quarantine).** `CovertOps.run_op` classifies the caught outcome with `HijinkThrowTarget.classify_outcome`; non-thief perpetrators act as a **1st-level thief** (`perpetrator_effective_level` returns 1 unless type ∈ {syndicate, merchant_guild}, which use the boss level). Syndicate-for-hire (`quote_for_hire`) keys the rate to the hirer→syndicate stance band (Friendly ×0.75 / Neutral ×1.0 / Unfriendly ×1.75) and REFUSES a mutually-Hostile pair. Assassination is gated (`can_assassinate`): assassins/nightblades only, unsuspecting off-screen NPCs only, never a PC / never on-screen — checked BEFORE any throw.
- **Lighting up an FF-2 `ff4_stub` action = drop the stub flag + give it real relevance AND an availability gate — never let it fire unconditionally.** `undermine_rival` (a covert op) is gated on `_find_rival` (an instantiated stance ≤ unfriendly); `declare_stance` (the allegiance evaluator) is gated on `_active_conflict_for` (a launched rebellion whose rebel/liege realm is the org's seat realm). `_score_candidates` filters on `_action_available` BEFORE relevance, so an org with no rival / no conflict produces the identical candidate set it did as a stub — no FF-2 regression. A live conflict gets a ×2 situational boost so `declare_stance` dominates the turn when it matters.
- **§11.7 audit counters are in-memory, flag-gated, and NOT in the trace stream.** `PoliticalAudit.bump_counter` (no-op when `debug_political_audit` is off) feeds `counters_summary` / `faction_stats`; keeping them out of the JSONL means the determinism harness (replay → byte-identical `read_all()`) is unaffected by counter state. The determinism guarantee holds because every evaluator/resolver takes an explicit game `day` + the shared `dice` seam and never reads the wall-clock or un-seeded `randi()`.
- **The one FF-4 migration is `party_loyalty_conflicts` (198), a dedup ledger — feign/betrayal reuse existing columns.** `DividedLoyaltyDetector.detect` persists a detected conflict keyed by a deterministic `signature` (`cause | sorted faction pair | conflict_ref`) so a monthly re-scan reports-but-does-not-re-emit; conflicts surface as CONTENT (loyalty demands / conflicting job offers), never a UI modal. Register any such new campaign-scoped leaf table in BOTH `_SCOPE_DIRECT_CAMPAIGN` and the purge cascade.
## 115. Dungeon Faction Generation — deterministic, dungeon-scoped, input-graph-driven (Wave 3 Track D, 2026-07-09) [PROVISIONAL]
...
## 116. Shared micro-helpers — null-safe coercion, realm resolvers, band-index, alignment axis (2026-07-10)

Wave-3 review finding #15 (reuse): several byte-identical or drift-divergent private helpers were duplicated across the faction / dialogue / dungeon-faction-generation subsystems. They are now consolidated. When you need one of these, call the shared form; do NOT reintroduce a private `_s` / `_band_index` / `_align_rank`.

- **`StringUtils.s(v: Variant, default_value := "") -> String`** (`engine/shared_types/string_utils.gd`) — the ONE null-safe Variant->String coercion for SQL-NULL columns. Canonical body: `str(v) if v != null else default_value`. Use `str()`, never `String()` (§106: `String(null)` is an invalid constructor). Replaced 18 private `_s` copies (12 faction + 6 dialogue files, 135 call sites) whose bodies had drifted (`str` vs `String`, `default` vs `default_value`, ternary vs if-branch, one single-arg, one non-static). NOTE: the `_s(data, key, default)` DICTIONARY-ACCESSOR in the `shared_types/*_data.gd` classes (FactionData, FactionStanceData, DungeonFaction, …) is a DIFFERENT helper (get-string-field-from-dict) and is intentionally left in place.
- **`FactionRegistry.realm_id_of_mirror(mirror_id)` / `.seat_realm_of_faction(faction)`** — the realm a realm-mirror faction backs (its `realm_id`), and the realm containing a faction's seat (`home_domain_id` -> domain -> `realm_id`). FactionRegistry owns realm-mirror concepts; both are pure static, side-effecting only through CampaignRepository. Replaced 5 copies (`_mirror_realm` / `_side_realm_id` / `_seat_realm` / `_faction_seat_realm`) across betrayal_resolver, faction_ai, allegiance_evaluator.
- **`FactionStanceData.band_index_or_neutral(band) -> int`** — faction-stance band index with a neutral(2) fallback for an unknown band. DISTINCT from the sibling `band_index()` (which returns -1 on miss). `FactionStanceService._band_index` and `DividedLoyaltyDetector` both route here. The REALM-band ladder (`_realm_band_index` / `REALM_BANDS` in treaty_resolver + realm_relations_drift) and the income-band helper (`_income_band_index` in domain_morale_resolver) are DIFFERENT vocabularies — leave them alone.
- **`AlignmentAxis.chaos_rank(alignment) -> int`** (`engine/shared_types/alignment_axis.gd`) — the Law(0) / Neutral(1) / Chaos(2) ordinal. Any string that is not `"lawful"`/`"chaotic"` ranks 1. Used both for alignment DISTANCE (`abs(rank(a) - rank(b))`, DungeonFaction relationship generation) and "how chaotic" comparisons (AllegianceEvaluator §7.3). Replaced `_align_rank` + `_chaos_rank` (byte-identical outputs). Determinism-safe: the dungeon-faction golden test stays byte-identical.
- **Seeded dice parsing stays LOCAL — do NOT route generation dice to the DiceSystem autoload.** `FactionWandering.roll_dice(expr, rng)` was flagged as duplicating DiceSystem's NdM+K parsing but is deliberately NOT merged: DiceSystem rolls on the GLOBAL RNG and has side effects (writes `dice_rolls`, emits signals) suited to player/GM rolls, and its strict regex parser rejects the permissive inputs generation passes (bare/empty count, malformed fallbacks). Deterministic generators keep a local, seeded, side-effect-free NdM(+K) parser — the same pattern as commerce `_roll_dice_spec` (x3, with a standing "if a fourth subsystem needs it" note), `treasure_resolver._roll_dice_expr`, and `disease_resolver._parse_and_roll_dice`. If a shared seeded-dice utility is ever wanted, extract a PURE parser taking an explicit `rng` that preserves each caller's exact edge cases; never call the stateful autoload from seeded generation.

## 117. Generated-content versioning — regenerate, never migrate (DG-C3D.A, 2026-07-10)

Generated dungeons carry a **generator version stamp**; stored content whose stamp does not match the current generator is **discarded and lazily regenerated on next access** — stored generated content is never row-migrated to a new shape (`gdd-dungeon-contiguous-3d.md` §13, Jedidiah ruling 2026-07-06). The pattern, for the dungeon generator and for any future generated-content system that adopts it:

- **One constant owns the version:** `DungeonGeneratorV1.GENERATOR_VERSION` (0 = the pre-contiguous floor-stitched generator; DG-C3D.F bumps it to 1). Bumping the constant is the ONLY step needed to invalidate all stored dungeons.
- **Stamp on every persisted copy:** the voxel JSON payload gets a top-level `"generator_version"` key (injected by `DungeonFixtureService` when it serializes `result.composed_volume.to_dict()` — `VoxelMapData.to_dict()` does NOT emit it, so forgetting the stamp makes every entry regenerate; the DG-C3D.F.3 deletion removed the old `DungeonVoxelSerializer.to_voxel_dict` stamp site) and `dungeon_floors.generator_version` is stamped at insert (migration 210). A payload persisted before the stamp existed reads as version 0 via the missing-key default — deliberately equal to the current constant, so pre-stamp content never regenerates spuriously.
- **Recover the shape from the relational store, not the payload:** the persisted `dungeon_floors` rows ARE the recovery source — `DungeonGeneratorRepository.derive_request_spec(dungeon_id)` reconstructs `{kind, size, floors, entrance_floor_index, tier}` (tier = the entrance floor's stored `floor_tier`, which equals the original `entrance_tier` because tier derivation anchors on the entrance floor). Do NOT embed the generation `spec` in the voxel payload to carry it forward: every generated dungeon persists its `dungeon_floors` rows (`generate(persist=true)`), so derive always works, and adding `spec` to the payload would change what the payload exposes to consumers today (e.g. the Get-Hex-Info dev tool renders extra rows off a `"spec"` key) — a zero-behavior-change violation.
- **Check at the lazy-generation seam only:** `DungeonFixtureService.get_or_generate_voxel` is the single gate (its two production callers are the hex-map renderers' dungeon-entry paths). On mismatch it (1) recovers the spec via `derive_request_spec`, (2) regenerates — `generate(persist=true)` replaces the relational rows wholesale (`insert_dungeon_layout` deletes-then-inserts), then (3) purges stale runtime voxel cells (`CampaignRepository.delete_voxel_cells_for_map`) and persists the new payload. Never check versions downstream of the seam.
- **Destroy stored state only after the regeneration succeeds.** Do NOT delete the `dungeon_floors` rows before regenerating — they are the recovery source, so a transient generate failure between the delete and a successful insert would strand the dungeon at the default shape on the retry. Leave them in place (the persist step replaces them wholesale on success) and purge only the runtime voxel cells, only once the new geometry is in hand. Regeneration is deterministic (seed derives from the entrance id), so a same-version regenerate reproduces the payload byte-identically.
- **F-bump caveat (open):** hand-authored dungeon payloads (test content) have `"cells"` but no generator provenance; a version bump must exempt them or they get replaced by generated dungeons. Owned by DG-C3D.F.

## 118. Namespaced RNG streams for new generator pipeline stages (DG-C3D.B, 2026-07-10)

A new stage added to an existing seeded generator must NOT draw from any existing stream — interleaving new draws shifts every subsequent value and silently changes all existing output for the same seed. Instead, each stage gets its own **namespaced stream derived off the master seed** with a distinct prime offset, following the `dungeon_generator_v1.gd` idiom (per-floor layout `master_seed + floor_index × 1000003`, stocking `+ (fi+1) × 7919 + 999983 × (attempt−1)`, key/lever `+ 104729 + bump`).

- **One derivation point.** The stage class owns a `derive_rng(master_seed) -> RandomNumberGenerator` static plus a documented `STREAM_OFFSET` const (instances: `VerticalPlan.STREAM_OFFSET := 15485863`; `DungeonStocker.BALCONY_STREAM_OFFSET := 32452843` for the DG-C3D.F.2d balcony-zone stocking pass, retry-bumped like the main stocking stream — both chosen to be unreachable by every existing derivation incl. retry bumps). Callers and tests both obtain the rng ONLY from there; never construct the stream inline.
- **Zero draws when the stage is inert.** A request the stage doesn't apply to (e.g. a single-band dungeon for the vertical plan) must consume ZERO values from the stream — early-return before any draw, and guard it with a test that compares `rng.state` before/after. This is what makes the DG-C3D.F byte-identity gate ("a single-band dungeon generates byte-identically before and after the rework") hold by construction.
- **Documented draw order.** The stage's doc comment states the exact draw sequence (e.g. "per adjacent band pair ascending: type roll, then placement rolls; then dungeon-level atrium rolls"); any rng-consuming loop must iterate a deterministic sequence (never bare Dictionary iteration order — Godot preserves insertion order, but derive loop order from explicit arrays/ranges anyway).
- Determinism harness discipline (same seed → identical output, different seed → different) per §80 applies to each stream independently.

---

## 119. Contiguous-3D vertical composition invariants (DG-C3D.D, 2026-07-11)

`dungeon_volume_composer.gd` stamps the per-band 2D layouts + the `VerticalPlan` into ONE `VoxelMapData`. The geometry rules below are load-bearing — the floor-integrity pass (§8 C5 of `gdd-dungeon-contiguous-3d.md`) hard-aborts a compose that breaks them, and DG-C3D.E asserts them on the movement graph. Follow them whenever the composer or any later stage carves cells.

- **Band → voxel levels.** A band occupies exactly two levels: its walk level (`VerticalPlan.band_for_floor(fi).walk_level`, the §5.1 formula) and `walk + 1` (headroom). Adjacent bands are always 2 levels apart; the physically-upper band's walk = lower band's walk + 2, so the lower band's headroom (`WL+1`) sits directly under the upper band's floor slab (`WU`). Every grid cell of every band is stamped (open→air/floor, wall/rock→solid at BOTH levels so walls are a full 10' tall); uncarved subterranean space is solid, and the topmost band is capped with a solid slab at `max_walk + 2`. Never leave a band cell absent — absent = the air/open/none sentinel, which the floor-integrity pass reads as an undeclared hole.
- **Connector geometry (§6).** A 10' rise is two stepped cells living in the LOWER band: the lower step at `WL` (`stairs_up_<dir>` / `ramp_<dir>`), the upper step at `WL+1`, with SOLID under-fill beneath the upper step and beneath the top landing. The shaft opening is the cell directly ABOVE the top step at `WU` (`floor_type "none"`, registered) — NOT at the landing. Straight/ramp runs make the whole upper footprint an open shaft except the single top landing (no isolated walkable pocket, and a flyer can rise the well). Switchbacks fold two steps around a mid-landing at `WL+1`. Spirals put `stairs_spiral`+floor-stone at each WALK level and a floor opening (`none`) at the intervening `WL+1`; the spiral top keeps its floor (the §10.5 movement clause phases through in-column, so it registers NO walk-level opening). Ramps reuse the straight carver with `ramp_<dir>`+`dirt`. Every stepped cell's direction suffix is the compass of the ascent delta (mirror `MovementResolver._direction_suffix_for_delta`). **No door cell ever lands inside a run; every run is symmetric (both-directions traversable) by construction.**
- **Registered openings + the floor-integrity pass.** Maintain a `Dictionary[Vector3i → true]` of every upper-band walk cell that is deliberately floor-less (stair/atrium shafts, atrium voids). The pass, per adjacent band pair: for each `WU` air cell whose `WL+1` neighbour below is air (open lower-band space), it must have a floor UNLESS its key is in the openings set; every registered opening must have NO floor. A violation returns a non-empty reason string and `compose()` aborts (`ComposeResult.ok = false`). Because C1 stamps every cell as air+floor-stone or solid, the ONLY way to make a hole is to carve a void — so registering every void you carve makes the pass pass by construction.
- **Zones are per (room, WALK-level) flood-fill.** Zone assignment only ever runs at a band's walk level — stair steps and mid-landings at `WL+1` are intentionally NOT zoned, which is what keeps band honesty (§10.2 check 4: every zoned cell sits at `walk_level(band)`). Disconnected same-band regions of one room become distinct `zone_index` values (4-neighbour flood-fill). Emit a room's HOME-band zones (`band == room.band`) before off-home zones so the main floor always takes `zone_index 0` and atrium balconies follow.
- **Per-band global room ids.** Layout room ids are per-band-local; the composed volume needs dungeon-unique ids, so remap `global = band_slot * 1000 + local_id` (band_slot = index in `VerticalPlan.bands`). A connector's two circulation rooms (one per band) keep distinct global ids; `StairwellData.room_id` records the lower (owning) one, and the upper landing is tagged with the upper room so its zone's band + walk stay honest. An atrium is one room (the base-band room's id) whose balcony zones live on the upper band.
- **Composition draws NO RNG.** Unlike the vertical plan (§118), the composer is fully deterministic from `(plan, band_layouts)` — every landing/orientation/void choice is derived from the footprint + type. No new namespaced stream is introduced; if a future pass needs randomness it derives its own per §118.

---

## 120. One ground-step legality predicate — MovementRules (DG-C3D.E, 2026-07-11)

`engine/shared_types/movement_rules.gd` (`MovementRules`) is the SINGLE home for ground-walker step legality on a `VoxelMapData` (contiguous GDD §4 "one movement predicate"). `MovementResolver` (combat + exploration pathing), the composed-volume navigability validator, and the composed key/lever placer all consume it — the validator and placer contain NO independent stair/support logic (grep-verifiable).

- **What it owns:** `has_support` (delegates to the single support source `FallingResolver.has_support`), `direction_suffix` / `reverse_direction`, `connects_via_feature` (a ±1-level transition — diagonal-vertical via a `stairs_up_/down_<suffix>` or `ramp_<suffix>` whose compass matches the horizontal delta, OR pure-vertical same-column via a `ladder` or `stairs_spiral`), `level_transition_legal` (0 flat / 1 needs a feature / ≥2 blocked), and `is_ground_step_open` (destination solidity air + supported + legal level transition).
- **Door LOCK policy is NOT in MovementRules** — it differs per consumer (combat strict/explore; validator structural-all-passable vs solvability-with-keys). A door cell is `solidity: air`, so it passes `is_ground_step_open`; each caller layers its own door rule on top. This is the clean seam that keeps the predicate reusable.
- **Spiral clause (voxel GDD §10.5):** a `stairs_spiral` cell is both (a) SUPPORT — `FallingResolver.has_support` treats it like a ladder, so the open intervening gap cell (`floor_type none`) holds a climber — and (b) an in-column ±1-level connector. Combat inherits both because `MovementResolver._has_stair_connection` now delegates to `MovementRules.connects_via_feature`. Adding the clause is purely additive: no pre-existing map has spiral cells, so combat/movement suites are unaffected.
- **Composed reachability (`DungeonNavigabilityValidator.reach_composed`):** BFS over the 26-neighborhood (`VoxelGrid.get_neighbors_3d`) filtered by `is_ground_step_open`, seeded from the dungeon ENTRANCE only (never per-stair — the DG-V1 masking lesson). Falls are never edges (every stair cell is supported, so edges are symmetric; `assert_edge_symmetry` checks it). `"structural"` = all doors passable (door cells are open air, no extra check); `"solvability"` = a door blocks per its initial state unless its key/lever is already reachable, with a fixpoint that re-seeds from the full reachable set while it grows, capped at `doors + 2` passes.
- **Composed key/lever placement (`DungeonKeyLeverPlacer.place_composed`):** one discovery-order fixpoint over ZONES using the same predicate. A gated door's key/lever lands in a fully-discovered zone EXCLUDING the entrance zone and every circulation-room zone — so keys are reachable before their doors by construction (no circular dependency). Repair ladder (§10.3): §10.4 sole-path downgrade when no candidate zone exists; clear a frontier secret+unlocked door that blocks unreached content; unreached content with no frontier secret is a rule-3 STRUCTURAL defect → `solved: false` (never geometric-repair from the key layer — the re-seed ladder owns it). Mutates both the door cell and its record. Gate blast-radius `[GATE]` telemetry: a key gating > a tunable fraction (0.40) of stockable zones logs a warning.
- **The composed door record.** `VoxelCell` carries `door_state`/`door_type`/`door_detected` but NOT `door_material` or the dungeon-unique connected rooms, so `DungeonVolumeComposer.ComposeResult.doors` (emitted in `_stamp_band`) carries `{cell: Vector3i, type, is_secret, material, band, connects}` — the material (bashable vs key-needing) and global room ids the validator/placer reason over.

## 121. Composed dungeon runtime/persistence conventions — id bridge, z convention, version-1 payload (DG-C3D.F.2c, 2026-07-12)

The DG-C3D.F.2c cutover made `DungeonGeneratorV1.generate()` produce the composed contiguous volume. Three cross-cutting conventions every future dungeon consumer must follow:

- **Two room-id spaces, one bridge.** The COMPOSED world (`VoxelCell.room_id`, `RoomZone.room_id`, `StairwellData.room_id`) speaks dungeon-GLOBAL room ids: `global = band_slot × DungeonVolumeComposer.ROOM_ID_STRIDE + local`, where `band_slot = floor_index − 1` (bands are planned ascending, guaranteed by `VerticalPlan.build`). The RELATIONAL per-floor tables (`dungeon_rooms.room_id_in_floor`, `monster_groups.room_id`, `treasure_hoards.room_id`, `key_items.placed_in_room_id`) speak per-band-LOCAL ids scoped by `floor_id`. Convert with the formula; never store a global id in a per-floor table or vice versa — with ONE flagged exception (DG-C3D.F.2d): a monster group / treasure hoard whose `zone_index >= 1` (balcony/gallery content) attaches to the ZONE's band layout, where the owning room has no local id, so its `room_id` is the GLOBAL id; `zone_index >= 1` is the discriminator (subterranean globals are always >= 1000, so they cannot collide with that floor's locals). Zones link to their contents by STRING `monster_group_id` / `treasure_hoard_id` — and the repository persists a group's/hoard's own generation-time id as its row PK so those links stay resolvable after reload. **CAVEAT (open, owned by the first runtime zone consumer — F.2d/G):** `VoxelMapData.from_dict` ends with `detect_rooms()`, which REASSIGNS every loaded cell's `room_id` to fresh per-level flood-fill ids — so a volume loaded from the persisted payload does NOT carry the global ids; only the in-memory generation-time volume and the `room_zones`/`stairwells` rows do. Any runtime consumer joining `room_zones.room_id` to a LOADED volume must resolve zones by their persisted `cells_json` + band walk level (positions survive the round-trip; room ids do not) or fix the loader to preserve serialized ids first.
- **Composed voxel z = the band's walk level** (`VerticalPlan.walk_level`: entrance band 0, subterranean bands negative, above-ground positive; always even — odd levels are stair mid-levels/headroom). Treasure hoards persist `cell_z` at the walk level (remapped from the stocker's legacy `level_number − 1` in `_generate_attempt`). The ONLY z→floor bridge is `DungeonGeneratorRepository.get_floor_id_for_voxel_level` (`floor_index = efi − dir·z/2`); it returns `""` for odd z and for dungeons without stored floors — callers fall back to the player-drop cache-only path.
- **Version-1 payload + hand-authored exemption.** The persisted dungeon payload is `result.composed_volume.to_dict()` with `"generator_version"` injected by `DungeonFixtureService` (VoxelMapData.to_dict does NOT emit it; forgetting the stamp makes every entry regenerate). A version-mismatched payload whose dungeon has NO `dungeon_floors` rows is HAND-AUTHORED content (generated dungeons always persist rows) and is returned unchanged — this closes §117's F-bump caveat. Bumping `GENERATOR_VERSION` (e.g. F.2d's balcony stocking) invalidates all stored generated dungeons and nothing else.

## 122. Composed dungeon is the only path — legacy 2D reachability/teleport removed (DG-C3D.F.3, 2026-07-13)

The DG-C3D.F.3 cutover deletion removed the legacy floor-stitched dungeon path wholesale. There is now ONE dungeon pipeline (the composed contiguous-3D volume) and ONE reachability/placement surface. Do NOT resurrect the deleted code or reintroduce a parallel 2D path.

- **Deleted (gone for good):** `DungeonVoxelSerializer` (whole file — composition owns voxel emission via `composed_volume.to_dict()`); `VoxelCell.stair_target_col/row/level` + `DungeonMapController.get_stair_target` + `teleport_party_to` + the Ascend/Descend context-menu entries and their `dungeon_explore_state` handler (composed stairs are ordinary walkable geometry — the party walks carved runs via "Move Here"; only Exit Dungeon survives on transition cells); the layout composer's cross-floor **stair-anchor** path (`DungeonLayoutRequest.required_stair_positions`, `DungeonRoomComposer._validate_stair_anchors` / `_pre_place_anchor_rooms` / `_ANCHOR_ANTECHAMBER_SIZE`, `RoomPlan.is_anchor_room`); `DungeonKeyLeverPlacer.place()` + its ~14 legacy 2D-BFS helpers; `DungeonNavigabilityValidator.validate_solvability()` + `_expand_stair` / `_can_traverse_solvability` / `_key_for_door_is_reachable_with_floors` / the 2D `_cell_key`.
- **KEPT because the composed path reuses them (do NOT delete):** `DungeonStairData` / `dungeon_stair_data.gd` / `layout.stairs` / `DungeonLayoutRasterizer.build_stair_data` / the repository's stair serialization — the ENTRANCE up-stair is still produced by `_plan_stairs(stairs_up=1)` on the entrance band, becomes `layout.entrance` → `composed_volume.entry_pos`, and is fingerprinted by the byte-identity golden (`content_fingerprint` hashes both the `DungeonStairData` record AND `FEATURE_STAIRS_UP` terrain). Removing it would break the F regression gate; the build-plan "delete `dungeon_stair_data.gd`" line is superseded by its own "audit importers first" caveat. `DungeonRoomComposer._ANCHOR_INTERIOR_MARGIN = 2` stays (byte-identity load-bearing — it clamps the up-stair cell). `validate_layout` + `_neighbours` + `_enqueue` + `fallback_seed_cell` stay (per-band pre-compose guard). `finalize_key_placements()` + `_append_key_to_monster_group` + `_demote_ungated_trap_rooms` stay — `finalize_key_placements` is the defensive room-based fallback `finalize_key_placements_composed` invokes when a zone record is unexpectedly absent, and the two helpers are shared by the composed methods.
- **No formal action-vocabulary registry for ascend/descend existed** — the context-menu `_option()` call WAS the registration, so removing the menu entries fully deregisters them (CLAUDE.md step 11 is a no-op here; noted so a future reader does not hunt for a registry that never existed).
- **Byte-identity held across the deletion:** every deletion either draws zero RNG (empty-array loops, pure teleport/UI surface) or is off the generation path (retired serializer, legacy validator/placer never called by `generate()`); `test_dungeon_cutover_identity.gd` stays green.

---

## 123. Composed dungeon runtime polish — atrium connectivity guarantee, UI stairwell labels, placeholder meshes, fall-audit telemetry (DG-C3D.G, 2026-07-13)

- **Atrium balcony connectivity is a GUARANTEE, not a coin flip (§7.2).** `DungeonVolumeComposer._carve_atrium` keeps a balcony ring when a door reaches it OR an internal grand stair can be carved; it degrades to a plain double-height hall ONLY when the geometry genuinely cannot fit a stair (never for the §7.3 ≥5×5 gate). Two rules make this hold, both composer-only (draws NO RNG — §119): (1) `_footprint_has_door` is PERIMETER/ring-aware — the corridor router places the ring door ONE cell OUTSIDE `atrium.footprint`, so it counts a door whose cell or a 4-neighbour is a ring cell (`_is_ring_cell`), not an inside-the-rect test; (2) `carve_stair = _internal_stair_fits(rect) and (atrium.internal_stair or not has_upper_door)` — the flavor stair (35% plan roll) OR the guarantee when no door reaches the ring; `keep_ring = has_upper_door or carve_stair`. This preserves variety (door-reached balconies keep the GDD's preferred path (a)) while making degradation the true last resort. The change shifts multi-band ATRIUM composed goldens (regenerate/re-capture on purpose) but NOT single-band byte-identity (no atriums) nor no-atrium multi-band goldens (seed 11) — `_carve_atrium` is only reached for atrium dungeons. Do NOT tune `INTERNAL_STAIR_CHANCE` for reliability — it is a flavor lever; reliability comes from the guarantee. The deferred enhancement (a GUARANTEED layout-time door onto the `atrium_upper` blocked-region stub — MST/anchor-net change in `dungeon_room_composer.gd`) would raise cloister-only variety but is out of scope (risky, shifts all layouts). Sweep after any change: `tests/tools/dungeon_stress_sweep.tscn` reports kept-vs-degraded (DG-C3D.G: 32/32 kept, 0 degraded — up from 4/28).

- **StairwellData carries the UI label logic (minimap glyphs + hover tooltips + main-view stair tooltip).** The shared helpers live on `StairwellData` (testable, DRY): `covers_cell(cell)` (matches `bottom_cell`/`top_cell`/`run_cells` by POSITION — never `room_id`, which a loaded volume loses per §121), `label_at(viewer_z)` ("Stairs down to Level N" when `viewer_z == top_cell.z`, else "…up to Level N"; type prefix Stairs/Switchback stair/Spiral stair/Ramp; `is_entrance` → "Dungeon entrance"), and the statics `label_for_cell(stairwells, cell)` / `generic_label_for_feature(feature)` / `is_connector_feature(feature)`. **StairwellData is NOT in the voxel payload** — the runtime UI fetches it via `DungeonGeneratorRepository.get_stairwells(dungeon_id)` (`DungeonExploreState._create_ui_panels` for the minimap; lazy on first hover in `dungeon_map_renderer_3d.gd` for the main view). **Hand-authored dungeons persist ZERO stairwell rows** — the minimap still draws glyphs off `is_connector_feature(cell.feature)` and tooltips fall back to `generic_label_for_feature`, so both degrade gracefully. `dungeon_minimap._color_for_cell` now uses `is_connector_feature` (the old `stairs_up/down` check missed spiral + ramp).

- **Placeholder feature meshes (steps / spiral / ramp / parapet) live in `TacticalGrid3D.build_features_voxel`.** Simple boxes in the existing per-level `Features` Node3D — so the tint/fade/fog machinery already covers them; EVERY `MeshInstance3D` sets a `"base_color"` meta (load-bearing for `set_level_group_tint`'s below-focus dimming). Spiral is checked BEFORE the generic `stairs_` branch (its feature has no compass suffix). Ramp (`ramp_*`) and parapet (`cover_value > 0` on a floored cell, rail on each `air_open`-facing 2D neighbour) previously rendered NOTHING — `test_dg_c3d_g_ui.gd` guards that. These are stand-ins flagged to `gdd-dungeon-asset-integration-plan.md` §2.2; real Quaternius meshes are TODO. `reveal_all_fog` does NOT gate the dungeon renderer (that setting is wilderness-only) — per-cell `fog_state` + `_apply_level_visibility` do, so in-engine verification must reveal the dungeon's OWN fog.

- **Fall-audit telemetry is a soft one-shot in `generate()`.** `DungeonNavigabilityValidator.fall_audit(volume, zones, band_walk)` (in `navigability_validator.gd`, class `DungeonNavigabilityValidator`) returns `{per_band: {floor_index → count of walkable zoned cells adjacent to a ≥10' drop}}`. Wired into `_generate_attempt` once on the ACCEPTED state, right after the `[GATE]` block, as `[FALL] per-band …` appended to `attempt_warnings` (rides the existing `attempt_warnings → result.warnings` path). Draws NO RNG; NEVER a gate (`result.success` stays governed by hard_pass + solvability + placement); suppressed when there are no drop edges (flat single-band dungeons stay warning-free, so single-band byte-identity is untouched — `content_fingerprint` hashes `result.floors`, never `result.warnings`). Bracketed-prefix convention alongside `[GATE]`/`[BALANCE]`.

## 124. Terrain-driven wilderness battle maps — natural slopes, water depth, obstacle catalog, reachability guarantee (2026-07-17) [PROVISIONAL]

The wilderness battle map generator (`engine/subsystems/generation/battlemap/`, gdd-combat-map-generation.md v2) replaced the flat `generate_open_field` placeholder. Conventions every future consumer (combat AI pathfinding, LOS/cover, size system, asset pass) must follow:

- **Map size is 70×70 (350'), the 30%-shrunk battlemap.** `BattleMapGenerator.DEFAULT_SIZE`. Encounter-distance overflow edge-caps (gdd §7.3); do not grow the map to fit plains rolls.
- **`VoxelMapData.natural_slopes` is the outdoor-terrain regime flag** (serialized). It gates (a) the natural-slope clause inside `MovementRules.connects_via_feature` — any diagonal ±1-level ground step is legal, no stair feature; pure-vertical still needs ladder/spiral; ≥2 stays blocked (climbing) — and (b) surface-tracking behavior in `MovementResolver`'s 2D-signature wrappers, which resolve (col,row) references via `VoxelMapData.surface_level_at(col,row)` (topmost standable air, or the liquid surface; bridge decks win over the water beneath). Dungeons keep `false` and are byte-identical in behavior.
- **Water is two features + an integer depth.** `water_shallow` = air cell, `floor_type "water"`, `water_depth 0` (<1 voxel — wadeable by ANY walker). `water_deep` = `solidity "liquid"`, `water_depth >= 1` (swim required). The single wading gate is `MovementRules.can_wade(cell, wade_depth_allowance)`; the allowance is 0 for everyone until the creature-size session supplies per-creature values via `MovementResolver._wade_allowance_for` (the ONE plug-in point). `"swimming"` movement branch exists as a hook. `lava` is liquid, never wadeable, never swimmable; CONTACT (fall/teleport/force-back into a lava cell — voluntary movement can never enter) resolves via `FallingResolver.resolve_lava_contact(combatant, dice)`: save vs Poison & Death, fail = instant death, success = 2d6 fire (Jedidiah ruling 2026-07-17; `resolve_fall()` flags `lava_contact` landings, `walk_direction_3d` reports `blocked_cell`/`blocked_feature` so forced movement can detect the hazard — every new path that can drop a creature into lava must resolve it). Force back into lava REPLACES the RAW wall-collision knockdown, and the victim ends at the BRINK cell, never in the flow (occupying lava is illegal + a corpse in lava strands its loot). Overrun never moves the target (RAW), so no hazard branch there. `water_depth` is serialized on VoxelCell JSON but NOT yet in the `voxel_map_cells` DB table — a migration is owed when lair battle-map persistence lands.
- **`BattleMapObstacleCatalog` is the single source of truth for obstacles** — generator stamping (`apply_to_cell`), renderer placeholders (`TacticalGrid3D.build_obstacles_voxel` shapes/colors), and `docs/tactical-map-obstacle-key.md` (human key) all read it. Three classes: solid blockers (tree/boulder/outcrop/hedgerow), LOW solids (fence/low_wall/wall_ruined/rock_pile/fallen_log — solid for movement, LOS-TRANSPARENT via `VoxelCell.LOS_TRANSPARENT_SOLID_FEATURES`, cover via `cover_value` + `VoxelLOS.get_cover_value`), soft cover (brush/reeds/scrub — passable air, cover 1). Difficult-terrain COST is deferred to the AI-pathfinding session; the features are stamped now.
- **`zone_index` dual use:** dungeon maps = room-zone membership (§119); battle maps = walkable-component id from `BattleMapValidator.analyze` (largest = 0). "Can A ground-walk to B?" is two dictionary reads. `natural_slopes` disambiguates the regime.
- **Reachability is a generation-time GUARANTEE (gdd §7.5):** the walkable-surface graph uses `MovementRules.is_ground_step_open` (the ONE predicate — §120), so validated spawn zones never require climb throws, ≥10' falls, or deep water. Non-split maps: main component ≥85% of surface, repair by corridor-carving (`_force_walkable_column`: deep→ford, lava→crust, heights re-grade to 1-level steps). Split maps (divider-only: uncrossed river 35% of river maps / mountain cliff-chasm-lava divider 15% roll): exactly two major sides, each ≥25%, party and enemies placed across the divider deliberately — never stranded incidentally.
- **Generator result contract:** `{map, party_zone, is_split, divider, template_key, components}`; enemy placement is combat-time via statics `BattleMapGenerator.pick_enemy_anchor(map, party_anchor, desired_cells, is_split)` + `spawn_cells_near(map, anchor, count)` because it depends on the encounter-distance roll. `CombatState` seeds the generator with `hash(encounter_id)`; rich terrain context (`biome/elevation/biome_subtype/water/has_river/civilization`) rides `encounter_data` (stamped additively by `SessionRunner.spawn_encounter_data`).
- **Renderer regime split:** `combat_map_renderer_3d` renders terrain maps via `TacticalGrid3D.build_terrain_level_group` per level (textured floor batches per floor_type from `TERRAIN_FLOOR_TEXTURES`, soil-cube terrain columns, obstacle placeholders, water/lava planes whose material mirrors the hexmap river material) — the legacy single-Level_0 `build_level_group` path is untouched for flat maps. Camera bounds are cached once in `setup()` (`_compute_map_bounds`) — never rescan `get_all_positions()` per frame. Click-picking on terrain maps tests level planes top-down and accepts the column whose SURFACE is at that level (`_screen_to_surface_cell`).
- **Determinism:** one seeded RNG + seeded FastNoiseLite instances; same context ⇒ byte-identical `to_dict()`. Template selection (`BattleMapTemplates.select`) is a pure function — ALL chance rolls live in the generator.

---

## 125. Creature size & multi-cell footprints (2026-07-18)

The tactical/dungeon grid supports over-sized creatures that occupy several 5' cells. GDD: [gdd-voxel-tactical-architecture.md](../generation/gdd-voxel-tactical-architecture.md) §23.

- **`CreatureSize` is the single home for the size→footprint table.** `engine/shared_types/creature_size.gd` (static, RefCounted). The six ACKS categories (`small`, `man_sized`, `large`, `huge`, `gigantic`, `colossal`) come from `rules/le_monster_creation.xml` (SACRED); the footprint *scale* (1×1 / 2×1 / 1×2 / 2×2 / 3×4 / 6×9) is Jedidiah's design input, NOT RAW. Only the Large tier's footprint depends on orientation (`long`→2×1, `wide`→1×2). Unknown categories degrade to `Vector2i(1,1)` so bad/absent data is a single cell, never a crash. The RAW size→AC modifier is a monster-CREATION rule only — existing catalog ACs already bake it in, so it is deliberately NOT re-applied at runtime (Jedidiah ruling 2026-07-18); this table is footprint-only.

- **Footprints are LOCAL-frame `Vector2i(length, width)` and rotate with facing.** `length` = along facing, `width` = across. `CreatureFootprint.cells(anchor, facing, local)` (`engine/shared_types/creature_footprint.gd`) derives the occupied cells: anchor = rear-center (always occupied), body extends forward along a facing snapped to its dominant cardinal grid axis. A 90° turn flips 2×1 ⇄ 1×2 (Jedidiah ruling). `is_single_cell(local)` gates the fast path everywhere.

- **The single-anchor contract is preserved.** A creature still stores ONE `grid_position` / `entity_positions[id]` (Vector3i). Multi-cell occupancy is a DERIVED overlay in `VoxelMapData.entity_footprints: Dictionary[id → Array[Vector3i]]`. **Absent from that dict ⇒ occupies only the anchor cell** — so every single-cell creature (all PCs/henchmen + man-sized-or-smaller monsters) and all dungeon-exploration/party code is byte-identical to before. `get_entities_at` / `is_occupied` / `is_occupied_by_other` are footprint-aware; `remove_entity` clears the footprint.

- **`MovementResolver.sync_entity_footprint(combatant)` is the ONE writer of `entity_footprints`.** Called on every placement/move/turn (`set_grid_position_3d`, `move_along_path`, `set_facing_and_revalidate`). It recomputes cells from anchor+facing+size and pushes them; single-cell creatures clear their entry. Never write `entity_footprints` from anywhere else.

- **Passage gating falls out of "the whole footprint must fit."** `footprint_can_occupy(anchor, facing, local, mover_id)` requires every footprint cell passable + supported + free of other blockers (the mover's own cells are exempt via `mover_id`). Threaded into `path_bfs_3d` / `get_cells_reachable_3d` **only for multi-cell movers** (single-cell keeps the original single-cell BFS), and into combat spawn (`CombatState._try_place`). A 2×2 body cannot anchor where any of its four cells is a wall, so it can't route through a 5'-wide door/corridor — no special corridor-width code needed.

- **ZoC & adjacency border the whole body.** `_build_enemy_zoc_set_3d` emits the 8-neighbor ring around ALL footprint cells (minus occupied cells); `is_adjacent`/`get_adjacent_enemies` use min-Chebyshev between the two cell-sets. Both keep the exact single-cell path when both creatures are 1×1. Respects the swarm `no_zoc_emission` flag.

- **Token stretching lives in `CombatantToken3D`.** `set_creature_size(local, height_scale)` + `apply_grid_footprint(anchor)` scale the placeholder cylinder to the footprint's world span and center it over the cells; it re-straddles on move/turn. `CreatureSize.height_scale` drives the vertical stretch. Single-cell tokens are untouched. `combat_map_renderer_3d` only calls these when `combatant.is_multi_cell()`.

- **Catalog data + flags.** `data/monsters/monster_catalog.json` entries carry a `footprint` block (`{size_category, orientation, cells_local, source}`) beside the corrected `size_category`. 167 wired: ~155 confident audit rows (incl. the ten `dragon_*` age entries from the audit's Dragon Age tab) + the 12 elementals (2026-07-20). **Footprint MAY be decoupled from the size category** — elementals (tall-thin air, radial fire/water; sized from `le_monster_catalog_6.xml *_form_dimensions`) carry authored `cells_local` that doesn't match the size scale, marked `"size_decoupled": true`. That marker is the exemption key: `test_footprint_matches_size_category` skips the cells↔category check when it's set (the size_category-mirror check still runs). New radial square shapes 3×3 / 5×5 / 7×7 were added to the test's legal-shapes set; the runtime already renders/gates any rectangle from `cells_local` (no engine change). Still un-wired (in `docs/monster_size_flags.md`, engine treats as 1×1): oozes, un-audited herd animals, thin-body bme-caveat, and every "uncertain body form" row. **Swarms are NOT a footprint case at all** — they are a diffuse *enveloping area*, handled by the `swarm_area` block + `SwarmDriver`, NOT a solid `footprint` (see §126). Data-integrity pinned by `tests/test_monster_footprint_data.gd`; geometry/occupancy/gating by `tests/test_creature_footprint.gd`. Follow the §42 backfill discipline (throwaway Python helper, `json.dump(indent=2)`).


## 126. Monster swarm subsystem — diffuse enveloping area, not a footprint (2026-07-24)

Swarms (insect / rat / bat) are the "enveloping" special case flagged out of the creature-size work (§125). RAW: `rules/le_monster_catalog_2_summary.xml` (Swarm — Insect/Rat/Bat variants). A swarm is a mass of tiny creatures acting as one; "any character within the swarm is automatically hit; no attack roll is needed."

- **`swarm_area` vs `footprint` — the core distinction.** A swarm's catalog entry has a **`swarm_area`** block (`{cells_local: [length, width], source}`) and **NO `footprint`** block. The swarm therefore stays 1×1 for movement — it moves as a single-cell, non-blocking anchor and is NEVER routed through the multi-cell `footprint_can_occupy` gate — but ENVELOPS every enemy standing in the rectangle its area covers. `Combatant.get_swarm_area_local()` reads the block; `CreatureFootprint.area_cells(anchor, facing, area_local)` (a thin, deliberately-separately-named alias over `cells`) turns it into cells. Area = RAW 10'×30' (2×6), HD-scaled: 2 HD = 10'×20' (4×2), 3 HD = 10'×30' (6×2), 4 HD = 10'×40' (8×2). `get_footprint_local()` still returns 1×1 for swarms, so `is_multi_cell()` is false and every movement/BFS/ZoC path stays on the fast single-cell branch. The non-blocking behaviour is the existing `ignores_cell_occupancy` / `no_zoc_emission` / `no_zoc_obedience` catalog flags (§125-era, set by `Combatant._apply_monster_catalog_flags`).

- **`SwarmDriver` (`engine/subsystems/combat/swarm_driver.gd`) owns the runtime.** Constructed by `CombatController._init`; its cell-entry hook connects on combat start and disconnects on end (mirrors `SpellCombatHooks`). Per-swarm transient state (`{persistence: {target_id: rounds_outside}}`) is keyed by swarm id and dropped on `disconnect_signals`. It fires on the swarm's **initiative tick** — `CombatController._resolve_monster_action` routes any `is_swarm()` combatant to `_resolve_swarm_action`, which drifts the swarm toward its prey (when pursuit is allowed) then calls `apply_swarm_tick`. It also fires **on cell-entry** — `SwarmDriver._on_combatant_moved` (subscribed to `EventBus.combatant_moved`) marks anyone who walks into a swarm's area. (This is the monster-combatant analogue of the Insect-Plague *spell* tick in `SpellCombatHooks`, which is keyed off a `plague_profile` active_effect.)

- **The three flavours are one data-driven code path.** Each swarm flavour's effect is described on its `swarmed_<type>` condition (`data/conditions/condition_catalog.json`) via additive fields `swarm_save_type` / `swarm_save_ward_flee_bonus` / `swarm_on_fail_condition` / `swarm_on_fail_extra_condition` / `swarm_on_fail_transient` / `swarm_on_fail_damage_dice` / `swarm_disease_chance_percent`. `_apply_type_effect` reads them:
  - **insect** (`swarm_save_type: ""`) — flat 2-pt auto-hit tick, doubled vs AC ≤ 3 / unarmored, halved while warding/fleeing. **Reuses `SpellCombatHooks._tick_swarm_damage_for`** so the spell and monster paths deliver byte-identical damage.
  - **bat** (`save_spells` → `confused`, transient) — no damage; save vs Spells each round (+4 warding/fleeing) or Confused *for that round* (the swarm-sourced `confused` is cleared and re-rolled each tick so a later save ends it). The swarm also rolls **morale once each round** (`_roll_bat_morale`, via `MoraleResolver.roll_morale` directly, bypassing the casualty-trigger gate); **controlled bats are exempt**.
  - **rat** (`save_petrification` → `prone` + `swarm_pinned`, persistent, `1d6`, 5% `diseased`) — save vs Paralysis (+4 warding/fleeing) or fall under the horde: `prone` (attacker to-hit bonus + crawl) plus the new **`swarm_pinned`** condition (prevents attacking/casting — "can make no attacks until back on his feet"), 1d6, 5% disease. A pinned victim stands (clears both) on a later successful re-save.

- **Warding & fleeing are a single shared predicate.** `SwarmDriver.is_warding_or_fleeing(target)` (static) is consulted by BOTH the monster-swarm effect and `SpellCombatHooks._tick_swarm_damage_for`. It is true when the target carries the transient `is_warding` flag (set on any attacker that directs a weapon/missile at a swarm — the ward-off clamp block in `attack_resolver`/`ranged_attack_resolver`; cleared every `SwarmDriver.on_round_start`) OR is fleeing (`Combatant.is_fleeing`, a declared full retreat / fighting withdrawal, or an explicit `is_fleeing` flag). This wired the previously-deferred halving + save-+4 paths.

- **General rules.** Flee = 3-round persistence (`PERSIST_ROUNDS_AFTER_LEAVE`, still suffering effects during the window); **1 round if fleeing into water** (`MovementResolver.is_water_cell`). Pursuit (`should_pursue`): a swarm only chases a *fleeing* target if it has **taken damage** (`hp_current < hp_max`) AND the target is in **line of sight** (`MovementResolver.has_los_3d`); non-fleeing targets are chased normally. Sleep → dormant is modelled as the `slumbering` (or `swarm_dormant`) condition on the swarm — `apply_swarm_tick` short-circuits and applies no envelope. Fire/cold attacks damage a swarm in full: the 1d4 ward clamp lives only in the weapon/missile resolvers (which emit `"physical"`), so fire/cold spells — which call `apply_damage` with a fire/cold type — never hit the clamp.

- **`<3 HD` auto-drive-off only (spell + monster reconciled, 2026-07-24).** The auto-drive-off is RAW for the **Insect Plague SPELL** — `acore_spell_catalog_a-i_summary.xml:1280`: "Creatures of less than 3 Hit Dice are automatically driven off." The swarm MONSTER stat blocks (`le_monster_catalog_2_summary.xml`) state NO such rule, so the driver's sub-3-HD `frightened` is a deliberate, RAW-consistent port (Jedidiah's chip asked for it — a tiny creature flees any swarm). Both paths — `SwarmDriver._apply_swarm_condition` AND the spell's `SpellCombatHooks._apply_swarm_condition_to` — now frighten **only** `hd < 3`; a `hd >= 3` creature is engulfed but keeps full agency so it can **ward the swarm off** (a `frightened` creature can't attack). The earlier ">3 HD also frightened" behavior in the spell tick was a P3 design-literal, **not RAW**, and was removed 2026-07-24 (it contradicted the RAW ward-off option).

- **Rendering.** A swarm token renders as a **translucent, flattened, beak-less cloud** straddling its area cells — `CombatantToken3D.set_swarm_area(area_local)` reuses the multi-cell straddle path (via `footprint_local`) but paints the body at `SWARM_AREA_ALPHA` with `TRANSPARENCY_ALPHA`. `combat_map_renderer_3d` checks `c.is_swarm()` BEFORE `c.is_multi_cell()` (a swarm's footprint is 1×1, so the multi-cell branch would never fire). The swarm still MOVES as a 1×1 anchor — this only affects rendering.

- **Catalog.** `insect_swarm_*` (existing, +`swarm_area`), `bat_swarm_2hd/3hd/4hd` (AC 2, morale −2, fly, Confusion), `rat_swarm_2hd/3hd/4hd` (AC 0, morale −3, swim, Paralysis) — all `sub_types [swarm, vermin, <flavour>]`, all three non-blocking flags, no `footprint`. `Combatant.get_swarm_type()` reads the flavour sub_type. Tests: `tests/test_swarm_monsters.gd` (driver + geometry + catalog) and `tests/test_swarm_conditions.gd` (spell-tick + shared warding/fleeing halve).

## 127. Copper pieces are the canonical monetary unit — integers only, gp at the display boundary (Jedidiah ruling 2026-07-31)

Every monetary value in the codebase is stored, passed, and computed in **copper pieces (cp)**, as an **`int`**. cp is the lowest ACKS denomination (`Currency.DENOMINATIONS`, `engine/subsystems/commerce/currency.gd:22-28`: 1 pp = 500 cp, 1 gp = 100 cp, 1 ep = 50 cp, 1 sp = 10 cp), so taking it as the base unit makes every price, wage, treasury, fine, tribute, and treasure value exactly representable without a fraction. This section supersedes the mixed-unit tolerance implied by §56, which permitted "internal math stays in its native unit (gp for `TributeCalculator`-style calculators; cp for column-backed values)" — there is no longer a gp-native internal unit anywhere. §56's naming and display-site discipline stands unchanged and is the enforcement mechanism for this section.

- **Never a float.** No monetary value is ever a `float` — not in a local, a parameter, a return value, a signal parameter, a dict key, a JSON field, or a column. A monetary column is `INTEGER`, never `REAL`: `domain_extraction_ledger.cumulative_extracted_gp_per_family REAL` (`db/schema.sql:928`) and `signal cache_raided(..., value_lost_gp: float)` (`event_bus.gd:432`) are the two historical violations and are not precedents. Where a RAW rule genuinely divides — a pro-rated extraction rate, a quarter-wages ledger, a per-week share of a monthly cost — do the division in `float` **inside a single expression** and land it with `MathUtils.bankers_round(...)` (`engine/shared_types/math_utils.gd:22`) before it is stored or returned. The float never escapes the expression. Banker's rounding per §12 applies.

- **Convert at the point of entry, exactly once.** RAW publishes figures in gp. A gp figure arriving from a rulebook, a data file, or a player-facing SpinBox is multiplied by 100 **at the moment it enters the engine**, and is cp from then on. Keep the RAW gp literal visible next to the conversion so the citation stays checkable — the reference implementation is `CrimeAndPunishmentResolver`, which holds the RAW fine table as `fine_gp` literals (`crime_and_punishment_resolver.gd:65-140`) and converts exactly once (`fine_cp = int(band.get("fine_gp", 0)) * 100`, line 279); the faith handlers do the same (`consecrate_altar.gd:46`, `extract_divine_power.gd:58-60`). What is forbidden is a value that is gp on one side of a call and cp on the other: `domain_handlers.gd:332` multiplied an already-cp stronghold sum by 100 a second time, and line 360 fed a gp tribute figure into a parameter named `tribute_in_cp`. Both survived for months because nothing at the seam named its unit.

- **The unit suffix is mandatory and IS the contract.** Per §56, every monetary local, parameter, return-dict key, signal parameter, JSON field, and database column carries `_cp`. Bare names (`stronghold_value`, `magnitude`, `treasury`, `gold_value`) are prohibited — they are what let `domains.tribute_out_owed` accumulate three contradictory unit conventions written by four different subsystems. A `_gp` suffix is legal in exactly two places: a local holding a RAW citation immediately before its `* 100`, and a UI-input variable bound to a gp SpinBox. A name may never carry both units (`gp_yield_cp`, `army_marcher.gd:368`), and a single column may never carry two dimensions (`vassal_obligations.magnitude` is gp for construction duties and a family count for everything else — split it).

- **`Currency.format_cost(value_cp)` is the ONLY money formatter, and it is called at the display site.** `engine/subsystems/commerce/currency.gd:188` renders gp/sp/cp with a comma+space separator; `Currency.format_wealth(coins)` (line 210) renders physical coin counts including pp/ep for inventory, treasure, and loot views. Do not write a second formatter — `EquipmentCatalog.format_cost` (`equipment_catalog.gd:95`) is one, with a divergent separator, and is to be deleted in favour of `Currency` — and do not hand-roll `/ 100`, `% 100`, `"%d gp"`, or `"%d gp %d cp"` in a panel. There is deliberately **no `cp_to_gp()` helper**: pre-dividing loses the arithmetic-friendly int and moves the boundary upstream where the next reader cannot see it. The conversion happens at `Label.text = Currency.format_cost(x_cp)` and nowhere earlier.

- **Gp-denominated player input is a UI-local affordance, not an engine unit.** RAW prescribes amounts in gp and players think in gp, so a SpinBox may hold a gp value — but the surrounding API is cp and the widget converts on both edges. The pattern is `scenes/ui/dialogs/dismiss_henchman_dialog.gd:61-63`: a cp-native `show_dialog(..., default_final_wages_cp: int, ...)`, `/ 100` to populate the SpinBox, `* 100` on emit. Never widen a gp SpinBox's unit into the handler, the activity payload, or the column.

- **A gp→cp column rename is a migration PLUS a grep, in the same commit.** godot-sqlite returns 0/null for an unknown column without raising, so a consumer left reading the old name fails **silently** — Migration 116's `strongholds.gp_value → cp_value` left three live readers for months (§56). Every rename migration ships with a project-wide grep for the OLD name across `engine/ scenes/ tests/ data/ docs/`, run before the migration lands, with the result recorded in the commit message. Scale the stored values in the same migration; never leave a renamed column holding unscaled data. The same discipline applies to renamed signal parameters and renamed return-dict keys, which are contracts of equal weight.

---

## 128. Troop Battle Rating is a per-CREATURE figure — never read it off the per-unit table (2026-07-31)

`rules/daw_campaigns_troop_tables_summary.xml` publishes Battle Rating **twice**, at two different granularities, and the two tables do not agree to more than one significant figure. Reading the wrong one is silent and expensive: `domain_stocker.gd:239` wrote `0.025 * count` for ordinary Light Infantry, making every generated NPC garrison roughly three times as strong in battle as RAW allows, for as long as the stocker had existed.

- **The per-creature table (§troop_tables, L60-262) is the source for anything per-soldier.** L9 states it outright: "Battle Rating is listed per creature." Human troops are L101-186, demi-humans L60-99, beastmen L187+. Light Infantry A is **0.008** (L105); Untrained Conscripts/Militia **0.003** (L102); Heavy Infantry A **0.017** (L129); Bowmen **0.013** (L144); Crossbowmen A and Longbowmen A/B **0.025** (L147, L150, L153); Light Cavalry A **0.061** (L156); Horse Archers and Medium Cavalry **0.082**; Heavy Cavalry **0.103**; Cataphracts **0.124**.

- **The per-unit table (§unit_characteristics_summary, L270-333) is NOT that figure divided out.** L278: "Battle Rating is per unit in these tables"; L273: a unit is 120 infantry or 60 cavalry. Its values are the per-creature ratings re-rounded to convenient halves — 0.008 × 120 = 0.96 is listed as **1**, and 0.003 × 120 = 0.36 is listed as **0.5**. So the per-unit table is coarser, and dividing it by 120 gives a *different* number than RAW's own per-creature row. Use it for one purpose only: **veterans**, which have no per-creature row anywhere (RAW's only per-creature veteran statement is L264-266, the "25% of human units are veterans" composition rule). A veteran per-soldier figure is therefore explicitly derived — unit BR ÷ unit size — and must say so at the constant.

- **The two tables share numbers across unrelated troop types, which is exactly how the bug hid.** 0.025 is simultaneously: the per-creature rating of Crossbowmen and Longbowmen (correct wherever those appear), and 3 ÷ 120, the derived rating of *veteran* Light Infantry. A bare `0.025` literal is therefore not self-diagnosing — it looks plausible next to any infantry row. Never write a per-soldier BR as a bare literal.

- **`TroopBattleRatingTable` (`engine/subsystems/troops/troop_battle_rating_table.gd`) is the single lookup.** `per_soldier(troop_type, tier)` and `for_unit(troop_type, tier, count)`; `troop_units.battle_rating` stores the whole-unit total, so `for_unit` is what a `create_unit` payload wants. It normalizes free-text `troop_type` ("Light Infantry" / "light_infantry" / "LIGHT-INFANTRY" all resolve alike) because the column is unconstrained TEXT and call sites genuinely vary. New code minting a `troop_units` row calls it instead of transcribing a figure; this is also the shape a per-type/per-tier garrison mix needs.

- **Untrained beats troop type.** Any unit at `tier='untrained'` returns 0.003 regardless of the troop-type label it carries, because RAW has exactly one untrained row (L102) and the training rules (L333-343) treat a troop type as something conscripts are *trained into*. This is a documented PROJECT reading, flagged as such at the function — RAW has no "untrained longbowmen" row to consult.

- **An unrecognized troop type warns and falls back to light infantry; it never returns 0.0.** A 0.0 rating silently erases the unit from every downstream BR sum (`FieldBattleResolver`, `SiegeResolver`, `CallToArmsHandler`'s `ORDER BY battle_rating DESC`), which reads as "this unit doesn't exist" rather than "this lookup failed". Per §Error Handling, fail loud and degrade conservatively.

- **The demi-human and beastman columns are per-race and are NOT interchangeable with the human ones.** Goblin Bowmen are 0.004 where human Bowmen are 0.013; Goblin Wolf Riders 0.107 and Orc Boar Riders 0.131 where the human table has no rider row at all. Tribal/beastman troops are therefore looked up through `TribalWarriorRegistry.stats_for(race, troop_type)`, which is per-race; `TroopBattleRatingTable` models human troops only and the two must not be "reconciled" into one table. See §72 for the per-race structure.

---

## 129. Base morale belongs to the troop; leader effects are roll-time modifiers (Jedidiah ruling 2026-08-01)

`troop_units.morale` stores the unit's OWN base morale and nothing else. Every leader, Charisma, proficiency and class-power effect is applied by whatever subsystem rolls the morale check, at the moment it rolls.

The ruling came from `solicit_followers.gd`, which minted bardic mercenaries with a hardcoded `"morale": 0` where RAW gives untrained conscripts and militia **-2** (`daw_campaigns_troop_tables_summary.xml:102`). The 0 was the bard's bonuses pre-applied. Jedidiah: *"base morale doesnt change for bards, they just grant a +2 morale bonus. It shouldnt have been hard coded the way it was and should just be a modifier granted by the bard."*

- **Why baking it in is wrong, not merely untidy.** A pre-applied bonus outlives its cause. The bard's Chronicles of Battle is explicitly conditional — `acore_campaign_classes.xml:572`, *"+1 morale if the bard is present to witness and record their deeds"* — and the officer morale modifier (`daw_armies_recruitment.xml:773-779`: Charisma bonus, +1 for a bard/fighter/explorer/barbarian/paladin of 5th level or higher, +2 for Command proficiency, +1 for legendary leader) applies only *"for units under the officer's command"*. A unit carrying those in its stored morale stays brave after the bard dies, is sold to another ruler, or is left in a garrison a hundred miles away. It also double-counts the instant a live modifier system is wired up.

- **The modifier mechanism already existed.** `ChroniclesOfBattleAura.compute_aura_bonus` returns `{morale_delta, bard_character_id}` and is presence-gated by party co-location; `AURA_MORALE_BONUS = 1` cites the RAW line. Callers add the delta to their roll and call `emit_aura_applied` for log surfacing. Reach for that path — do not fold the number into a stored column.

- **Base morale is per troop type, and per race for beastmen.** It is the `morale` attribute on the same per-creature rows that supply wage and battle rating (§128), so it comes from the same lookups: `TroopBattleRatingTable.base_morale(troop_type, tier)` for human troops, `TribalWarriorRegistry.stats_for(race, troop_type).base_morale` for tribal ones. Human values run untrained -2, light infantry / slingers / bowmen -1, heavy infantry / crossbowmen / longbowmen 0, most cavalry +1, heavy cavalry / cataphracts +2. Beastman values are race-dependent on the same axis as everything else: kobold -2, goblin -1, orc / hobgoblin / gnoll 0, lizardman / bugbear / ogre +2, and wolf- and boar-riders +2.

- **Untrained overrides the troop-type label** (-2), and **veterans add +1** (`daw_campaigns_troop_tables_summary.xml:266` — *"Veterans have 1 HD, 5 hp, +1 morale, and +1 to damage rolls"*). Both are handled inside `base_morale`, mirroring how `per_soldier` collapses untrained units onto RAW's single untrained row.

- **A one-time RAW adjustment IS stored — a conditional one is not.** The distinction is whether RAW makes the effect permanent at the moment of creation. Tribal warriors are the worked example: `ax_domains_of_chaos.xml` §tribal_warrior_morale says *"Tribal warriors use the base morale of their troop type"* AND grants a **one-time** ±1 for a Steadfast/Stalwart or Apathetic/Demoralized domain at levy time. Both belong in the stored value, so `LevyTribalWarriorsHandler` writes `base_morale + morale_modifier`. It previously wrote the modifier alone, which dropped the troop type's morale entirely and made a kobold levy (RAW -2) and an ogre levy (RAW +2) come out identical.

- **Test the base, not the sum.** Assert the stored value equals the troop type's own RAW base (plus any one-time adjustment), and assert leader bonuses are absent from it. A test that only checks the total cannot tell a correct base from a wrong base plus a compensating bonus — which is exactly how the bardic `0` (= -2 base + 2 bard) survived review.

---

## 130. A constant mirroring a DB CHECK is tested for set EQUALITY, against the live schema (2026-08-01)

Several subsystems keep a GDScript constant that mirrors a `CHECK(col IN (...))` enum so callers can validate before insert. `DepartureLogRecorder.VALID_EVENT_TYPES` is the canonical example, and it is also the cautionary one.

Migration 129 added six `tribal_warriors_*` values to the `domain_departure_log.event_type` CHECK and never added them to the constant. `record()` rejects anything absent from the constant, so **every** tribal-warrior departure-log write was refused before it reached a column that would have accepted it — `record()` returned `""`, callers ignored the return, and the entry vanished. It fired seven times in a single suite run and had been doing so since Phase 11D.5.

- **The guard test existed and could never have caught it.** `test_valid_event_types_matches_check_constraint` walked `VALID_EVENT_TYPES` and asserted the DB accepted each one — that is `constant ⊆ CHECK`, the direction a *lagging* constant always satisfies. The failure mode is the constant being SMALLER than the schema, so the load-bearing assertion is the other direction, `CHECK ⊆ constant`. **Assert both, and assert the sizes match.** A subset assertion on a mirror is not a weak test, it is a test of the wrong proposition.

- **Read the constraint from the live database, not from `db/schema.sql`.** `SELECT sql FROM sqlite_master WHERE type='table' AND name='<table>'` returns the `CREATE TABLE` DDL as it exists after every migration has actually run, which is what the code will hit at runtime. The old test's comment claimed "we can't read the constraint directly" — that was simply wrong, and the belief is what left the test guessing. Strip `-- ...` comments from the DDL before extracting the quoted literals, and walk to the matching close-paren so an adjacent CHECK cannot bleed into the parse.

- **Exercise the guard, not just the constraint.** `tests/test_tribal_warriors.gd::test_event_type_check_accepts_tribal_warrior_events` inserted straight into the table with raw SQL. That proves the CHECK accepts the value and proves nothing about the GDScript gate in front of it — it passed happily while every real write was being dropped. When a write path is `guard → repository → column`, at least one test must go through the front door. The replacement round-trips **every** valid event type through `record()` and asserts a row lands.

- **A validation helper that returns `""`/`false` on rejection needs a test that would notice.** `record()` does `push_error` and returns `""`; no caller checks, and none reasonably could — a levy cannot recover from a failed log write. That is an acceptable design, but it means the *only* thing standing between a drifted enum and silent data loss is the test. Where a subsystem's constant is declared to mirror the schema, the equality test is not optional polish; it is the mechanism.

- **This was audited across the codebase on 2026-08-01 and the departure log was the only real instance.** `RealmRepository.VALID_DISPOSITIONS` matches `realm_relations.disposition` exactly. Other constants that look like subsets of a CHECK — `WORN_WEIGHTLESS_SLOTS`, `MAGE_PROGRESSION_CLASSES`, `HUNT_BIOMES`, `FRIENDLY_OR_BETTER` — are deliberately narrower filters over a column's domain, not mirrors of it, and must NOT be "reconciled". The distinguishing question is whether the constant claims to be the column's full domain; if it does, it owes the schema an equality test.

---

## 131. Unit Loyalty is its own ladder — not the henchman one, and no leader bonuses (2026-08-01; generalized to every source type 2026-08-03)

Established while building `UnitLoyaltyResolver` (`engine/subsystems/troops/unit_loyalty_resolver.gd`, named `TribalWarriorLoyaltyResolver` until 2026-08-03). ACKS has **three** 2d6 loyalty ladders that share the same five bands (2− / 3-5 / 6-8 / 9-11 / 12+) and are otherwise different mechanics. Sharing the bands is a design coincidence, not evidence they are the same table.

| Ladder | RAW | Fanatic carryover | Grudging carryover |
|---|---|---|---|
| Henchman loyalty | `acore_equipment.xml:806-808` | +2 all future rolls | one-shot −1 on the next roll |
| Vassal edge (project §5.2 stack over the henchman roll) | `gdd-faction-framework.md` | inherits henchman | inherits henchman |
| **Unit Loyalty** | `daw_armies_recruitment.xml:99-107`, table at `:265-275` | **+1** all future rolls | **two CONSECUTIVE grudging results end service** |

- **Do not route unit loyalty through `HenchmanLoyaltyResolver`.** It was tempting — `HenchmanTables.loyalty_result` already resolves the identical bands, and `VassalLoyaltyResolver` is a working template for the persist-carryover-on-the-row pattern. But its `set_fanatic` / `clear_grudging` result keys encode the +2 / one-shot-−1 semantics, which are wrong for units by 1 point and by an entire departure condition. `UnitLoyaltyResolver.outcome_for_total` resolves the DaW bands locally with its own citation so a future change to the henchman table cannot silently move unit loyalty. Mirror the *pattern*; don't share the *function*.

- **The officer / leader morale modifier does NOT enter a loyalty roll.** `daw_armies_recruitment.xml:775` — *"Morale modifier affects Unit Morale rolls for units under the officer's command; it does not affect Unit Loyalty rolls."* So the bard's Chronicles of Battle aura, the commander's Charisma bonus, Command proficiency and legendary-leader status are all excluded. The only morale term is the unit's own stored `troop_units.morale`. This is what makes §129's stored-vs-roll-time split load-bearing rather than tidy: a unit whose stored morale had the bard's +1 baked in would carry that bonus into a roll RAW says the bard cannot influence at all. The whole modifier stack for a unit loyalty roll is `unit morale − 2 per calamity after the first + 1 if already fanatic`, and nothing else may be added to it.

- **Carryover that spans rolls needs a persisted column, and a *run counter* beats a boolean.** Migration 212 adds `troop_units.loyalty_is_fanatic` and `troop_units.loyalty_consecutive_grudging`. The grudging condition is "two **consecutive**" — a boolean "last result was grudging" would satisfy the rule, but a counter makes the run length auditable after the fact and lets a test assert the reset actually happened rather than inferring it from a departure that didn't occur.

- **Suppression applies to the OUTCOME, not the roll.** `:107` — *"fanatic loyalty can never result from going without pay and becomes ordinary loyalty in that case."* Downgrade after the band is resolved, and do not touch a fanatic status the unit already held: RAW bars the unpaid roll from *creating* fanaticism, not from a fanatic unit rolling well while unpaid.

- **Test the suppression with a control.** `test_loyalty_fanatic_never_results_from_going_without_pay` rolls the same 12 twice — once unpaid (expects the downgrade) and once out-of-supply (expects Fanatic). Without the control the test would pass just as happily if the total had missed the 12+ band for an unrelated reason.

- **Only chronicle the departures.** The migration-129 event type is `tribal_warriors_loyalty_failed`. Writing a survived calamity under it would make the log assert something false. Survived rolls stay observable through the returned dict and the persisted carryover columns.

- **Calamity trigger points are a fan-out, per §70.** Each subsystem that can detect a calamity calls `roll_loyalty` itself with the kinds it knows about: `ArmyCasualtyResolver._resolve_side` supplies rout + ≥25% casualties, `ArmySupplyTracker.run_supply_tick` supplies out-of-supply (RAW `daw_campaigning_armies.xml:360-361` — **each** unsupplied week is a calamity, not a 2-week threshold), and `DomainHandlers._tick_unit_loyalty` supplies the 3-month-spoils stretch, **the unpaid month** (§132) and **the militia campaigning season** (`:459`). The resolver takes the kinds as a parameter and owns only the roll, the carryover and the departure — it never goes looking for calamities itself.

### The 2026-08-03 generalization — one roll, six departures

The roll was never tribal-specific: `:99-107` is printed in the mercenary chapter and the conscript (`:353`), militia (`:458`), follower (`:477`) and slave-soldier (`:611`) chapters each point back at the same table. What was source-specific was the **departure**. Generalize the class and dispatch the exit; do not fork the roll.

- **Split the two gates.** `UnitLoyaltyResolver.rolls_loyalty(unit)` answers *may this unit roll at all* — the RAW source-type allow-list plus the `:483` religious-fanatic exemption — and every fan-out site calls it instead of re-implementing a `source_type ==` check. `_resolve_departure` answers *where do they go*. Trigger sites decide only **which calamities**.

- **An absent source type must be absent on purpose.** `vassal` does not roll, because `§vassal_troops` gives vassal troops no loyalty rule at all — it says a vassal's garrison "is some mix of followers, mercenaries, conscripts, and militia", i.e. they *are* those types once mustered. `ROLLING_SOURCE_TYPES` maps each type to the RAW line that grants it the roll, so an entry nobody can cite is an entry nobody adds.

- **Departure ≠ release, and conflating them imports the wrong rule.** RAW gives militia two different exits: `:461` "cannot voluntarily leave service, but may **desert**" and `:462` "if voluntarily **released**, militia return to their farms." Only the first is a loyalty outcome. Sending deserters home would have been a plausible-looking bug that quietly handed the ruler back a resource RAW says he lost. Same shape for conscripts (`:355` desertion vs `:356` release) — the release half is a separate activity and is deliberately unbuilt.

- **Desertion relieves a standing cost; only DEATH is permanent.** `ArmyCasualtyResolver._apply_militia_population_loss` permanently reduces `peasant_families` and morale per `:432`, and it counts only battle `crippled`. Nothing on the loyalty path may reach it — a bad 2d6 must not depopulate a domain. What desertion *does* relieve is the §133 levy penalty, and it does so for free because that basis is a live query over ACTIVE rows. Two tests pin both halves, because the failure mode is silent in each direction.

- **A time-based calamity needs an anchor, and the anchor needs ONE owner.** `:459`'s season of continuous campaigning is the only calamity that depends on elapsed time. `troop_units.campaigning_since_calendar_day` (migration 214) is set, cleared and re-anchored entirely inside the monthly tick from `assignment_kind`, so no muster, call-to-arms or extraction path has to remember to maintain it — the same reasoning as §133's live levy basis. Re-anchor rather than clear after firing: RAW says **each** season, so a unit kept in the field for a year takes it roughly four times. Clearing on "not campaigning" is what makes the stretch *continuous* rather than cumulative.

- **Exempt on a recorded fact, not on a coincidence.** `:483` bars religious fanatics from calamity rolls, and `:481` defines them as cleric and bladedancer followers. The tempting test was `monthly_cost_cp == 0`, which `FollowerArrivalResolver` writes exactly when the class table says `wages_required: false` — but the **mage** table is also `wages_required: false`. That proxy is correct today only because the mage table happens to have no `soldier_attractor` and mints no rows: one data-file edit away from silently exempting the wrong units. Migration 214 records the RAW fact directly. **When a derived test is right by accident, write down the fact instead.**

- **An event type that names a source type must not be reused for another.** Migration 129's `tribal_warriors_loyalty_failed` stays tribal-only; migration 214 adds `troop_unit_loyalty_failed` for the rest, and the `EventBus` signals split the same way (the generic one is the superset and fires for every departure). Reusing the old name for a mercenary company would have made the chronicle assert something false — the same rule as "only chronicle the departures", applied to *whose* departure.

## 132. Aggregate pay, ex-post-facto unpaid designation (2026-08-02)

Established while wiring the last RAW Unit Loyalty calamity, *"going without pay for a month"* (`ax_domains_of_chaos.xml:455`, `daw_armies_recruitment.xml:98`). The pattern generalizes to any RAW rule that asks *which specific thing* a bulk transaction failed to cover.

- **Do not build per-unit arrears to answer a per-unit question.** Troop wages are summed into `GarrisonExpenditureCalculator` / `DomainExpenseCalculator` and settled as one number against the treasury; no unit is ever individually paid. The obvious implementation — per-unit payment rows and an arrears column — would have touched every mint site, every disband path, and the monthly settlement, to serve one loyalty roll. Per Jedidiah (2026-08-01) the payment path stays untouched: `TroopPayShortfallResolver` reads the shortfall *after the fact* and designates which units it must have been. No migration, no new columns, no change to how money moves.

- **The designation rule is a replaceable function, not a branch.** `resolve_for_domain(domain_id, funds_available_cp, designator := Callable())` — the default is cheapest-first by `monthly_wage_cp` until the designated wages cover the shortfall; a future player-facing "choose who goes unpaid" screen passes its own `(shortfall_cp, units) -> Array` at the DomainHandlers call site and nothing else moves. NPC domains never supply one. **A designation that under-covers is topped up cheapest-first, not rejected** (`DESIGNATOR_CUSTOM_TOPPED_UP`): the money is gone either way, so leaving part of the shortfall unassigned would silently pretend those wages were paid.

- **Designate across the whole roster; gate only the ROLL by source type.** The wage bill spans every source type and every `assignment_kind`, because who a ruler can afford is a question about all of them at once. Only `source_type = 'tribal_warrior'` units actually roll, since nothing else has a departure pipeline. Keeping those two gates separate means extending Unit Loyalty to mercenaries/conscripts/militia/followers later changes one condition in `DomainHandlers._tick_tribal_warrior_retention` and nothing in the money layer.

- **Do not reuse `GarrisonExpenditureCalculator`'s filters for a payroll question.** It filters `assignment_kind = 'garrison'` because it computes the RAW garrison *morale threshold*. Levied tribal warriors are minted `'available'` (`levy_tribal_warriors.gd:187`) and `ExtractionResistanceRouter` flips units to `'on_campaign'` and back for the length of a muster — so borrowing that filter would have made the rule a no-op for the only units that can roll, and made the calamity flicker with musters. Two calculators, two questions.

- **`monthly_wage_cp` is the whole unit's wage; `monthly_cost_cp > 0` is the "on the payroll at all" test.** Every mint site multiplies by count before storing, so "cheapest" means the cheapest *unit* — which is what a ruler chooses between. `monthly_cost_cp` is the denormalized wage + specialist + 4 × weekly supply figure; supply is not pay. Units at cost 0 are the RAW by-value-only cases (faithful followers, trained militia — `acore_axioms` §garrison L229-230) and have no pay to go without; `daw_armies_recruitment.xml:481` independently bars religious fanatics from calamity rolls.

- **Same-tick calamities combine into ONE roll.** RAW `daw_armies_recruitment.xml:100` gives −2 per calamity after the first, so the monthly tick collects the spoils stretch and the unpaid month into a single `roll_loyalty` call rather than firing twice. Two rolls would hand the unit two independent chances to depart at full strength — strictly harsher than the printed rule, and a bug that would have been invisible without asking "can these two trigger in the same tick?" **When adding a trigger point, check whether an existing one shares its tick.**

- **Test a random-outcome tick by making the outcome impossible to miss.** The monthly tick owns its own dice (no injection seam), so these tests set unit morale to −20, which puts every possible 2d6 result in the 2− Enmity band. "Departed" then means "rolled" and "active" means "did not roll", deterministically — with an un-designated control unit in the same domain to prove the trigger fired, not just the tick.

---

## 133. Peasants under arms are a standing domain cost, not a free action (Jedidiah ruling 2026-08-03)

Established closing the §100 gap and implementing the tribal excess levy. `LevyPenaltyCalculator` (`engine/subsystems/troops/`) owns the whole model.

- **RAW charges for a levy the moment it is raised, not when someone dies.** `daw_armies_recruitment.xml:429-431`: each levied peasant costs one family's revenue, the levy costs −1 domain morale at ≤1 per 10 families or −2 at ≥2 per 10, and "these penalties remain until the militia is sent home." Only the permanent-on-death half of this had ever been built (§100 flagged the rest on 2026-07-04), so a militia levy was **free** until a battle. Closed now for militia AND for excess-levy tribal warriors, through one source-agnostic calculator.

- **The penalty basis is a LIVE query, never a stored counter.** `levied_peasants_for_domain` sums active `source_type='militia'` plus active `tribal_warrior` rows with `is_excess_levy=1`. That is what makes ":431 until sent home" true for free: stand-down, battle death and loyalty departure all set `status='departed'` or drop `count`, and the penalty relieves itself. A domain-level counter would need every one of those paths to remember to decrement, and one forgotten path means a permanent phantom penalty on a domain with no troops.

- **Read the penalty ONCE per tick and feed both consumers.** `DomainHandlers._resolve_domain_month` computes `LevyPenaltyCalculator.penalties_for_domain` and passes the count to `calculate_monthly_revenue` and the morale figure to `resolve_base_morale`. Two independent reads would be two chances to disagree about how many peasants are levied, and the disagreement would be silent.

- **Revenue loses FAMILIES, not a percentage, and `peasant_families` is untouched.** `:429` reduces revenue "by one family" per levied peasant, so the levied count is subtracted from the revenue-bearing population before service, tax and land are computed — `:429` says "domain revenue", not one stream of it. The population itself still exists; it is carrying spears. Reducing `peasant_families` would double-charge against the permanent-on-death mechanic and shrink the levy cap that bounds the resource.

- **Standing conditions belong in BASE morale; monthly events belong in the event-modifier sum.** `:431`'s "remain until sent home" makes the levy penalty the same shape as the classification and insufficient-stronghold penalties already in `resolve_base_morale`, not the same shape as a this-month garrison shortfall. The test for which one you want is whether the cause persists across ticks on its own.

- **When one population gets a second, differently-priced allotment, the invariant must scope to the first.** A clanhold's free 1-per-family warriors and its `:399` excess levy are both `source_type='tribal_warrior'`, but `available + levied <= peasant_families` is a statement about the FREE allotment only. Counting excess rows in it makes a perfectly legal overflow report as a violation the instant a chieftain uses it. `TribalWarriorRegistry._sum_levied_count` filters `is_excess_levy = 0`; the excess gets its own ceiling and its own `excess_cap_ok`. **Two allotments, two ceilings, checked separately.**

- **Both ceilings derive from `peasant_families`, so a shrink must enforce both.** `DomainHandlers._release_tribal_warriors_for_population_loss` originally early-returned when the free side still fit, which silently left excess warriors above a shrunken cap. The two overages are independent — a shrink can breach either, both or neither — so compute the free side without returning on it, then always trim the excess. Caught by test, not by review.

- **A per-unit flag beats a domain counter when rows are the thing that leave.** Migration 213's `troop_units.is_excess_levy` works because `LevyTribalWarriorsHandler` composes the free and excess portions SEPARATELY, so every spawned row is wholly one or the other. Had it composed once and split afterwards, a row could hold both and the flag would be unscoreable.

- **Rewriting a test that encoded the old rule is not the same as deleting it.** This change turned `test_levy_caps_at_available` and `test_levy_rejects_zero_available` red — correctly, since both asserted cap-and-refuse. Each was rewritten to pin the NEW ceiling rather than dropped, and the rejection test gained a control case (same empty pool, a domain large enough to have excess room, expect success) because "levy refused" would otherwise pass just as happily if the excess path had never been wired at all.

---

## 134. Designating a set from a shortfall: name the harmed set or the spared set, and let that choose the ordering (Jedidiah ruling 2026-08-03)

Established implementing RAW's partial-supply allocation (`daw_campaigning_armies.xml:365-367` — "if an army can feed only some units, its leader chooses which units are supplied"). This is §132's pattern applied a second time, and the second instance is what exposed the part §132 could not have known.

- **The ex-post-facto designation pattern generalizes; the ordering heuristic does NOT.** §132 established that when a bulk transaction falls short you derive *which units* it failed to cover rather than building per-unit ledgers. `ArmySupplyAllocationResolver` reuses that wholesale — one stockpile, one weekly cost, designation derived after the fact, no migration. But copying `TroopPayShortfallResolver`'s cheapest-first ordering across would have been wrong, because the two resolvers designate **opposite sets**. The pay resolver names the HARMED set (who goes unpaid), so cheapest-first maximises the number harmed. The supply resolver names the SPARED set (who eats), so cheapest-first would MINIMISE harm — the same greedy, inverted meaning. Best-battle-rating-first is what preserves the harsh reading here, and it is also the choice a commander actually makes: feed the cataphracts, let the levy tighten its belt.

- **So: when you reuse a designation resolver, state which set you are naming before you pick a comparator.** "Mirror the existing one" is not a specification. Write the sentence "this resolver names the ___ set" in the docstring first; the ordering follows from it.

- **The validation guard inverts with the set too.** The pay resolver's constraint is a FLOOR (the designation must cover the shortfall) so an under-designation is **topped up** — leaving part unassigned would pretend wages were paid. The supply resolver's constraint is a CEILING (the stockpile) so an over-designation is **trimmed**, worst-first — honouring it would conjure food that does not exist. Both refuse to reject the player's input outright; both degrade toward the default. Same principle, opposite direction.

- **A "partially supplied" unit is a real RAW state and it resolves the residue cleanly.** After the greedy walk the stockpile holds less than the cheapest remaining unit costs. That residue is spent on the highest-priority *starving* unit, which stays unsupplied — RAW `:360` counts a week "partially or completely unsupplied" as a calamity and `:366` exempts only *supplied* units. This is why a shortfall week still drains the stockpile to 0, which is what the all-or-nothing model it replaced already did, so the pre-existing `stockpile_after` test needed no change. Reach for a RAW state that already exists before inventing carry-forward semantics.

- **Skip an unaffordable unit, do not stop the walk.** Priority order says who gets fed first, not that the first unaffordable unit ends the meal. A quartermaster with 100 cp left, a 240 cp cavalry unit next and a 60 cp infantry unit after it feeds the infantry. Stopping would strand food nobody eats.

- **A cost of 0 means "not a participant", not "cheapest".** Hungerless units (`daw_campaigning_armies.xml` §hungerless_troops L265-269) are force-supplied and consume nothing — including when a custom designator omits them, because a designator that did not name them did not mean to starve them. Any greedy allocator needs this branch before its comparator, or a free participant sorts to one end and distorts the walk.

- **A RAW modifier that is not a calamity must not enter the calamity stack.** `:367`'s "additional -1" for being visibly left to starve is -1, and it is *additional to* the out-of-supply calamity — so modelling it as a sixth calamity kind would have gotten both the size (-2) and the count (the -2-per-extra-calamity multiplier) wrong. It goes through `UnitLoyaltyResolver.roll_loyalty`'s `situational_modifier` parameter, RAW `:99`'s "adjustments" term, added last and defaulting to 0 so every pre-existing caller's arithmetic is untouched. **When RAW says "additional", check whether the existing stack would double-count it.**
...
## 140. Structured per-family deity morphs in the conlang kits (2026-07-01; renumbered from 87 in the 2026-08-06 consolidation merge) [PROVISIONAL]

`religion.sample_deity_renames` in each `data/conlang/culture_*.json` maps every Agrippan canon-deity to a STRUCTURED object (it was a packed string `PRIMARY (Lang. SECONDARY) - gloss`), so the future deity router (gdd-hybrid-conlang-fusion.md §6.2 — temple/noble = clan-family morph, shrine/commoner = civ-family morph) selects by parent family without re-parsing. Split offline by `tools/restructure_deity_renames.py` (idempotent one-off, the §7.4 data-prep precedent; cross-refs §81 name banks).

- **Shape is ROLE-keyed, NOT family-keyed.** `{ "primary": morph, "primary_family": fam, "secondary": morph, "secondary_family": fam, "gloss": text }`; `secondary`/`secondary_family` are omitted when only one morph was authored (bases, fully-fused Confederated / single-register hybrids, same-family Peer pairs), `gloss` omitted when empty. The naïve `{germanic: .., near_eastern: ..}` (the GDD's original sketch) does NOT work against the real data: (a) INVERSIONS — `sebasos` (near_eastern×classical) and the `kaimets/Delorum` entry carry the primary morph in `inherits[1]`, so position ≠ backbone; (b) same-family Peer hybrids (ausonians/hekana/mudana/tollteca) hold two morphs from ONE family that collide on a family key. Role keys carry the family as a VALUE (`*_family`), sidestepping both.
