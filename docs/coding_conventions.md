...
- **Sim WorldGenRng stream names (the per-tick draws).** Registry so a future phase doesn't collide: `contest` (4b border contest), `war_escalate` / `war_margin` / `war_transfer` / `war_pillage` (4d), `secede` (4d). Each is keyed `(campaign_seed, name, tick, "P>Q"|entity)`; independent per pair so iteration order never affects a value.