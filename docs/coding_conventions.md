...
- **The CONQUEST trigger RETARGETS assimilation, not a new phase.** `_assimilate_held_hexes` — on a conqueror's held hex carrying a foreign BASE substrate — computes a `culture_target`: default the owner, but if `_conquest_merge_target(owner, subject)` returns a hybrid, the hex assimilates toward HYB(A,B) instead (conquered/contested zones become the hybrid). §6.4 gate: a clan×civ pair merges ONLY when the OWNER (conqueror) is the clan (`oc != sc and oc != "clan"` → displace); same-class is symmetric. Reuses the §4c per-pair lock (`_merge_decisions`), so a pair resolves to the SAME hybrid/decision whether it first meets at a border (§4c) or a conquest (§4d). Alignment still assimilates toward the realm's own alignment. This also converts same-class PEACEFUL-border owned hexes (owned mixed substrate) — accepted (a stable border → a hybrid people; lets Peer/Confed realms reach dominance).
- **Realms are RELABELED to the hybrid at FINALIZE, never mid-sim (Jedidiah).** The sim runs on base ids; only substrate goes hybrid. `_finalize_hybrid_identities()` (first line of `_finalize`, before serialize/handoff) relabels any realm whose mass-weighted populated-substrate dominant (`_dominant_populated_culture`) is a HYBRID at ≥ `hybrid_adopt_min_share` (0.5): `culture_id` → the hybrid, `culture_synthesis_parents = [base_a, base_b]`; base realms carry `[]`. Emits a `cultural_shift`/`hybrid_emerged` event. Runs at finalize → never perturbs the tuned mid-sim dynamics (calibration-safe).
...
**§4e — go-native adoption + calibration (2026-06-30, culture-emergence arc COMPLETE):**
- **Hybrid REALMS emerge via GO-NATIVE, not (mainly) the finalize relabel.** Because the seeder injects all 55 hybrid instances (§4c), a hybrid can be a large, prestigious SUBJECT of a realm — so `_phase_go_native` (a conqueror adopting a more-developed subject) adopts the emergent hybrid, flipping the realm's `culture_id` mid-sim. This is the "reuse go-native" the emergence ruling called for (Jedidiah via AskUserQuestion chose "embrace go-native + record" over finalize-only). Go-native does NOT set `culture_synthesis_parents`, so `_finalize_hybrid_identities` has TWO cases: **Case 1** — a realm whose `culture_id` is ALREADY a hybrid records its parent bases; **Case 2** — a base-owned realm whose populated substrate is dominantly a hybrid (≥ `hybrid_adopt_min_share`) is relabeled. Both persist `culture_synthesis_parents`.
- **Hybrids manifest at two levels:** (1) SUBSTRATE / regions — conquest+border zones become dominantly a hybrid, robustly (~26 dominant hexes/large map at `base_p`=0.5, up to 135 on contact-heavy seeds); Layer-5 region naming reads the dominant substrate, so the 55 hybrid name banks appear on regions even without a realm relabel. (2) whole REALMS — occasional (mean ~0.67/large map at 0.5, contact-dependent; some maps 0).
- **Calibration is via `tools/calib_sweep.tscn`** (extended with hybrid_realms / distinct_hybrids / hybrid_dom_hexes + realms_hyb_plurality / max_hyb_share diagnostics). `hybrid_merge_base_p`=0.5 is the balance: §17-NEUTRAL (wild 57.3 vs 58; realms 28.5 vs 30.5) with a notable-minority hybrid presence; 0.7 is hotter (mean 1.83 realms but a seed-1000 runaway ≈ half the map). All `hybrid_*` knobs PROVISIONAL — raise `base_p` for more prominent hybrids. The §3.3 parity/alignment merge drivers stay DEFERRED (contextual; don't fit the per-pair lock — the base×family roll + go-native developed-gradient already give a sensible spread). Integration guard: `test_setting_hybridization.gd::test_hybrid_emerges_on_generated_map` (full large seed-1000 gen → substrate + ≥1 recorded hybrid realm).
