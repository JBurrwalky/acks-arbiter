...
- **Check at the lazy-generation seam only:** `DungeonFixtureService.get_or_generate_voxel` is the single gate (its two production callers are the hex-map renderers' dungeon-entry paths). On mismatch it (1) recovers the generation spec — the payload's preserved `"spec"` key, else `DungeonGeneratorRepository.derive_request_spec()` from the stored `dungeon_floors` rows — (2) purges BOTH stores (`delete_dungeon_layout` relational cascade + `CampaignRepository.delete_voxel_cells_for_map` for persisted runtime fog/door state), then (3) falls through to normal regeneration. Never check versions downstream of the seam.
- **Preserve the spec across the voxel overwrite:** the fixture service's step-5 metadata merge carries `"spec"` (plus the narrator keys) into the stored payload so a future bump can regenerate the dungeon with its true shape. Regeneration is deterministic (seed derives from the entrance id), so a same-version regenerate reproduces the payload byte-identically.
