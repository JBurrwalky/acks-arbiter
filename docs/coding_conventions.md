...
- **A faction-goal quest predicate = a persisted `progress.goal_satisfied` flag** the faction layer sets (`set_faction_goal_satisfied`) and `QuestCompletionWatcher.poll_faction_goals` reads monthly (idempotent, migration-free) — the "faction goal state" is that flag, not a live cross-subsystem query.
