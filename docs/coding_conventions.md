...
- **ACKS composition (RAW, ascending AC, base 0):** body armor `armor_ac_bonus` (Hide 1 … Plate 6, `acore_equipment.xml:143-148`) + shield `armor_ac_bonus` (+1, `:149`) + Dexterity modifier (`CharacterData.ability_modifier` of the EFFECTIVE DEX; `acore_basics_and_characters.xml:242`, creation step 8 `:150`) + equipped armor/shield `magical_bonus` (the +N). Body armor = `item_category == "armor"` in slot `"body"`; shield = `item_category == "shield"` in slot `"hands_off"`. Magic-armor WEIGHT reduction is `EncumbranceCalculator`'s job, not this one.
...
## 78. Class templates: build-time import of the Player's Companion catalog (2026-06-03)

The 31 per-class template tables in `rules/pc_class_templates.md` are imported to the runtime resource `data/templates/class_templates.json` by `tools/import_class_templates.py` — a §7.4 build-time extractor (rules/* is never read at runtime; the four out-of-scope classes Dwarven Machinist / Gnomish Trickster / Mystic / Thrassian Gladiator are skipped). `ClassTemplateRepository` (`engine/subsystems/characters/`, a non-autoload `class_name` service mirroring `EquipmentCatalog` / `MagicItemCatalog`) loads it into typed `ClassTemplate` / `TemplateProficiency` / `TemplateEquipmentEntry` resources (`engine/shared_types/`). `generation/gdd-class-templates.md` §6.2 is the schema of record. Patterns established:
...
- **PC creation template UI (§10 step 12, added 2026-06-06).** The §4.2 choice point + §4.2.1 editor land as a new step in the existing `CharacterCreationScreen` (renumbered 13→14): `ClassTemplatePanel` (`scenes/ui/character_creation/`, `class_name` + `extends VBoxContainer`, the standard `setup(state, …)` + `is_complete()` panel contract, UI built programmatically per §13.2). The panel is two-phase — a wealth roll (`DiceSystem.player_roll(6,3,0,"starting_gold",…)`, the SAME roll type the EQUIPMENT step uses) → Path A/B fork + template cards, then on a Path B card the full §4.2.1 editor (class-prof swap flagged as a departure, general swaps, cull-override, INT-bonus extra-general pickers) with a live preview that REUSES `PcTemplateCreationFlow.finalize_proficiencies` to render the exact committed result. **Decisions Jedidiah locked:** (a) "pick after template" — `CLASS_CUSTOMIZATION` (barbarian region / witch tradition) MOVED to after the template step in the enum; Path B locks origin/tradition (skip it), Path A keeps it for those 2 classes; (b) "full editor" — swaps surfaced, not just cull/extras. Patterns established:
  - **Step-ownership invalidation.** A fork that pre-fills multiple downstream outputs at once breaks the wizard's linear "each step owns the next's inputs" `_invalidate_from` model. Rule: the CLASS_TEMPLATE step OWNS `wealth_roll` + the Path A/B choice + (Path B) the template-derived proficiencies/spells/inventory/loose-coin; steps AT or BEFORE it wipe them all (`_clear_template_outputs`), steps AFTER it NEVER wipe template-owned fields — they reset only their own output and un-spend gold (`gold_remaining_cp → starting_gold_cp`). Post-template steps that run on Path B (FAMILIAR, divine SPELLS) guard their spells/inventory clears with `template_path != "B"`. EQUIPMENT keeps owning the gold roll ONLY for classes with no template (`template_path == ""` → it may clear `starting_gold_cp` for a fresh roll); when a template ran, EQUIPMENT only un-spends.
  - **Path A reconciliation = one roll (§4.1).** Path A writes `starting_gold_cp = gold_remaining_cp = roll×10×100`; `EquipmentShopPanel._restore_from_state` already hides its roll button when `starting_gold_cp > 0`, so the shop opens pre-funded — the wealth die is never rolled twice.
  - **finalize_proficiencies swap extension (PC-only).** `choices` gained `class_swap_key` + `general_swaps:{from→to}` (empty = no-op, so the NPC path is untouched). Swaps build FRESH records (rank 1, no specialization) rather than mutating the repo-cached `TemplateProficiency`; natural/tradition are LOCKED (the kind-`match` routes them to a no-swap branch); invalid keys are DEFENSIVELY ignored (validated vs the class/general lists). `swap_options(template)` feeds the dropdowns valid candidates only (`general_options` excludes every held key so a swap is always net-new). `is_locked()` ⇒ natural/tradition only; arcane_bonus IS swappable.
  - **Skip predicates key off `template_path`.** `_should_skip_{template,proficiencies,equipment}` + updated `_should_skip_customization`/`_should_skip_spells`. Path B skips PROFICIENCIES/EQUIPMENT always and SPELLS only when the template filled the repertoire (predicate keys off `spells` non-empty, so a DIVINE Path B keeps the normal SPELLS step). FAMILIAR is UNCHANGED — its existing "`familiar` prof present" predicate Just Works because Path B pre-fills proficiencies including `familiar`, so a Familiar template auto-shows the bonding step (and THAT step, not the panel, persists the familiar — the panel DROPS the non-catalog familiar to avoid double-create). `_should_skip_template` keys off `ClassTemplateRepository.get_templates_for_class(class_id).is_empty()` so the 4 out-of-scope classes fall back to EQUIPMENT's own gold roll.
  - **DEVIATION from the approved plan:** PC-side valuable/totem/poison non-catalog routing was DEFERRED (logged via `push_warning`, not routed to inventory) — the value-backed inventory shape (`save_character_inventory_with_creatures` + `value_cp`) wasn't confirmed safe for the PC finalize path, and a silent-but-wrong row is worse than a logged deferral; the NPC builder's `_route_non_catalog_items` is the reference for the follow-up. **Headless tests cover the flow layer** (swap/cull/extra/locked/invalid-swap added to `test_pc_template_creation_flow.gd`); the panel UI + wizard step-flow are verified by launching the wizard (programmatic panels aren't unit-tested, per §13).
