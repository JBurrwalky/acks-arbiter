...
- **Stamp on every persisted copy:** the voxel JSON payload gets a top-level `"generator_version"` key (stamped in `DungeonVoxelSerializer.to_voxel_dict`) and `dungeon_floors.generator_version` is stamped at insert (migration 210). A payload persisted before the stamp existed reads as version 0 via the missing-key default — deliberately equal to the current constant, so pre-stamp content never regenerates spuriously.
