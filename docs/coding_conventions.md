...
- **One derivation point.** The stage class owns a `derive_rng(master_seed) -> RandomNumberGenerator` static plus a documented `STREAM_OFFSET` const (first instance: `VerticalPlan.STREAM_OFFSET := 15485863` — chosen to be unreachable by every existing derivation incl. retry bumps). Callers and tests both obtain the rng ONLY from there; never construct the stream inline.
...
- **Two room-id spaces, one bridge.** The COMPOSED world (`VoxelCell.room_id`, `RoomZone.room_id`, `StairwellData.room_id`) speaks dungeon-GLOBAL room ids: `global = band_slot × DungeonVolumeComposer.ROOM_ID_STRIDE + local`, where `band_slot = floor_index − 1` (bands are planned ascending, guaranteed by `VerticalPlan.build`). The RELATIONAL per-floor tables (`dungeon_rooms.room_id_in_floor`, `monster_groups.room_id`, `treasure_hoards.room_id`, `key_items.placed_in_room_id`) speak per-band-LOCAL ids scoped by `floor_id`. Convert with the formula; never store a global id in a per-floor table or vice versa. Zones link to their contents by STRING `monster_group_id` / `treasure_hoard_id` — and the repository persists a group's/hoard's own generation-time id as its row PK so those links stay resolvable after reload. **CAVEAT (open, owned by the first runtime zone consumer — F.2d/G):** `VoxelMapData.from_dict` ends with `detect_rooms()`, which REASSIGNS every loaded cell's `room_id` to fresh per-level flood-fill ids — so a volume loaded from the persisted payload does NOT carry the global ids; only the in-memory generation-time volume and the `room_zones`/`stairwells` rows do. Any runtime consumer joining `room_zones.room_id` to a LOADED volume must resolve zones by their persisted `cells_json` + band walk level (positions survive the round-trip; room ids do not) or fix the loader to preserve serialized ids first.
