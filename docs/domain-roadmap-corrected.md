...
| **3** | Daily activity slots + Activities sub-tab (universal + domain category) | Player can spend daily/monthly time on RAW activities |
...
**UI:**
- `scenes/ui/notebook/tab_pages/domain_tab_page.gd` — replace empty-state with full nine-sub-tab strip (Overview / Stronghold / Garrison / Realm / Treasury / Activities / Class-Specific / Encounters / Departure Log) keyed to active entity. Implement entity strip filter (PCs + Humanoid Henchmen only) per `gdd-management-notebook.md` §6.1.
...
## Phase 3 — Daily Activity Slots + Activities Sub-Tab (Universal + Domain Category)

**Engine changes:**
- `engine/subsystems/activities/activity_slot_tracker.gd` — daily 1 major + 2 minor + unlimited trivial OR 8 minor + unlimited trivial; strenuous-day counter; overtime −1/day after 6 strenuous days; rest-day remit. All per `ax_campaign_play` §daily_capacity L146-150, §effort_rules L166-172, §overtime_rules L173-186.
- `engine/subsystems/activities/activity_catalog.gd` — registry of activity definitions (id, frequency, slot type, strenuous flag, location requirement, prerequisites).
- `engine/subsystems/activities/activity_executor.gd` — execute singular / start ongoing / advance ongoing per daily tick; **`[RESOLVED 2026-05-06]`** tick-tolerance heuristic (formerly O-D15) confirmed: each ongoing activity tracks `ticks_accumulated` (game-days of qualifying work) and `absence_accumulated` (game-days the character was unable or unavailable to perform it); the activity is **forfeit when `absence_accumulated > ticks_accumulated`** at any daily boundary check. This is an internal design model approximating RAW's "ongoing activities require continuous effort" rule (per `ax_campaign_play` §frequency_types.ongoing L159-163), not a direct RAW formula; it provides a deterministic, testable proxy for the rulebook's narrative "must be performed throughout the listed time period."
- `engine/subsystems/activities/handlers/` — one handler per RAW activity, e.g., `administer_domain.gd`, `issue_decree.gd`, `call_to_arms.gd`, `conscript_troops.gd`, `levy_militia.gd`, `hire_mercenaries.gd`, `inspect_troops.gd`, `train_troops.gd`, `oversee_troop_training.gd`, `oversee_construction.gd`, `oversee_investment.gd`, `manage_henchmen.gd`, `military_campaign.gd`, `solicit_mercenaries.gd`, `supervise_construction.gd`, **`[RAW PATCH]`** `repress_population.gd` (sets `domains.is_repressed_this_month=true` and `repression_gp_per_family_this_month` based on troops assigned; militia troops are not eligible per `acore_axioms` §repression L511).
- `data/activities/domain_category.json` — RAW spec for every activity in `ax_campaign_play.xml` §domain (~14 entries) including duration formulas (e.g., `administer_domain_days = 0.5 × (hex_count + vassal_count + (6 − market_class))` per `acore_axioms` §administration L526-529).

**Schema:**
- `058_activity_state.sql` — `activity_state(id, character_id, activity_id, status, ticks_accumulated, absence_accumulated, started_calendar_day, location_required, gp_committed)`.

**UI:**
- `scenes/ui/notebook/domain/sub_tabs/activities.tscn` — vertical activity-card list grouped by frequency (singular / ongoing / restricted); each card shows state / parameters / prerequisites / Execute button. Tooltip explains location-gating; "Plan travel to [location]" affordance integrates with future Travel sub-tab. Pre-9 entities see "domain followers/peasants unlock at level 9" empty-state; activities like administer/decree/hire mercenaries/invest are still allowed per `acore_axioms` §before_ninth_level L121 ("the character may still make investments to attract peasants or hire mercenaries").
- `scenes/ui/notebook/domain/activity_card.tscn` — three visual states (in-progress on-track / amber-warning absent-within-tolerance / red-urgent absent-near-forfeit) per GDD §15.1.7.

**Verification:** `tests/test_activity_slot_tracker.gd`, `tests/test_activity_executor_tick_tolerance.gd`, **`[RAW PATCH]`** `tests/test_repress_population_morale_cap.gd` (verify current morale cannot exceed 0 while repressed even with high repression bonus, per `acore_axioms` §repression L515). Manual: queue `administer_domain` (major ongoing), advance days, verify ticks accumulate; travel away, verify absence accumulates and warning fires when projected absence > remaining tolerance.
...
- Phases 2 + 3 can overlap once Phase 0 schema is settled; Phase 2 builds the shell, Phase 3 fills the Activities sub-tab.
