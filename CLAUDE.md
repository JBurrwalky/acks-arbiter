...
## First Task: Project Reorganization

The project files are currently flat in the root directory. Before any implementation work, reorganize into the directory structure above:

1. Create directories: `rules/`, `generation/`, `docs/`, `engine/`, `data/`, `test/`
2. Move all `*.xml` files to `rules/`
3. Move all `gdd-*.md` and `combat_behavior_tags.md` to `generation/`
4. Move `acks_arbiter_design_brief_v11.md` to `docs/`
5. Move `document_authority_architecture.md` to `docs/` (this is a reference document, now superseded by this CLAUDE.md but kept for historical context)
6. Leave `CLAUDE.md` and `build_log.md` in the project root
7. Create an empty `build_log.md` if it doesn't exist
8. Verify nothing was lost: count files before and after
9. Stage and commit: `git add -A && git commit -m "Reorganize project into rules/, generation/, docs/ directory structure"`

**Do not modify any file contents during reorganization.** Only move files.
