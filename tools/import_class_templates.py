...
def build(report=True):
    ...
    for class_name, rows, notes_text, line_span in parsed:
        ...
        for lineno, cells in rows:
            ...
            target = BAND_TARGET_GP[(lo, hi)]
            dev = (total_gp - target) / target if target else 0.0
            reports["wealth"].append((template_id, round(total_gp, 1), target,
                                      round(dev * 100, 0)))
...
def _print_report(seen_classes, templates, reports, overrides_raw):
    ...
    big = [w for w in reports["wealth"] if abs(w[3]) > 40]
    if big:
        for tid, val, target, devpct in big:
            print("  %-26s gp=%6.1f target=%5.1f dev=%+d%%"
                  % (tid, val, target, int(devpct)), file=out)
    else:
...
def main(argv=None):
    ...
    if args.check:
        new_text = json.dumps(result, indent=2, sort_keys=True,
                              ensure_ascii=False) + "\n"
        if not DEFAULT_OUT.exists():
            print("CHECK FAILED: %s does not exist." % DEFAULT_OUT)
            return 1
        old_text = DEFAULT_OUT.read_text(encoding="utf-8")
        if old_text != new_text:
            print("CHECK FAILED: data/templates/class_templates.json is stale. "
                  "Run: python tools/import_class_templates.py")
            return 1
        print("CHECK OK: class_templates.json is up to date.")
        return 0

    out_path = Path(args.out) if args.out else DEFAULT_OUT
    write_json(result, out_path)
    if not args.quiet:
        print("wrote %s (%d templates)" % (out_path, len(result["templates"])),
              file=sys.stderr)
    return 0
