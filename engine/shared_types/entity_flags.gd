...
##                  absorb magic missiles of the sort generated by spell
##                  or spell-like effect. A brooch can absorb up to 101
##                  points of damage from magic missiles before it melts
##                  and becomes useless." Metadata:
##                  {absorbs_spell_keys: ["magic_missile"],
##                  total_capacity: 101,
##                  charges_remaining: <from row.uses_remaining>,
##                  item_id}. Consumer in CastingResolver._apply_damage:
##                  when spell_key == "magic_missile" AND target carries
##                  this flag, redirect damage from target to brooch's
##                  charge counter; when counter hits 0, remove the
##                  inventory row ("brooch melts and becomes useless").
##                  default_charges = 101 stamped via FIXED_DEFAULT_CHARGES.),
##               "has_elven_cloak" (Elven Cloak — ACKS Core p.215+
##                  Jedidiah-supplied RAW 2026-06-03: "Adds a +8 bonus
##                  to any proficiency throws to hide in shadows.
##                  Characters wearing elven cloaks can always hide in
##                  shadows with a throw of at least 12+." Metadata:
##                  {skill_key: "hide_in_shadows", bonus: 8,
##                  ceiling_target: 12}. Engine: ModifierContainer
##                  entries on `hide_in_shadows_magical_bonus` (+8 add)
##                  and `hide_in_shadows_ceiling_target` (set 12).
##                  ThiefSkillResolver._build_skill_check reads both at
##                  skill-check assembly time.),
##               "has_elven_boots" (Elven Boots — ACKS Core p.215+
##                  Jedidiah-supplied RAW 2026-06-03: "Adds a +8 bonus
##                  to any proficiency throws to move silently.
##                  Characters wearing elven boots can always move
##                  silently with a throw of at least 12+." Metadata:
##                  {skill_key: "move_silently", bonus: 8,
##                  ceiling_target: 12}. Same mechanic as Elven Cloak,
##                  different skill key.),
