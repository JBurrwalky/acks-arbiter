class_name GameLogStore
extends RefCounted

## GameLogStore — in-memory ring of game events for one party.
##
...
##
## Owned by the GameLog autoload (one store per active party). The autoload
## is the public interface; GameLogStore is its internal state.
##
## Renamed from GameLog (the previous class name) to free the GameLog
## identifier for the autoload Node. See gdd-ui-architecture.md §6.1.
