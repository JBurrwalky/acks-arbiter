...
static func launch(plot_id: String, liege_character_id: String, day: int,
	...
	# be SQL NULL (realm-mirror rows don't set every field), so coerce through StringUtils.s()
	...
	var rebel_head: String = StringUtils.s(instigator_faction.get("leader_npc_id"), "")
	var rebel_realm_id: String = RealmRepository.create_realm({
		"campaign_id": campaign_id,
		"name": "Rebel Coalition of %s" % StringUtils.s(instigator_faction.get("name"), "the Vassals"),
		"head_character_id": rebel_head,
		"alignment": StringUtils.s(instigator_faction.get("alignment"), "neutral"),
		"dominant_religion": StringUtils.s(instigator_faction.get("religion_id"), ""),
		"culture": StringUtils.s(instigator_faction.get("culture_id"), ""),
		"realm_kind": "tracked",
	})
