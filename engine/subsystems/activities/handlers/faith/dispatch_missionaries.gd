...
##
## State.params_json shape: {"gp_committed": <int>}.
## Treasury was already debited at launch (the player committed gp). The
## handler just accrues the gp into congregants.monthly_growth_pending_gp.
...
static func on_complete(state: Dictionary, _runner) -> Dictionary:
	...
	if gp_committed <= 0:
		return {"summary": "dispatch_missionaries completed (no gp committed)"}

	CampaignRepository.add_congregant_pending_gp(character_id, gp_committed)
	EventBus.missionary_dispatch_recorded.emit(character_id, gp_committed)
	...
	if not domain_id.is_empty():
		CampaignRepository.add_ledger_entry({
			...
			"description": "Missionary wages — %d gp committed; rolls into next-month congregant growth" % gp_committed,
		})

	return {
		"summary": "Missionaries dispatched: %d gp tracked for next-month growth" % gp_committed,
		"presentation": {"type": "toast", "text": "Missionaries dispatched (%d gp)" % gp_committed},
	}
