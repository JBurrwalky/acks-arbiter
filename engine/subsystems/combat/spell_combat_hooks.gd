...
func on_pre_attack(
		attacker: Variant, target: Variant, attack_type: String) -> Dictionary:
	...
	# --- Scrolls of Warding bearer-melee-out dismissal (2026-06-03) ---
	# Per RAW (acore_treasure_and_magic_items_rules.xml:268-272): "The
	# protection lasts until dismissed or until anyone inside the area
	# attempts melee against a protected creature type." When the ATTACKER
	# carries a `warded_against_creature_type` flag (= attacker is the
	# bearer) AND the attack is MELEE AND the target's creature_type
	# matches one of the bearer's warded types, the bearer-melee-out
	# dismissal fires: each matching ward source is cleared. The attack
	# itself still proceeds normally (the ward is now gone for the rest
	# of combat). Non-melee attacks (ranged, spells) by the bearer do
	# NOT dismiss the ward per RAW.
	if attacker != null:
		var attacker_flags: EntityFlags = attacker.get_flags() if attacker.has_method("get_flags") else null
		if attacker_flags != null and attacker_flags.has_flag("warded_against_creature_type"):
			var attack_is_melee: bool = String(attack_type).to_lower() == "melee"
			if attack_is_melee and target != null:
				var ward_entries: Array = attacker_flags.get_flag_source_entries(
					"warded_against_creature_type")
				var sources_to_clear: Array = []
				for entry in ward_entries:
					var ward_meta: Dictionary = entry.get("metadata", {})
					var warded_types: Array = ward_meta.get("creature_types", [])
					if warded_types.is_empty():
						continue
					if _target_matches_warded_types(target, warded_types):
						sources_to_clear.append(String(entry.get("source_id", "")))
				for sid in sources_to_clear:
					if not sid.is_empty():
						attacker_flags.clear_flag("warded_against_creature_type", sid)
...
## Returns true if [param entity]'s creature_type matches any of
## [param warded_types]. Uses is_creature_type() when available, falling
## back to direct field inspection for test fixtures. Static helper so
## both MovementResolver (entry block) and SpellCombatHooks (bearer-melee-
## out dismissal) can reuse it.
static func _target_matches_warded_types(entity: Variant, warded_types: Array) -> bool:
	if entity == null:
		return false
	for t in warded_types:
		var t_str: String = String(t).to_lower()
		if entity.has_method("is_creature_type"):
			if entity.is_creature_type(t_str):
				return true
		if "creature_type" in entity:
			if String(entity.creature_type).to_lower() == t_str:
				return true
		if "tags" in entity:
